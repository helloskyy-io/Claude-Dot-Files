...
**The hook lives in two tiers, and the upper one cannot be loosened from below.** `install.sh` symlinks it into `~/.claude/hooks/` (the user tier, which you own and extend freely — add `permissions.allow` rules at will) *and* copies it root-owned to `/etc/claude-code/hooks/`, declared by `config/managed-settings.d/claude-dot-files.json` placed at `/etc/claude-code/managed-settings.d/`. Claude Code ranks that managed floor above every user, project and CLI setting; a managed hook fires under `--dangerously-skip-permissions` and a managed `deny` cannot be loosened by a user `allow` — both **measured**, not read off the docs (`scripts/helpers/managed-tier-probe/`). The floor is thin by ruling: the safety hook and the deny set (empty since 2026-08-15), nothing else. Placing it needs root, so `install.sh` uses `sudo` and **refuses loudly, naming the path and the missing privilege, when it cannot** — a machine without the floor is one somebody chose (`--without-managed-floor`), never one the installer fell through on — and it reports success only after verifying the placed floor is root-owned, writable by nobody below root, and readable by everybody below root, file and directory alike, because a floor the user can rewrite is not a floor and a floor the user's session cannot read never loads. Edit the hook in `config/`, re-run `./install.sh`, and the root-owned copy is refreshed.
