...
- For research standards, refer to `docs/standards/research/` — **vendored (MIRROR)**. Research is EVIDENCE, never binding; pools live at TWO altitudes — `docs/architecture/research/` for findings that could change WHAT we build, and `docs/development/phases/<phase>/research/` for the ~98% that decide HOW to build something already committed to.
