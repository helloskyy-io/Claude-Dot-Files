...
- **For how this repo's configuration reaches a machine — and what it owes as it grows — refer to `/opt/skyy-net/skyynet-master-planning/standards/architecture/declarative_convergence_standard.md`.** SkyyNet adopts it; its declared position is `standards/architecture/declarative_convergence_position.md` in that repo, and **this repo is the system that position describes** — `config/` is the declaration and `install.sh` is the agent. Binding here: nothing is configured by hand in `~/.claude/`, everything deployed is declared in `config/`, and no change may make the two unmet principles harder to reach.
- For workflow script standards, refer to `/opt/skyy-net/skyynet-master-planning/standards/workflows/workflow-scripts.md`
- For research standards, refer to `/opt/skyy-net/skyynet-master-planning/standards/research` — **owned by SkyyNet; amendments are made there**. Research is EVIDENCE, never binding; pools live at TWO altitudes — `/opt/skyy-net/skyynet-master-planning/standards/architecture/research` for findings that could change WHAT we build, and `development/<component>/research/` for the ~98% that decide HOW to build something already committed to.
