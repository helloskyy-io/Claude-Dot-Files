...
2. **Open Issues by label.** `gh issue list --state open --json number,title,labels,body`. Bucket by label — especially `research-required` and `evidence-faulty` (planning-STOP outcomes filed by plan-new/plan-revision), plus any other stop-class labels. Each open issue is a pending decision by construction. If the body carries a `plan_stop:` yaml block (same schema family as `pr_review:`), parse it and surface its `next_steps` verbatim — those are the ready-to-fire options (research-first / waiver re-dispatch) the operator picks between. (This surface may be empty — report nothing rather than inventing items.)
