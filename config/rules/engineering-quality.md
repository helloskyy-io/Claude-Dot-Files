...
3. **Place the deferral, do not park it.** [Tracked Items Standard § 6](../../docs/standards/documentation/tracked_items_standard.md) is binding — it supersedes Documentation Standard § Deferred Work — and its placement order comes FIRST: resolve it now if the context is loaded; a trigger-gated item is a **checkbox on the phase that owns the trigger**; a related one **expands the existing item**; only what fails all three reaches a tracked store, and **§1.1 decides which of the four**. **A carried-work ledger — a loose-ends file, a per-sprint deferral list — is a VIOLATION, not a fallback.** "Deferred" without a placement is silent dismissal.
...
3. **Place it — last resort, and NEVER into a ledger.** Only when ALL of: (a) genuinely out-of-scope for current work, (b) non-blocking, (c) large enough to warrant a separate context-rebuild later, (d) won't be reached this session. Then the two placement questions decide *where*: a trigger-gated item is a **phase checkbox**; a related one **expands the existing item**; a deferral carrying watch-criteria goes in the **CPI decisions log**; only what fails all three reaches a **tracked store** — `tracked/issues/` for a defect, `tracked/candidates/` for a proposal, `tracked/standards/` for an amendment to a named standard. **`tracked/operations/` is human-only and is never where a machine places anything.**

**One shape reads like a deferral and is not one: the remedy is ALREADY FILED, and your only new information is that it RECURRED. Increment its `count` and append a dated recurrence line** ([Tracked Items §3.1](../../docs/standards/documentation/tracked_items_standard.md)). Do not open a second item, and do not merely mention it in a PR body nobody aggregates.

> **THIS RULE USED TO SAY THE OPPOSITE, AND THE REVERSAL IS THE POINT.** It read: *"report the recurrence with its count where you are already working — the PR body, the reflection — and **edit nobody's row**."* That was correct while the stores were tables: amending another filer's row collides with column ownership, and re-filing duplicates, so reporting-in-place was the only move left. **It was routing around a missing integer.** `count` is that integer, and **incrementing is not amending** — a count is not reasoning, and a recurrence note is additive, so neither of the two objections reaches it.
>
> **Recurrence now outranks age in triage.** A `count: 3` filed last week is a pattern; a `count: 1` from June may be noise. That signal was previously unrepresentable, which is why three consecutive review passes each independently invented the same workaround — and why the workaround is now deleted rather than kept beside its replacement.
