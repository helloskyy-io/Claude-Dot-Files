...
**One shape reads like a deferral and is not one: the remedy is ALREADY FILED BY SOMEONE ELSE, and your only new information is that it RECURRED.** Report the recurrence with its count where you are already working — the PR body, the reflection — and **edit nobody's row.** Amending another filer's entry collides with the column-ownership rules every workflow here carries; re-filing duplicates. **This is not a fourth placement — it is a report, and it creates no store**, which is why it does not breach the rule below. Three consecutive review passes hit this shape and each independently invented the same answer, which is the evidence it belongs written down rather than re-derived.
