...
**Autonomous runs MAY write there, by the routing in `skyynet-master-planning/standards/findings/finding-routing.md` § 7** — a research run appends papers and files candidates into `skyynet-master-planning/tracked/candidates/`, a producing run places a proposal it surfaced there, `triage-candidates` rules a candidate's `decision`, and a `requires review` ruling stays on the candidate as the operator's queue. **What stays human-only is `ratification` on a standards candidate**: applying or rejecting a proposed amendment is the ruling this rule exists to protect.

> **This sentence used to also claim `decision` was human-only, and that was WRONG in a way that mattered** — it contradicted `finding-routing.md` §7, the document it cites in the same breath, which has always given `decision` to a workflow. A reader could take either half and be quoting a binding rule. Corrected 2026-08-12 when `triage-candidates` was split out of `plan-sprint`: **`decision` on a candidate is an automated ruling with a named single writer; `ratification` on a standards candidate is the human one.** Nothing was widened here — the contradiction was resolved toward the routing standard that owns the question.
