...
- **WRITE `citations.json` AT THE ROOT OF THE POOL YOU WROTE INTO — one file per run, and append to it rather than replacing it if it already exists.** A JSON array of `{"claim_id", "quote", "url", "sha", "paper"}`, one entry per span you byte-verified, where `quote` is the span you saw VERBATIM in what that URL returned, `url` is the EXACT raw URL your `curl … | grep -F` hit (the SHA-pinned `raw.githubusercontent.com/…/<sha>/<path>` form, never the blob page), `sha` is that 40-hex commit (omit it for a page that has none), `paper` is the `raw/<topic>.md` the claim lives in, and `claim_id` names the claim it supports. **This is the ONLY machine-readable pairing of a claim to its source**, and a fleet activity uses it to fetch and store each cited source before the run exits, so the paper can be re-checked offline later. **A paper that cites sources with no sidecar beside it marks the run's record INCOMPLETE** — write the file, then confirm it parses.
