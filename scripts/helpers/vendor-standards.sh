...
# A TARGET MAY CARRY A TAIL, AND THE TAIL IS ITS OWN.
#
# `FILES` is the BASELINE — what any repo operating in this tooling needs
# regardless of what it builds. A consumer often needs more: `image-manager`
# vendors five stack-specific standards (image-pipeline, kubernetes, secrets,
# logging, networking) chosen by its own plan's scope analysis.
#
# THOSE CANNOT GO IN `FILES`, and that is why this exists rather than an array
# edit. `FILES` is global, so adding them would try to vendor Kubernetes rules
# into a planning repo that has no cluster. The tail belongs to whoever planned
# the component — the baseline is ours to decide, the tail is theirs.
#
# WITHOUT THIS, `--check` RETURNING CLEAN MEANS "the seven I manage are clean"
# and says nothing about the other five, ever. `image-manager` measured four of
# its five as byte-faithful but STALE, and one carrying a local body edit — none
# of which any check would have reported.
#
# Format, one per line, `#` comments and blanks ignored:
#   <OWNER>:<source-relative-path>:<destination-relative-path>
MANIFEST="${DEST_REPO}/.vendored-standards"
if [[ -f "$MANIFEST" ]]; then
    ...
    echo "· ${#FILES[@]} entries: baseline plus this target's .vendored-standards"
fi

# THE OWNER'S OWN HEADER IS STRIPPED ON THE WAY OUT. An owned file opens with an
# `OWNED HERE` block saying "amendments are made here" — correct at the source and
# FALSE in a copy, where "here" resolves to the copy. Left in, a mirror opened with
# "DO NOT EDIT LOCALLY" and then, four lines down, told the reader to amend it. The
# generated banner already names the owner and the source path, so the block is
# redundant in a mirror as well as wrong.
#
# `--check` MUST APPLY THE SAME TRANSFORM or it reports every stripped mirror as
# DRIFTED forever. That is why this is a function rather than a pipeline inline in
# the write path — the two callers comparing different bodies is precisely the bug.
mirror_body() {
    awk 'BEGIN{skip=0}
         /^<!-- OWNED HERE/{skip=1; next}
         skip && /^---$/{skip=0; next}
         skip{next}
         {print}' "$1" | sed '/./,$!d'
}
...
for entry in "${FILES[@]}"; do
    ...
    if $CHECK_ONLY; then
        ...
        elif [[ -z "$n" ]] || ! diff -q <(tail -n +$((n + 2)) "$dst") <(mirror_body "$src") >/dev/null; then
