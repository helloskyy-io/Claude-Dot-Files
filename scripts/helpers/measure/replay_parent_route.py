...
def report(rows_: list[dict], log_dir: Path, total_logs: int) -> None:
    ...
    print("## THE DENOMINATOR'S OWN LIMIT — candidate C-060, and no N removes it")
    print("   Every row above comes from a run that reached `append_parent_route`, which")
    print("   sits AFTER `run_claude`'s completion gate. A dispatch whose child emitted a")
    print("   valid typed record and no prose `VERDICT:` line raises at that gate, before")
    print("   `route()` — so it never becomes a row here. `channels_agree` is therefore")
    print("   conditioned on the incumbent prose channel having SUCCEEDED, and the")
    print("   instrument built to weigh prose against the typed record is structurally")
    print("   blind to prose's own failure mode. Phase 3's run set is the measured")
    print("   instance: nine live dispatches, eight recorded.")
    print("   READ THE AGREEMENT FIGURE ABOVE AS CONDITIONAL, NOT AS A RATE.")
    print()
