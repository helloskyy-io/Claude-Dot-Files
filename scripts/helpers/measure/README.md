...
anything but `.claude/logs/`, the run journal's bags, `gh` output, and — through
`planning_evidence.py` only — the planning corpus's tracked stores and CPI log.
They are read-only over local artifacts.

| Tool | Answers | Read by |
|---|---|---|
| `replay_completion_predicate.py` | How often does the fleet's completion grep miss a real terminal outcome? | Open standards candidate `skyynet-master-planning/tracked/standards/S-fnpuwoy4.md`; Memory Management Framework [Phase 1](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/memory-management-framework/phase1_measure_the_channel.md) E5 |
| `replay_pr_review_blocks.py` | How often is the finding-set delta between consecutive `review-pr` passes empty, and does the stable-id convention hold? | Memory Management Framework [Phase 1](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/memory-management-framework/phase1_measure_the_channel.md) E7; [Phase 5](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/memory-management-framework/phase5_convergence_stopping.md) |
| `replay_convergence_predicate.py` | Over the whole archive, how often would the shipped convergence predicate have fired, how often *early*, and does it reproduce the model-asserted `converged` flag? | Memory Management Framework [Phase 5](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/memory-management-framework/phase5_convergence_stopping.md) § Measurement and § What would let this gate |
| `replay_run_resources.py` | What fraction of runs could not be measured and why; how `peak_anon` distributes by workflow; whether footprint tracks subagent COUNT or content VOLUME; and what concurrent runs sum to | Memory Management Framework [Phase 6](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/memory-management-framework/phase6_read_what_it_writes.md) requirement 3, and requirement 4's retraction re-check |
| `replay_convergence_events.py` | What the LIVE predicate actually said — including the `pass_not_evaluable` and `history_unreadable` rates the GitHub archive structurally cannot produce — and **Phase 5's gate conditions 1 and 2 with their denominators** | Candidate **C-wb1xc1xs**; [Phase 5](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/memory-management-framework/phase5_convergence_stopping.md) § What would let this gate; [Phase 6](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/memory-management-framework/phase6_read_what_it_writes.md) § Phase 5's un-owned activation |
| `judge_marginal_yield.py` | Whether the separate review pass finds anything the producing run had not already disclosed — each disposition finding classified ECHOED or NEW against the run's own reflection bullets, with the denominator and the lexical-matching bias stated | [`sprint.md`](/opt/skyy-net/skyynet-master-planning/development/sprints.md) § *Continuous Process Improvement*, *measure the judge's marginal yield*; the 2026-08-14 deferral asking whether four review layers earn their cost; `self_report_measure.py`, which imports its matching rule for E1b |
| `journal_baseline.py` | The per-child baseline over the JOURNAL's bags: F1–F4 and three trajectory figures per child, F5–F7 for `review-pr`, each a distribution with its window; coverage by the imported `assess_completeness`; the gapped-bag count; the sweep's wall-clock against journal size | Self Improvement [Phase 1](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/self-improvement/phase1_the_reader.md) (the reader); PMP [Phase 1](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/persistent-memory-protocol/phase1_the_run_bag.md)'s no-database revisit trigger (r9's wall-clock); candidate **C-wb1xc1xs** (F7 reads `convergence.agrees` live) |
| `journal_evidence.py` | Not a question — the READ INTERFACE the journal readers go through: enumerate evidence units, read one, state what it lacks; a `review-pr` pass's typed findings; a PR's thread from the harvest, or from the forge through the harvest's own fetch. The only module here that knows a bag is a directory | `journal_baseline.py`; `self_report_measure.py` (Self Improvement [Phase 2](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/self-improvement/phase2_the_self_report_and_recurrence_measured.md) r4, which reads every record through it) |
| `planning_evidence.py` | Not a question — the READ INTERFACE over the planning corpus's deferral records: the tracked stores (through `tracked_items.parse`) and the CPI log's DEFERRED entries. The only module here that knows where those files are | `self_report_measure.py` |
| `self_report_measure.py` | Does an asserted signal agree with a computed one? Three rates: E1b — typed findings already stated by the producing runs vs new (the `judge_marginal_yield.py` rule, imported, cut at the pass's own comment); the share of reflection recurrence claims a tracked `count` or a recurring finding id corroborates; and the V1 calibration ratio over the CPI log and the tracked stores, censored at 90 days. Each with its method, its bias direction, a hand-sample disagreement rate, and the model-read sweep's hypotheses applied beside — never inside — the computed figure | Self Improvement [Phase 2](/opt/skyy-net/skyynet-master-planning/development/edge-assistant/self-improvement/phase2_the_self_report_and_recurrence_measured.md) r1–r7; synthesis candidate 7, the negative (the asserted channel is not weighted until its agreement is measured) |
