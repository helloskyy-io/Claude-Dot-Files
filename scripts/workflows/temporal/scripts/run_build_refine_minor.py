"""Kickoff entrypoint for build-refine-minor — the light tier's review pass.
...
Differs from `run_build_draft.py` because `--pr` is REQUIRED. `run_refine_minor` takes
...
Differs a second time in what it DOES NOT expose. `run_refine_minor` also takes
...
Does NOT differ from `run_build_refine.py` in any respect but the three coupled
names — the module it calls, `WORKFLOW_KEY`, and the `prog`. The two tiers differ
in turn budget and in how many review lenses the CHILD runs, never in what a
refine invocation IS, so the CLI is deliberately identical: an operator who
...
from modules.assistant.build.build_refine_minor.build_refine_minor_workflow import (  # noqa: E402
    run_refine_minor)

BANNER = "=" * 64

WORKFLOW_KEY = "build-refine-minor"


def parse_args(argv: list[str] | None = None) -> tuple[BuildInput, bool, bool]:
    parser = argparse.ArgumentParser(
        prog="build-refine-minor",
        description="Review and correct an existing PR in FRESH context, with "
                    "ONE review lens. The run that authored a change never "
                    "defends it.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"\n{BANNER}\n  BUILD-REFINE-MINOR COMPLETE — PR #{task.pr_number} "
          f"reviewed and corrected\n{BANNER}", file=sys.stderr)
