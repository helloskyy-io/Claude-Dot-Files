#!/usr/bin/env bash
# build-refine — kickoff shim for the Python workflow.
#
# Thin by design: it resolves the interpreter and hands every argument through
# untouched. Argument parsing lives in run_build_refine.py so there is exactly one
...
#
# Usage:
#   ./build_refine.sh --pr 42 "the ORIGINAL task"
#   ./build_refine.sh --pr 42 --task-file /tmp/claude-<name>.md --correction-pass
#   ./build_refine.sh --pr 42 --dry-run   # no model, no worktree, no spend
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec python3 "${SCRIPT_DIR}/run_build_refine.py" "$@"
