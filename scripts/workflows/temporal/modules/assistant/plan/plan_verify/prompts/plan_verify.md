...
- **Every phase gets one — gated and COMPLETE phases included.** A phase already complete gets **`~0 hrs remaining — COMPLETE`** plus a sentence saying why. An estimate is a forecast of work, a shipped phase has no work left to forecast, and the cost it actually took is history that belongs in no total; a zero states that without inviting anyone to add anything.
- **Write the zero rather than omitting it, and this is not a style preference.** The unsized check counts **phase-doc FILES ON DISK**, and a complete phase still has its doc. Omit its figure and the run fails as `UNSIZED` at the last guard, losing everything else it did correctly. **A complete phase is outside the estimates you write and INSIDE the count you are measured against** — those two totals are not the same total.
