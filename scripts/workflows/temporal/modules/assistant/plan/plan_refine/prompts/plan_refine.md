...
8. **The PROJECT-level research pool and its synthesis** — normally `/opt/skyy-net/skyynet-master-planning/standards/architecture/research`, confirm against the map. Question 4 asks whether a phase's evidence supports it; a phase can also rest on something the project settled differently at a higher altitude, and only this shows you that.
9. **`/opt/skyy-net/skyynet-master-planning/development/sprints.md` — READ IT, and it is READ-ONLY.** You are producing the numbers that feed it, against a sprint calibration, and you cannot judge whether a phase is sized like the work around it without seeing that work. **You may never write it** — see the authorization table.
...
- **EVERY phase gets one — no exception, not a gated phase and not a COMPLETE one.** Size a complete phase for the work it contained, from its doc, the way you size any other; one nobody ever sized is the ordinary case, not an edge one. You re-size it every run and that is intended: an estimate was never set in stone, so a figure that moves a few hours has lost nothing.
- **Sizing a complete phase does not inflate what is left.** `plan-sprint` derives a TOTAL of every estimate and a TO-DO that subtracts every phase whose heading carries `✅` — the complete phase lands in the first and never the second. **Omitting its estimate protects nothing and costs two things:** the total then describes less than the component cost, and the unsized check counts phase-doc FILES ON DISK, so the run fails at its last guard.
