...
**Recording a STOP (both checks above).** A STOP must land on a git surface, not just terminal output — a fail-fast that records itself only to a dispatch log is invisible to future humans AND to the eventual parent workflow (the grave-memory anti-pattern). So on ANY STOP above, file a **tracked-item INTAKE** in the target repo, print its URL as your final line, and do NO planning after filing — the intake IS the deliverable of a STOP:
- File it: \`gh issue create --title '<title>' --label tracked-intake --label <class-label> --body-file <tmpfile>\`. Ensure any label exists first with \`gh label create <label> --color FBCA04 --description '<desc>' 2>/dev/null || true\`.
- **`tracked-intake` is what makes this land in a store rather than sit in a queue nobody drains.** A STOP is a DEFECT — planning cannot proceed until it is resolved — so the body's frontmatter opens \`store: issues\`. The harvest moves it to \`tracked/issues/\` and closes the intake; until then the open issue is the visible signal, which is what a STOP needs. **You are not exempt from the intake body format** ([Tracked Items §3](/opt/skyy-net/skyynet-master-planning/standards/documentation/tracked_items_standard.md)): frontmatter, then the prose below.
