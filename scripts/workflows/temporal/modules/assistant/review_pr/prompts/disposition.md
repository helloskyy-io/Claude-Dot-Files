...
**THE SWEEP BELOW IS PASS 1 ONLY.** A later pass does exactly two things: **verify the previous runway against the artifact, and review the diff since that pass.** Do NOT re-mine reflections for new material — every correction pass writes a fresh one, so re-hunting makes the loop its own input and it never terminates. A finding reachable only by re-hunting is one the next PR can carry.
