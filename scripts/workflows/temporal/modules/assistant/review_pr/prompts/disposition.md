...
**ASYMMETRIC-PRESENCE SWEEP — the co-equal of the sweep below, and it fires far more often.** That one asks *what did this PR REMOVE, and where did it go?* This one asks *what does this PR have in ONE place and not its siblings?* — a rule added to one prompt of a pair, a guard registered for one member of a class, a figure updated in one of two documents that state it. **Produced the headline finding on two separate PRs and neither was reachable by the deletion sweep**, because nothing was deleted: something was added to one side only. Run it on every artifact this PR touched that has a sibling.
...
|---|---|---|---|
| `build.sh` | **opus** | 250 + 250 | **five** — code-reviewer, refactoring-evaluator, standards-auditor, security-auditor, quality-control |
| `build-minor.sh` | **sonnet** | 100 + 100 | **one** — code-reviewer |
| `plan-revision.sh` | opus | 300 | doc/plan edits |

**The model is the difference that matters and it is NOT a speed or cost choice.** `-minor` is a materially less capable reasoner. A correction can be small, known and scoped and still need judgement — deriving a set and ruling which exclusions are legitimate, stating a defect's consequence rather than its mismatch, choosing where a finding belongs. **Scoped-and-known is necessary for `-minor`, never sufficient. If the fix needs a DECISION rather than an EDIT, name `build.sh` however small the diff.** And note the second column: one lens means a `-minor` correction is seen by one reviewer before it reaches you.
