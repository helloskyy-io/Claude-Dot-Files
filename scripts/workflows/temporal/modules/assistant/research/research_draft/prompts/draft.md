...
Then:
- Locate and READ the repo's research standard (expected at `/opt/skyy-net/skyynet-master-planning/standards/research/research_standard.md` or the repo's equivalent — check CLAUDE.md / the docs index). If NO research standard exists in this repo, STOP and report it — the artifact contract is a required input, not something to improvise.
...
- **Tell the analyst these §3 obligations are binding and non-negotiable, and let it read the rest from the standard:**
  ...
  - **EVERY SPAN IT JUST VERIFIED GOES INTO `${RESEARCH_DIR}/citations.json` — a JSON array of `{"claim_id", "quote", "url", "sha", "paper"}`, one row per verified span, `url` the exact raw URL the `curl` above hit, `sha` the 40-hex commit in it (omit for a page with none), `paper` the `raw/<topic>.md` the claim lives in.** The analyst's own definition carries the full spec. **This is the ONLY machine-readable pairing of a claim to its bytes**: a fleet activity reads it after the run to store every cited source so the paper can be re-checked offline, and a paper with citations and no sidecar marks the run's record INCOMPLETE. The run already holds every tuple at the moment it runs the `grep -F`; the file is where it puts them down. Never invent a span to fill a row.
...
- After the analyst returns, checkpoint-commit the paper **and confirm `${RESEARCH_DIR}/citations.json` exists and parses** (`python3 -c 'import json,sys; json.load(open(sys.argv[1]))' <path>`). An analyst that verified spans and wrote no sidecar has left the run's evidence unrecoverable; re-dispatch it for the file alone.
- **`citations.json` IS ONE FILE PER RUN.** Before the first analyst dispatch, delete any `citations.json` a previous run left at the pool root — analysts APPEND within a run, and a stale file would make this run claim sources it never verified.

**WRITE BOUNDARY (binding).** You write ONLY inside ${RESEARCH_DIR}, and inside it only `raw/`, `synthesis.md`, `topics.md` and `citations.json`. Never edit a roadmap, phase doc, sprint file, or standard; **never write into any of the four `tracked/` stores** — not `issues/`, not `candidates/`, not `standards/`, and never `operations/`, which is human-only; never file a `tracked-intake` issue; never touch `direction.md`. **The researcher researches, the planner plans, the reviewer triages.** Anything your paper surfaces that looks actionable is SURFACED in the paper and goes no further. A research run that surfaces a finding and stops is FINISHED behaviour, not incomplete behaviour.
