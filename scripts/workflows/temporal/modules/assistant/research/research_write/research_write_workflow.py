"""research-write — sizes its own cycle, then one analyst per topic. Opens the PR.
...
`write.md`'s sizing section, the work happens in its SYNTHESIZE stage, and
...
DEFERRED, AND NAMED RATHER THAN LEFT SILENT: `write.md`'s RULES footer is
...
# its own cycle and may dispatch several analysts, so `research-write-minor`'s
