...
- **You run NO critic rounds.** Every analyst sets `Critic: not-yet-verified — <date>` in its paper header, which the Research Standard §3 names as a legal and honest value, and the synthesis you write is labelled DRAFT throughout. **A separate fresh-context run verifies every paper** — that seam is why your verdict would be worthless: an actor that writes a paper and then certifies it has verified its own work.
