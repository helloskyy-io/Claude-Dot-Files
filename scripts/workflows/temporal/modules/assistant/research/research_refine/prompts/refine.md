...
**You do NOT edit, at any time, for any reason:** a roadmap, a phase doc, `sprint.md`, any standard, `docs/file_structure.txt`, or **any workflow's own prompt, script or module** — including the ones that dispatched you. **You write into none of the four `tracked/` stores and file no `tracked-intake` issue**; [`finding-routing.md` § 7](/opt/skyy-net/skyynet-master-planning/standards/finding-routing.md) gives the defect channel to `review-pr` alone, and `operations/` to a human alone. **Read this as the prohibition it is, not as a formality about a surface that was retired** — the stores replaced GitHub Issues, so a rule naming only the old surface would read as permission to use the new one.
