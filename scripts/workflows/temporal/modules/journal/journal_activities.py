...
literally true before the first side effect. TEN of this fleet's sixteen
...
that cuts one — so in fourteen of sixteen a bag opened inside the workflow module
...
fourteen of sixteen, and `test_the_worktree_cutting_count_this_argument_RESTS_ON` now
...
def load_journal_config(config_path: Path | None = None) -> Mapping[str, object]:
    ...
    escape would give all sixteen entrypoints a raw traceback for the one class of
    ...
    # not a `RuntimeError`, so none of the sixteen entrypoint handlers catches it
...
def open_run_bag(*, run_id: str, writer: str | None, repo_root: Path,
    ...
    or tag-file write, raising a bare `OSError`. Fifteen of the sixteen entrypoints
