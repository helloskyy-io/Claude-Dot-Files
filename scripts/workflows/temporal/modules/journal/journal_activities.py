...
literally true before the first side effect. ELEVEN of this fleet's seventeen
...
that cuts one — so in fifteen of seventeen a bag opened inside the workflow module
...
fifteen of seventeen, and `test_the_worktree_cutting_count_this_argument_RESTS_ON` now
...
def load_journal_config(config_path: Path | None = None) -> Mapping[str, object]:
    ...
    escape would give all seventeen entrypoints a raw traceback for the one class of
    ...
    # not a `RuntimeError`, so none of the seventeen entrypoint handlers catches it
...
def open_run_bag(*, run_id: str, writer: str | None, repo_root: Path,
    ...
    in the eleven-entrypoint fleet of 2026-08 nine of them omitted it — so nine
    runs in eleven would have
    ...
    or tag-file write, raising a bare `OSError`. Sixteen of the seventeen entrypoints
