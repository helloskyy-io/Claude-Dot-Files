...
- **Don't rely on Bash CWD persistence between calls:** the Claude Code harness documents CWD as persistent across Bash tool calls, BUT certain configurations (custom statusLine command, subprocess invocation patterns) silently disable persistence. Defensive practice: always chain with `&&` in the SAME Bash call (`cd lib/temporal && pytest tests/unit/`) or use absolute paths. The defensive form works regardless of whether persistence is active in the current environment.
