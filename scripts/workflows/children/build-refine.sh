#!/usr/bin/env bash
#
# build-refine.sh — the BUILD-REFINE child workflow (CHILD of build.sh)
...
#
# Usage:
#   ./build-refine.sh --pr <N> "the original task text"
#   ./build-refine.sh --pr <N> --task-file /tmp/task.md --verbose
#
# Flags:
...
if [[ -z "$PR_NUMBER" ]]; then
    ...
    echo "       (It is normally invoked by the parent: scripts/workflows/build.sh)" >&2
    exit 1
fi
...
WORKTREE_NAME="build-refine-${TIMESTAMP}"

LOG_DIR="${REPO_ROOT}/.claude/logs"
LOG_FILE="${LOG_DIR}/build-refine-${TIMESTAMP}.jsonl"
mkdir -p "$LOG_DIR"
...
echo
echo "================================================================"
echo "  BUILD-REFINE WORKFLOW COMPLETE"
echo "================================================================"
echo
