...
if [[ -z "$PR_NUMBER" ]]; then
    ...
    echo "       (It is normally invoked by the parent: scripts/workflows/revision-minor.sh)" >&2
    exit 1
fi
...
Dispatch the `code-reviewer` agent — **one agent, and that is the whole review**. This is the minor tier: its scope is a scoped correction to known lines, where the dominant risk is a change that is simply WRONG (an inverted condition, an off-by-one, a case the fix misses), not a design that will not scale. Correctness is the lens that catches that class; the structural, standards and holistic lenses that `revision-refine` runs are sized for multi-file architectural work and would spend most of this run's budget confirming there is nothing to say.

**If the review keeps finding structural or standards problems, that is a ROUTING signal, not a reason to add agents here.** It means the task was mis-sized for the minor tier and belongs on `revision.sh`. Say so plainly in your summary.
...
echo
echo "================================================================"
echo "  REVISION-MINOR-REFINE COMPLETE"
echo "================================================================"
echo
