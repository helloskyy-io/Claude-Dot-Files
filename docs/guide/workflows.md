...
**Wiring live is not the same as input flowing.** As of 2026-08-13 exactly one of 77 candidate rows names a `component`, and its directory already exists, so the next `plan-project` run scaffolds nothing and reports an empty working set. That is the design working: the step becomes productive as filers name components on rows they file, and the pre-existing rows with a blank cell wait for the operator.
