...
| `research.sh` | **parent** | `research-draft` → `research-refine` → `review-pr`. **Altitude is DERIVED from the pool path** — `docs/standards/architecture/research/` is project-level, `docs/development/<component>/research/` is component-level. One workflow, two altitudes |
| `plan.sh` | **parent** | `plan-draft` → `plan-refine` → `plan-sprint` → CI gate → `review-pr` → bounded loop-back. Takes **ONE component directory** and drives it from plan to sized sprint entry. This is what you dispatch at a component |
...
**Four CLI rules that apply to every V2 entrypoint above.**

- **`--run-id` names the run, and `--writer` says this invocation is only PART of one.** Every entrypoint takes both. Omit them and nothing changes for you: the run is named for you and the name is printed on stderr as `journal: run id <id> (minted here — pass --run-id <id> …)`. **Pass that name back with `--run-id` to retry into the SAME journal bag** rather than filing a second one — the bag is adopted, its existing record is kept, and a redaction or gap the first attempt wrote is not overwritten. **Add `--writer <name>` when you are dispatching a child that belongs to a run somebody else started**: it joins that run's bag through its own subfolder, so a parent and its three children file one bag with one subfolder each rather than four bags for one piece of work. `--writer` without `--run-id` is refused, because a child that guesses whether it is its own run is the failure both flags exist to prevent. The name may contain `A-Z a-z 0-9 . _ -` and nothing else.
...
- **A task source is EXCLUSIVE — a description, `--task-file` or `--phase`, never two.** `build.sh` refuses two outright (`at most one task source`), and the refusal is correct: two artifacts each claiming to define the work is how they drift. **So put decisions in the ARTIFACT, not the invocation.** A ruling written into the phase doc survives a changed flag, a re-dispatch and a different operator; the same ruling living only in a `--task-file` evaporates the moment the invocation changes. *(Measured 2026-08-29: a PM had written its architecture rulings into the phase doc rather than only the brief, so hitting this refusal cost nothing. That was luck, and it is cheaper as a habit.)*
- **`--pr <n>` and `--phase <doc>` together still update that PR.** Naming a PR sets the DESTINATION; naming a phase doc sets the TASK SHAPE, and the destination wins the choice of workflow — the run is told it is working on PR `<n>`, never to open a new one. What the combination gives up is the plan-driven stage prompts; the plan doc's contents still arrive as the task. *(Before 2026-08-20 the major tier resolved this the other way and instructed the run to `gh pr create` on a branch that already had an open PR.)*
- **`build.sh --pr <n>` and `build_minor.sh --pr <n>` need no task restated.** A correction pass is a correction of work already described on that PR, and the child is already required to read the thread — so re-supplying the original `--phase` or description only creates a second copy of the runway that can disagree with the one being read. Supply a description, `--task-file` or `--phase` **only** when the new pass has a genuinely different brief; **any of the three then takes precedence over the PR runway**, and the **refine** child still reads the thread regardless — `gh pr view <n> --json body,commits,comments` is mandatory on every refine pass, at both tiers. (The *draft* child is only told to read it when `--pr` is the sole task source, which is why the runway is the default rather than an override.) A dispatch with neither a task source nor a `--pr` is still refused.
...
research(feature)   →  research-draft  →  research-refine  →  review-pr   →  HiL
plan                →  plan-draft  →  plan-refine  →  plan-sprint  →  review-pr
```
...
**`plan-refine` is the CORRECTOR, and naming it that is what fixed the family.** All three families are now `draft → refine → judge`: research is `research-draft → research-refine`, build is `build-draft → build-refine`, plan is `plan-draft → plan-refine → plan-sprint`. The middle child reads the artifact COLD — it did not write it — and it **fixes what is determined and reports what is a judgement**. `review-pr` is the only thing in the fleet called a reviewer, and it is the only one that writes nothing.

> **This child was called `plan-verify` until 2026-08-29, and the name was load-bearing in the wrong direction.** Its own code argued from it — *"a reviewer that may edit a phase doc is a reviewer that can quietly rewrite the plan it was sent to judge"* — which is sound reasoning from a false premise: it is not the reviewer, `review-pr` is. The fleet had two things called *verify* with opposite write authority, and the plan family ended up with two judges and no corrector. `research-verify` survived the same name only because someone later overrode it in writing.

**The loop-back re-enters `plan-refine`, never `plan-draft`, and this is the property that stops a correction cycle re-designing.** `plan`'s loop used to re-dispatch its author before every correction pass. An author invoked with authoring authority authors: MDC PR #173 was sent to fill a five-phase gap in a seven-phase roadmap and returned ten phase docs, three in no roadmap at all — one approved phase split into three across two loop-backs, each split generating the findings that justified the next. Five passes, $97, never converged, and the run carried `correction_pass=True` throughout. **`MAX_LOOPS` caps the spend; only the loop's shape caps the scope.** Held by `test_no_loop_back_re_enters_the_AUTHOR.py` for all three families.

**Two judges now see a planning PR and they are not redundant.** `plan-refine` reads the ARTIFACT cold — a roadmap and its phase docs, whether the boundaries are right and what each costs. `review-pr --type planning` judges the DIFF against the planning criteria and returns the verdict `plan-project` routes on. A plan can be a clean diff and a bad decomposition.

**The hour estimates live in `<component>/roadmap.md` and nowhere else, and that is a decision with two reasons.** The roadmap is the only place every phase is visible together, which is what a total is taken over — and **a phase gated on something outside the component has a roadmap entry and NO phase doc**, so sizing that lived in phase docs could not size a gated phase at all. One figure with one home also cannot drift from its copy. The enforcement is the write grant rather than a second scanner: `roadmap.md` is the only file in the component `plan-refine` can reach.
...
- **`build*`** — fix existing code. **Both are parents**; the difference is the model the middle child runs on, and that is a REASONING question, not a budget one.
  - **`build-minor.sh` is the SMALLER tier, not the weaker one** (ruled 2026-08-18). Same model as `build.sh`; the saving is scope — **100 turns against 250, and one review lens against five.** It is the right choice when the work is narrow and its shape is already known: a scoped change to one file, a fix a reviewer has already specified, a correction whose runway is written. **Choose it because the work is SMALL — not because it is easy, and not because it is cheaper.** A small task can still need judgement, and that is no longer a reason to avoid this tier.
...
feature-manager: (parent of a parent, most likely cron based on expired research)
   ...
   |  ├─research-refine: (targeting feature)
   |  ├─review pr
   |  └─(usually HiL at this point, repeat till happy)
   |
