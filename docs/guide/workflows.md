...
| `plan_project.sh` | **parent** | `triage-candidates` → **`plan-candidates`** (an ACTIVITY — plain code in the parent, no dispatch) → `research-write` → `research-verify` → `plan-feature`, the last three per NEW component → `plan-sprint` → `review-pr`. Was `plan-master` |
...
**The chain below is what runs today.** Every step in it is built and wired; the only bracketed name is `plan-verify`, which does not exist.

```
research(project)  →  HiL
plan-project       →  triage-candidates  →  plan-candidates  →  research-write  →  research-verify  →  plan-feature  →  plan-sprint  →  review-pr
...
**`plan-verify` is the missing half, and the gap is deliberate rather than an oversight.** The other two families each split their producer from a fresh-context judge — research is `research-write` → `research-verify`, build is `build-draft` → `build-refine` — on a stated argument: *the run that wrote an artifact defends it*. `plan-feature` is the WRITE half of that split for planning; `plan-verify` is the read half and is not built. **Until it is, `review-pr --type planning` judges the PR** and no automated pass sizes the phases — which is why `plan-feature` reports *NOT SIZED* explicitly rather than leaving the absence to be read as an oversight.
