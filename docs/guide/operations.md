...
Single pass, no review agents, for changes where a review cycle costs more than it finds. Stage 1 stops and escalates to `revision.sh` if the task turns out bigger than it looked. Bulk mechanical renames belong here.
```bash
