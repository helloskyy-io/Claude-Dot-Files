...
- **CI on this repo.** No `.github/workflows/`. PRs merge without `shellcheck`, `bash -n`, JSON/YAML lint, hook-pattern regression tests, markdown link-check. **Approach when triggered:** `.github/workflows/ci.yml` with the above checks. **Trigger:** after testing infrastructure exists — **FIRED 2026-08-06**, the three-tier suite now exists. The pytest-tier half of this is tracked at issue #30; the shellcheck / lint / hook-regression half is still unowned.
