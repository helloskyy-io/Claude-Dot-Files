...
- [x] Each check has a test proven able to fail. **17 mutations run across the two suites; 17 caught, 0 survived**, each by the test that names the property. The list is in the PR body. Two beyond the modes above are worth naming here because they guard *this phase's obligation to Phase 4* rather than the predicate: widening the `parent_route` payload, and `append_convergence` reusing the `parent_route` type string — both go red
