...
- [x] **Run the grep; do not re-read.** Every claim this phase makes about a string being *absent*, or a fact being *stated once*, is checked by a command whose output is recorded — not by re-reading the file, which is the plausibility check that passed three times while the contradiction was live. Three of them, and **each is scoped `README.md docs/` rather than `docs/`** — the earlier `docs/`-only scope is what let `README.md:11` state the three-surface model, and the repo's front door, through the whole correction:
  ...
  - `grep -rn 'three surfaces\|Three surfaces\|four surfaces\|Four surfaces\|of the four surfaces\|other three surfaces' --include='*.md' --include='*.txt' README.md docs/` — **the surface count itself.** Every hit must state five, or quote a smaller number as superseded. The two sub-counts are the ones to read rather than pattern-match: *three GitHub surfaces* is still correct and must not be swept to five, and `/standup`'s *three* writes is a numerator that did not move when the denominator did
- [x] **`config/` is deliberately outside all three scopes, and that is a boundary rather than an oversight.** Widening to `config/` goes red on `config/commands/standup.md:3` and `:174`, which contradict that file's own Stage 2 body. That contradiction is real, is live on `main`, and is escalated — this phase documents it (§2.3) and does not edit prompts. The scope widens to `config/` in the same change that reconciles those lines.
