...
**Finding 1 — `/standup` is not read-only, and has not been since `1e7d6ce` / `88c4e81`. It is a writer on THREE of the five surfaces.** `gh issue edit <tracker> --body-file` (`config/commands/standup.md:83`, `:107`), `gh issue close <N> --comment <evidence>` (`:66`, `:69`), and **deleting rotated rows from `direction.md` plus correcting stale ones** (`:67`, `:105`). **Consequence:** a consumer list omitting a writer is worse than no list — Phase 4 verifies fleet-wide against that list. The write set is now stated **once**, at [`memory-model.md` §2.3](../../guide/memory-model.md); every other site points at it and none restates it. **When Finding 2's count moved to five, the denominator moved and the numerator did not** — checked before propagating, because *three of four* and *three of five* are the same claim about `/standup` and a different claim about the fleet. `/standup` opens the source candidate's Note in `candidates.md` to establish that a `direction.md` row is safe to delete (`:67`, `:105`) and writes nothing back to it, so the fifth surface is a read.
...
- [`candidates.md`](../../standards/architecture/research/candidates.md) — same six lines, same result: *"nobody deletes a row"*, a fixed-column table both `plan-sprint` and the research tree parse, *"both carry reasoning in the Note"*, an orthogonal `status:` bit the file devotes a section to keeping separate from `decision`, `C-NNN` ids *"never reused, and never renumbered"*, and a § *Why it exists* that is literally a context-death incident. Documented as §2.5.
