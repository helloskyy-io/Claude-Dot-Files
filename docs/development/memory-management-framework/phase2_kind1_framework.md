...
- [x] **Run the grep; do not re-read.** Every claim this phase makes about a string being *absent*, or a fact being *stated once*, is checked by a command whose output is recorded — not by re-reading the file, which is the plausibility check that passed three times while the contradiction was live. The two that matter here, both repo-wide rather than aimed at a file already known to be wrong:
  - `grep -rn -i 'read-only' --include='*.md' --include='*.txt' docs/ | grep -i standup` — every surviving hit must be one that quotes the stale claim **as** stale, or a dated research paper governed by its own revalidation clause
  - `grep -rn 'two places\|TWO places' --include='*.md' docs/` — same rule; no hit may assert the undercount in its own voice
