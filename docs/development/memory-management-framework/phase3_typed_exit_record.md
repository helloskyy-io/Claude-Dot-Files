...
- [ ] If Phase 1 E2 found a partial record can exist, add a **consumer-side completeness check the parent enforces** — a terminal marker the parser requires. Atomic write is an additional producer-side measure, not the requirement, because the producer is precisely the party whose reliability this component is designed not to assume
