...
| **Research the payload contract** | `code_routed_control_flow.md` §2.4.1 (Tekton 4 KB, Airflow "small amounts of data", and Argo's 1 MB etcd resource ceiling — whose documented remedies are SQL offload or workflow-of-workflows restructuring, *not* the separate `@filename` mechanism that Argo applies to individual container arguments over 128 KB), P4, and §2.4.2 (TEP-0074 withdrew a rich typed handoff object for coupling and conceptual opacity) | **Answered, and more sharply than the sprint item states it.** The references-not-payloads lesson is verified first-party. No new topic. |
