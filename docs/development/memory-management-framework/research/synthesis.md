...
>
> **This synthesis went through the same gate twice, and the second pass found the first one's work here incomplete.**
>
> **Pass 1**, alongside the papers, found five defects and fixed four cleanly: **one span presented as a quotation that was not character-exact** (the A2A phrase in §11); **one table row recording a disagreement that does not exist** (§11 — a first look was credited with a claim it never made); **one conflation of two distinct Argo mechanisms** (§11, traced into `topics.md` as well); and **one wrong pool count** ("0 of 0", now enumerated). The fifth — **a false section locator in Escalations** — was reported closed but was not: see pass 2. One further span was hardened rather than corrected: §7's SWE-Bench+ figure was a bolded near-verbatim paraphrase sitting next to the word "reports" — not a quotation, so not a §3 breach, but close enough to be lifted as one downstream, and it is now the character-exact sentence. Every replacement was re-verified against its source: the unabridged arXiv Atom API for the two abstracts, and a direct read for each local document.
>
> **Pass 2** was a separate fresh-context review of the Escalations span alone, and it found three defects there — one of them a **repair that had not repaired**. (a) The locator was still wrong: the "strongest available reading of element 3" quotation was welded onto a `§6.6` locator when that sentence lives in the cited paper's front matter at line 103, and the span had converted §6.6's **co-equal** (a)/(b) re-cuts into a ranking §6.6 does not make. (b) **A confidence inflation on the escalation's load-bearing inferential step** — it asserted this pool closes the upstream N6 gap directly, where `non_model_observables.md` P2 closes it *from outside* the agent corpus and marks that framing **derived**. (c) **An escalation title unqualified against its own qualified body**, which traced to a second dependent at §5's opening sentence (§4: a corrected fact traces to ALL its dependents). All three are fixed and each repaired span was re-verified at its source line before being reported closed. Verdict on this document: **PASS**.
>
...
- **`non_model_observables.md` closes upstream negative finding N6 from outside the agent corpus (*derived*), and closing it weakens the last defensible reading of problem-statement element 3.** `code_routed_control_flow.md` §6.6 already ruled element 3 "ordinary as stated" and offered two narrower re-cuts — **(a)** code-routing across whole agent-workflow runs that are separate processes with disjoint contexts, and **(b)** "routing on values the model did not author" — but it ranks neither: "Both are narrower than the current wording and both are actually uncovered by the literature." The ranking sits elsewhere in that paper. Its front-matter summary, bullet **(b) Literal "no model in the loop."** (line 93), applies the ranking not to re-cut (b) as such but to the **N6-rescoped** formulation stated in the sentence before it — "no located agent-framework doc presents a non-model value *carrying the outcome of real work* as its canonical branching example" — of which it says, at line 103: "That narrower gap is real and is the strongest available reading of element 3." **That N6-rescoped gap is what this pool's evidence bears on — but it is closed from OUTSIDE, and the closure is the source paper's own framing, which that paper marks *derived*.** `non_model_observables.md` P2 puts it as "The upstream N6 gap is a documentation gap in the AGENT corpus specifically, and this paper closes it from outside", flagged *(definitive on the artifact; the "closes N6" framing is **derived**.)*. The instances this cycle finds — that pattern documented as the **canonical branching example** in Argo, GitHub Actions, Airflow, Tekton and Kubernetes — are general-purpose orchestrators and CI/CD, not agent frameworks: no agent-framework doc showing the pattern was located by either paper, so the narrow agent-framework absence survives and what falls is (b)'s novelty *in the field*. N6's gap was in the agent corpus's documentation, not in the field's practice, and the upstream paper said so; this pool supplies the instances. **What it bears on:** `docs/standards/architecture/problem-statement.md` element 3, and any differentiator resting on it. **What I think it means:** re-cut (b) should be retired as a novelty claim, leaving re-cut (a) — cross-process, cross-run, disjoint-context, resumed-from-persisted-state — as the only reading still uncovered by the located literature. **This is not mine to file.** The product pool is read-only to this run; the operator disposes.
