...
| D11 | **Milestone 1 stays THIN despite the product reframe** | Strangler fig: seed VM up via bootstrap → ArgoCD+Temporal+Postgres+UI synced → ONE dispatch workflow → ONE edge worker on ONE dev VM → one PR produced end-to-end. Django dormant. Bash scripts stay live in parallel (reversible). Everything after = adding LEGO to a running platform. **AMENDED 2026-08-20: the mTLS clause is SUPERSEDED by D13.** The starter is internal-IP and key-based and is destroyed rather than hardened, so hardening it in milestone 1 buys nothing that survives the pivot. |
...
| D13 | **The starter control plane is TEMPORARY, and it is destroyed rather than migrated** (operator, 2026-08-19) | One VM, one k3s node, internal IP, key-based access, no frills, running **Temporal + ArgoCD + Django** — the same composition as MDC's `k3s-0`. It is a legitimate resting state, not a placeholder. Its secrets are placed before secrets management exists and are therefore compromised by construction, so at the pivot it is **destroyed, not migrated**. MDC's rollout standard states the same rule: *"Bootstrap-era secrets die with `k3s-0` — never migrated."* **Supersedes D11's mTLS clause**: hardening a node that gets destroyed buys nothing. |
| D14 | **The permanent clustered control plane is a FUTURE SPRINT, and it is INHERITED from MDC** (operator, 2026-08-19) | The 3-node HA cluster, the wireguard mesh between its nodes, moving the control plane onto it, and decommissioning the starter are all later work. Two reasons it is not planned now: MDC's own development has only just reached the permanent cluster, and **the migration and decommission procedures do not exist upstream yet** — both guide documents are stubs saying *"This procedure does not exist yet."* MDC also has an unresolved contradiction there (its Architecture Standard still calls `k3s-0` the permanent seed), being resolved on that side. Wireguard is the only piece with no upstream to copy. |
| D15 | **Skyy-Net expects NO workload tier** (operator, 2026-08-19) | MDC runs three tiers — control plane, platform services, and a separate workload cluster (`k3s-w1`, plus a conditional `k3s-w2` for untrusted work). Everything Skyy-Net runs is its own services, so they belong on the platform cluster. If workloads are ever run they go **on an MDC, not in Skyy-Net**. Consequence when reading MDC's documents: the workload-cluster steps are **not ours to copy** — the one place where borrowing wholesale builds something we do not need. |
