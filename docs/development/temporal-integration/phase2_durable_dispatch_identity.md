...
- [ ] **Add the request fingerprint** — a hash over the task inputs, stored in the record, erroring when the same key arrives with a different payload. **Temporal does not supply this and never will**, so it is built once and carried across the port unchanged.
...
- [ ] **Demonstrate requirement 4.** Re-dispatch the same logical id and show one record and one bag; launch a duplicate while one is live and show it failing loudly. Both go in the checklist as demonstrations, not as assertions in a summary.
