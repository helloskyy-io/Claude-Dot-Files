...
**Outside this component, and NOT satisfied:** [PMP Phase 9](../persistent-memory-protocol/phase9_one_run_one_identity.md) **r7** — atomic create-if-not-exists on the bag, **scoped to requirement 4 only**. The chain is three hops and each one is real: r4's loud failure needs mutual exclusion between two simultaneous openers; r7 is that mechanism and *"cannot be marked complete while its carrier does not exist"*; its carrier is the identity section the Temporal addendum does not yet have, queued as candidate **C-zhdm5gh1** and awaiting a human ruling. **Requirements 1, 2, 3 and 5 are unaffected and proceed** — this gates the *demonstration* half of requirement 4 and nothing else. **The ruling itself is this component's** ([`temporal-integration.md`](temporal-integration.md) § *The dispatch record already exists*: *"what this component still owes r7 is the ruling, not the mechanism"*), so a run reaching requirement 4 with C-zhdm5gh1 untriaged should get the ruling rather than pick one.
...
- [ ] **Rule how a bag create tells a RETRY from a DUPLICATE LAUNCH, and write down which component decides.** **TWO things are absent, not one — this bullet asserted the first was present and that was false.** `open_bag` creates by winning or losing an `os.mkdir`, and **the `mkdir` is atomic while bag CREATION is not**: its own docstring records that creating a bag is *"three syscalls, not one"*, so a race loser can adopt a bag whose `bag-info.txt` does not exist yet (`scripts/workflows/temporal/modules/journal/bag.py`, `open_bag`). **So the atomicity the superseded `refs/dispatch/*` store would have supplied is NOT present** — it is [PMP Phase 9](../persistent-memory-protocol/phase9_one_run_one_identity.md) r7's to supply, and it is not built here. **And the discriminating signal is absent too**, because `open_bag` adopts an open bag either way and refuses a sealed one. **The signal is unimplementable before the mechanism exists**: a read-then-decide check against an adopted bag *is* the race it is meant to detect. Requirement 4 needs one caller to adopt and the other to fail loudly, so it turns on #2 and #4 **and on r7**. **This is what [`roadmap.md`](roadmap.md) § *What is deliberately not built* records as still owed after the tier-1 store was superseded.** Do not amend PMP's shipped `open_bag` from this phase — if the ruling needs a change there, surface it.
