...
**Status: ⬜ NOT STARTED.** Listed after [Phase 9](phase9_review_runs_in_the_python_tree.md) in [rollout order](roadmap.md) — **fifth in that order since 2026-08-20**, when Phase 9 was inserted between Phases 3 and 4.
...
2. **The transcript is a file and the result carries a reference.** Event history carries references, never payloads. This is simultaneously a correctness constraint and a security control: event history persists in the central Postgres, and repo content does not belong there. **The rule binds three payloads, not one — the result, the activity's INPUTS, and the FAILURE detail.** Temporal records all three. The failure path is the one least likely to be tested and the most likely to carry a transcript tail or the CLI's stderr into the database; the input path is what makes [§A4](../../standards/temporal/claude-dot-files-addendum.md)'s *prompt-as-input* option a question for this requirement rather than only for the blob limit.
...
4. **[§A1](../../standards/temporal/claude-dot-files-addendum.md) is closed in the addendum** — all three of its open bullets, heartbeating, payload limits and retry semantics. **Closing it does NOT settle input constraints on `claude_cli`, and the difference matters.** The [seed handoff](../skyy-net-seed-handoff.md) § *Security surface* names queue injection as the real hub risk — a compromised server enqueues work that spokes voluntarily execute, and with this activity registered that approximates code execution on a machine holding a real repo and a real credential — and it names *"input constraints on `claude_cli` defined in its standard"* as one of two mitigations. **That is not one of §A1's three bullets.** A closed §A1 reads settled, so state in the closing edit that the input-constraint question is still open and unhomed rather than letting it be retired by adjacency.
...
8. **The worker's activity slot count is set from the host's real concurrency budget by a STATED METHOD, not left at the SDK default of 100** — and the method is recorded, not only the number, because [Phase 6](phase6_the_rest_of_the_fleet.md) re-derives it per host and cannot re-apply a judgement nobody wrote down. Measure one `claude` run's resident cost on the host (memory, threads, child processes), divide the host's committed capacity by it, and record the per-run cost and the divisor as well as the answer. **A number with no method behind it satisfies this requirement's letter and none of its purpose** — and `ThreadPoolExecutor(max_workers=...)` is set to at least that number. `python_sdk_long_activities.md:168` is the derivation: a slot is a counter, but what a slot *holds* is one `claude` CLI process plus, in the sync-threaded shape, one OS thread pinned for the full hour. **The default admits 100 simultaneous `claude` runs on the operator's own workstation.** This binds **every worker that registers this activity**, not only the one requirement 5 demonstrates against — see [Phase 5](phase5_the_first_dispatch.md)'s systemd worker and [Phase 6](phase6_the_rest_of_the_fleet.md)'s worker set.

---

## Dependencies

**Requirement 7 is exempt from the gate below and should not wait for it.** Measuring the CLI's longest natural silence needs the installed CLI and nothing else — no server, no worker. It is also the longest-lead item here, because twenty real runs is elapsed wall-clock rather than work, and requirement 1 cannot be settled until it exists. **Collect it alongside [Phases 2](phase2_durable_dispatch_identity.md), [3](phase3_the_retry_boundary.md) and [9](phase9_review_runs_in_the_python_tree.md); only the cadence ruling waits for the rest of this phase.**
...
- [ ] **Measure one run's resident cost on the host, then set the activity slot count and executor width from it** — requirement 8. Record the per-run cost, the host's committed capacity, and the resulting number, so [Phase 5](phase5_the_first_dispatch.md) and [Phase 6](phase6_the_rest_of_the_fleet.md) re-apply a method rather than copy a constant. `max_concurrent_activities` left unset means 100.
...
- [ ] **Assert the same of the FAILURE payload.** The raised exception carries the transcript's *reference*, never its content — and never the CLI's raw stderr. A failing run is exactly when the tempting thing to attach is the output that explains it.
