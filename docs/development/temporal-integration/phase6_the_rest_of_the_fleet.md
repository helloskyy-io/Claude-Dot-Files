...
**Status: ⬜ NOT STARTED.** Last of the phases with a doc in [rollout order](roadmap.md) — seventh since [Phase 9](phase9_review_runs_in_the_python_tree.md) was inserted — and the last phase this component plans in detail.
...
- **READ THIS BEFORE § *Runtime Verification* ABOVE — one sentence in it is now false and is left standing deliberately.** That block is dated 2026-08-19 and reads *"`review-runs` has no module in that tree, which is why requirement 2 says written rather than moved."* The observation is still true; **the consequence is not.** `review-runs` left this phase on 2026-08-20 and became [Phase 9](phase9_review_runs_in_the_python_tree.md), so *written rather than moved* is now **Phase 9's requirement 1**, and requirement 2 here is orchestration only. The block is a historical record and is not edited; this bullet is placed first so the correction is read before the thing it corrects.
- **`review-runs` is different and the difference is the point.** It has a live role and a run history, and a downstream phase in another component is waiting on it. It is the one of the four whose port is justified before anyone argues about it — which is why it earned a phase of its own rather than a requirement inside this one.
