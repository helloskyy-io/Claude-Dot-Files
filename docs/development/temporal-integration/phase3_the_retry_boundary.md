...
- [ ] **Audit every re-wrapping site across the three files that call `gh` or `gh_json`.** The rule is first-party documented: the SDK checks the **outermost** error type, so one `except … raise RuntimeError(...)` anywhere between the raise and the activity boundary reverts everything to one opaque string.
