...
| [Phase 2](phase2_durable_dispatch_identity.md) requirement 4 **only** | [PMP Phase 9](../persistent-memory-protocol/phase9_one_run_one_identity.md) **r7** — atomic create-if-not-exists on the bag | **gates the demonstration half of requirement 4**, and nothing else in the phase. r7 waits on its carrier, the addendum identity section queued as **C-zhdm5gh1**; the ruling is this component's to make. PMP Phase 9 is itself ungated and can be scheduled ahead |
...
- **§A4 of the addendum — whether a prompt is an INPUT or a RESOURCE — is NOT ruled by this plan, and no phase here closes it.** [Phase 4](phase4_the_claude_cli_activity.md) closes [§A1](../../standards/temporal/claude-dot-files-addendum.md) and [Phase 1](phase1_the_starter_control_plane.md) closes §A3's **queue-naming half** (see the bullet below); §A4 stays open. **It becomes concrete at [Phase 5](phase5_the_first_dispatch.md)**, where a parent first becomes a workflow and a replay first has to load a prompt from somewhere. The ruling is tracked as candidate **C-obarkm4s** in [`candidates.md`](../../../tracked/candidates/), `component: temporal-integration`, untriaged — **naming it here is the placement, not the decision.** **One criterion belongs on that ruling and is easy to miss:** [Phase 4](phase4_the_claude_cli_activity.md) requirement 2 makes keeping payloads out of event history a *security* control, not only a size one — and workflow and activity **inputs** are recorded in event history exactly as results are. So *"prompt as input"* puts prompt text into the central Postgres for the ≥ 30-day retention [Phase 1](phase1_the_starter_control_plane.md) requirement 4 mandates. **It has to clear that rule or be ruled out on it**, and replay-correctness alone is not the whole question.
- **§A3 cannot as written hold the RUN-IDENTITY ruling, and no phase here widens it.** The bullet above records that [Phase 1](phase1_the_starter_control_plane.md) closes §A3 — it closes §A3's **queue-naming half**. The section is titled and scoped to machine-axis task-queue naming, while two planning documents nominate it as where *what the run id IS* gets ruled: [`temporal-integration.md`](temporal-integration.md)'s migration path at the dispatch-identity step, and [PMP Phase 9](../persistent-memory-protocol/phase9_one_run_one_identity.md), whose r2 and r7 both refer their readers there from the other side. **Giving it a carrier is a human-in-the-loop edit to a standards file under [`standards-governance.md`](../../../config/rules/standards-governance.md), and no phase in this plan writes it** — and the SHAPE is open, not decided: `C-zhdm5gh1` leaves widening §A3 and splitting identity into its own §A-section both live, so no planning document should assert one of them — the plan records the debt rather than paying it. It is tracked as candidate **C-zhdm5gh1** in [`candidates.md`](../../../tracked/candidates/), `component: temporal-integration`, untriaged — **naming it here is the placement, not the decision.** **The deadline is Stage B**, because after it a change to where a name is minted is a migration of wrapped activities rather than a parameter on plain functions.
...
- **A second, git-native tier-1 dispatch record on `refs/dispatch/*`.** This component's own pool proposes one — [`raw/durable_dispatch_identity.md`](research/raw/durable_dispatch_identity.md) §3.6 splits dispatch state into two tiers, §5.3's migration table carries the tier-1 row at a *zero* **port-time rewrite** cost (which is not a build cost), and §6 item 6 prices the build at *1–2 days*: *"Tier 1 store: `refs/dispatch/*` with CAS creation"*. **It is superseded by the PMP run bag, which already IS the store such a record lives in and is deliberately ONE tier** ([PMP Phase 1](../persistent-memory-protocol/phase1_the_run_bag.md) § *One location, a folder per run, many formats*), so retention, transfer and replay operate on a single unit rather than on two that can disagree about what a run was. The plan already builds on the bag rather than beside it: [Phase 2](phase2_durable_dispatch_identity.md) § *Dependencies* records that *"The run bag is already keyed by `run_id`, and the identity record extends a store that exists rather than creating one"*, and [Phase 4](phase4_the_claude_cli_activity.md) says the same for the transcript — *"Reuse the run bag rather than inventing a second store …"*. **The paper claims two advantages for its tier 1 over a store like the bag, and saying which one survives is what makes this a supersession rather than a dismissal.** **(1) `git update-ref` gives compare-and-swap on ref creation — and this one DOES survive.** **This bullet previously said it did not, *because the bag's create is already atomic*, and that premise is false.** `open_bag`'s docstring in `scripts/workflows/temporal/modules/journal/bag.py` is the evidence and states it in terms: *"creating a bag is three syscalls, not one, so a second caller that loses the `mkdir` race can observe the bag directory between its creation and its tag files being written, and will adopt a bag whose `bag-info.txt` does not exist yet"* — the `mkdir` is atomic, bag **creation** is not. Sequential retry, which is the case Temporal actually produces, is fully safe; *"a true simultaneous race is not, and closing it needs a lock or a create-then-rename."* Cited here so the next reader does not re-derive it from the code. **So the residual is real, and it has a MECHANISM and a CONSUMER rather than two owners.** [PMP Phase 9](../persistent-memory-protocol/phase9_one_run_one_identity.md) **r7 is the mechanism** — atomic create-if-not-exists on the bag, deliberately left unchecked there until its ruling has a carrier. **[Phase 2](phase2_durable_dispatch_identity.md) requirement 4 is the consuming requirement that depends on it**: the signal that tells a *retry*, which must adopt the open bag, from a *duplicate launch*, which must fail loudly — and **a duplicate launch cannot fail loudly unless something first makes two concurrent opens mutually exclusive**, so r7 is its prerequisite rather than a competing claim on the same property. **The relationship, in the words this component already uses for it: *"PMP owns the record; this component owns what it needs FROM the record."*** Phase 2 carries a checklist item for its half; this file records the seam and does not keep a second copy of either side's reasoning. **(2) The paper's tier 1 is *"the only fleet state that crosses a machine boundary"* — and this one does NOT survive, because the bag is cross-machine state too**: the bag is one root per edge that ships to object storage local-first and asynchronously ([PMP roadmap](../persistent-memory-protocol/roadmap.md) commitment 8) — it transfers across machines, so the paper's claim to be the *only* such state does not hold against it. **What the bag does not do is ARBITRATE between machines — and that is advantage (1) again, not a second residual.** **Nothing hand-rolls a cross-machine lock in the gap**, which [Phase 2](phase2_durable_dispatch_identity.md) requirement 5 forbids anyway: the uniqueness scope is `(machine-id, repo)` today, there is one edge, and it becomes `(Namespace, Task Queue)` at the port. **A reader arriving from the paper should treat its tier-1 recommendation as ruled on here** — the paper stays as written, because a pool is evidence and this file is where the plan disagrees with it.
