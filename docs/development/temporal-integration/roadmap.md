...
- **A second, git-native tier-1 dispatch record on `refs/dispatch/*`.** This component's own pool proposes one: [`raw/durable_dispatch_identity.md`](research/raw/durable_dispatch_identity.md) §3.6 splits dispatch state into two tiers, and its §6 recommendation table costs the tier-1 half at *1–2 days* — *"Tier 1 store: `refs/dispatch/*` with CAS creation"*, including the refspec question. **It is superseded by the PMP run bag, which already IS that record and is deliberately ONE tier**, so retention, transfer and replay operate on a single unit rather than on two that can disagree about what a run was. This plan already builds on the bag rather than beside it: [Phase 2](phase2_durable_dispatch_identity.md) § *Dependencies* records that *"the run bag is already keyed by `run_id`, and the identity record extends a store that exists rather than creating one"*, and [Phase 4](phase4_the_claude_cli_activity.md) says the same for the transcript — *"reuse the run bag rather than inventing a second store"*. **One property the proposal has that the bag does not, and saying so is what makes this a supersession rather than a dismissal: `git update-ref` gives compare-and-swap on ref creation, so create-if-absent is atomic and the conflict policy is enforced by git itself.** [Phase 2](phase2_durable_dispatch_identity.md) answers the same need as a **ruling plus a store check** rather than a primitive — the request fingerprint (*same key, different payload ⇒ error*, § *The six components* #4), the two conflict rulings (#6), and requirement 4's demonstration that a duplicate launch while one is live fails loudly. **What it does not yet name is what makes that check atomic against the bag — #4 and #6 both read *absent* in that table today, so Phase 2 owes the mechanism and not only the policy.** **A reader arriving from the paper should treat its tier-1 recommendation as ruled on here**; the paper stays as written, because a pool is evidence and this file is where the plan disagrees with it.
