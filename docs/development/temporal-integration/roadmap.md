...
- [ ] **§A3 is closed for QUEUE NAMING** — machine-axis task-queue naming decided and written into the addendum, including the no-fallback-queue rule. **It does not discharge the run-identity entry** two planning documents also nominate §A3 to carry; that widening is human-in-the-loop and is tracked as C-120 (see § *What is deliberately not built*)
