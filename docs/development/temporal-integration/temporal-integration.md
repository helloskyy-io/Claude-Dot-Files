...
**The one property candidate 7 has that the bag does not is compare-and-swap on the small part.** That is a real property, and naming it is why this is a supersession rather than a dismissal — but it is **a mechanism the bag can acquire, not a reason to carry a second store.** **And it has a named owner rather than a vague future**: the residual need is not claim/lease/TTL — the pool's own candidate 8 rules that layer out, because Temporal replaces it — it is **atomic create-if-not-exists on the run record**, which is verbatim [PMP Phase 9](../persistent-memory-protocol/phase9_one_run_one_identity.md) r3. **Naming that matters because the two are not the same property:** idempotent-on-retry is what r3 says today, and mutual exclusion between two concurrent openers is what compare-and-swap gives — a supersession that does not say which one r3 must deliver retires a real property into nobody's backlog.
...
*(This is a planning ruling and it is recorded **in the planning narrative only, never in the pool** — the same ruling is stated from PMP's side at the link above, which is the deliberate both-sides pattern this doc family uses for a constraint two components share. The pool's candidate 7 row stands exactly as written: research is evidence, and a ruling about how we build does not belong inside it.)*
