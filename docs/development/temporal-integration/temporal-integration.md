...
**What the residual need IS: atomic create-if-not-exists on the run record — mutual exclusion between two concurrent openers.** [PMP Phase 9](../persistent-memory-protocol/phase9_one_run_one_identity.md) r3 is where it is carried, **and satisfying r3 as it stood was never enough**: r3 reads *"Opening a bag twice under one run id yields one bag, not two — that is the shape a RETRY takes"*, which is **idempotent-on-sequential-retry**, a strictly weaker property. `open_bag`'s own docstring says the same thing from the code side — sequential retry is fully safe, a true simultaneous race is not. **So closing this gap means EXTENDING r3, not merely satisfying it**, and that extension is now written: r3 names both properties, commits this phase to the sequential one, and refers the simultaneous one to the identity ruling in [the local Temporal addendum](../../standards/temporal/claude-dot-files-addendum.md) — where this component is the one that has to say whether two openers can be live at once. **A supersession that retires the only carrier of a property without handing it to a named one puts it in nobody's backlog**, which is the failure this section exists to prevent and the reason the seam is stated from both sides.
