...
2. **The server-side topology is written down — EVERYTHING running on the node, not only Temporal's services.** Which Temporal services run where (frontend, history, matching, worker-service), plus Postgres, the Web UI, and the namespace — **and ArgoCD and Django, which [D13](../skyy-net-seed-handoff.md) puts on this same node.** The vendored standard does not cover this and nothing else in the repo does either. **The completeness matters one phase-pair later:** [Phase 8](roadmap.md) requires *every* secret placed on the starter to be reissued rather than copied, and that requirement cannot be checked complete against a record that never named two of the node's service families.
...
5. **[§A3](../../standards/temporal/claude-dot-files-addendum.md) is closed for QUEUE NAMING** — machine-axis task-queue naming decided and written into the addendum, together with the no-fallback rule in requirement 7. **The RUN-IDENTITY entry that §A3 is separately nominated to carry is NOT discharged by this requirement** — widening §A3's scope is a human-in-the-loop standards edit tracked as candidate C-zhdm5gh1, and no phase of this plan writes it ([roadmap](roadmap.md#what-is-deliberately-not-built)).
6. **The node was stood up by a script that can stand up the next one.** Even a rough one. Twenty manual rebuilds teach twenty times and leave nothing behind; the automation is the artifact. **The script's security posture is re-decided when it is reused, never inherited.** It is authored under *do not harden this node*, and that instruction is scoped to a node scheduled for demolition — [Phases 7 and 8](roadmap.md) reuse the script on machines that are not.
...
| [`raw/python_sdk_long_activities.md`](../../standards/architecture/research/raw/python_sdk_long_activities.md) **§5** (`:170`) — product pool, `Last validated 2026-08-03`, Critic `PASS-WITH-FIXES` | **An input to the §A3 ruling, and the reason repo-locality is not a scheme to invent.** The first-party `worker_specific_task_queues` sample [S15] exists for exactly our case: one worker process runs two workers, one on a shared distribution queue and one on *"a uniquely generated Task Queue"*, for *"tasks where interaction with a filesystem is required"* — and in production those per-worker folders *"would typically be independent machines in a worker cluster"*. **Requirement 5's ruling should be made against that sample rather than from first principles** |
...
- [ ] **Identify the source images and charts from MDC and pin them.** Mirroring means the same versions, not the latest — see implementation task. **Check Temporal's published security advisories at pin time and floor the pin at the current patched line.** Mirroring is a floor, not a ceiling: these pins are what the standup script carries into [Phases 7 and 8](roadmap.md), where the permissive-authorizer acceptance below no longer applies, and a version chosen only for parity with MDC ages into the permanent plane.
...
- [ ] **Decide the machine-axis queue naming and write it into [§A3](../../standards/temporal/claude-dot-files-addendum.md).** Read [§2 of the vendored standard](../../standards/temporal/worker_deployment_standard.md) first: its convention is `<domain>-<env>`, ours needs a machine axis on top of it because Claude Code must run on the machine holding the repo. **Then read [`python_sdk_long_activities.md` §5](../../standards/architecture/research/raw/python_sdk_long_activities.md) and its `worker_specific_task_queues` sample [S15]** — repo-locality is a solved pattern with a first-party sample, and the ruling should say how ours relates to it rather than arriving at a scheme in isolation. The [seed handoff](../skyy-net-seed-handoff.md) proposes `dispatch-<machine>-<env>`; that is a proposal, not a ruling, and this step is the ruling. **Rule queue naming only** — the run-identity entry §A3 is also nominated to carry is candidate C-zhdm5gh1 and is not this step's to write.
- [ ] **Include the no-fallback rule in the same §A3 edit.** An unresolvable assignee parks with a typed event. **These two checkboxes together close §A3's queue-naming half and nothing beyond it.**
...
- **There is no divergence left to trip over, and this note records why the earlier one closed.** D11 of the [seed handoff](../skyy-net-seed-handoff.md) describes a milestone 1 that ships *"one PR produced end-to-end under mTLS"*, which read against this phase as a contradiction while the two sat in different documents. **The handoff has since been amended: D11's mTLS clause is marked SUPERSEDED BY D13**, and D13 is the operator's deployment brief recorded in the same table. **A build run reading the handoff in isolation now gets the right scope from the handoff itself** — which is the point of recording the brief there rather than only here.
