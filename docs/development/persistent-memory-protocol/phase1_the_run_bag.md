...
- **`--dry-run` is a deliberate exemption.** Seven of the 11 entrypoints return from their dry-run branch before reaching bag-open, so on that path the call is present and does not execute. *(The other four have no `--dry-run` at all. This said EIGHT until the figure was derived rather than restated — see [`test_journal_prose_figures_are_DERIVED.py`](../../../scripts/workflows/temporal/tests/unit/test_journal_prose_figures_are_DERIVED.py), which now fails when a count in this document contradicts the tree.)* That is intended — a dry run states *"nothing invoked, nothing posted"*, and creating a directory would falsify it and fill the journal with empty bags from previews. No run happens on that path, so no run lacks a bag.
- **The ordering check reaches 9 of the 11 entrypoints.** It can see a `worktree_add` call and a workflow handoff called by name; `run_research.py` and `run_review_pr.py` reach their workflow through an aliased module that also carries pure prompt-assembly functions legitimately running before the bag, so treating any call on it as a side effect would fail those two falsely. *(`run_research_minor.py` was named here as a third until 2026-08-28, when the research tiers merged and it was deleted. The coverage figure is derived from the test's own uncovered set and so followed the deletion by itself; the hand-written name did not, which is the argument for deriving a name the way this package already derives a count.)* The set it does not cover is **pinned in the test**, so an entrypoint dropping out of coverage fails rather than passing vacuously. Presence is still checked for all 11.
...
**⚠ `research_minor` was the workflow tier the first two rows were measured on, and it no longer exists** — the research tiers merged into one `research` workflow on 2026-08-28, whose altitude is derived from the pool path. The measurement stands as taken; the tier name is kept because rewriting it would misattribute a 2026-08-12 figure to a workflow that did not yet exist. **Every other `research_minor` in this component is the same citation**, and a build reproducing these figures runs a component-level `research` cycle.
