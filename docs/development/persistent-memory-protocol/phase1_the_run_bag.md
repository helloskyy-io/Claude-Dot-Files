...
- **The ordering check reaches 9 of the 11 entrypoints.** It can see a `worktree_add` call and a workflow handoff called by name; `run_research.py`, `run_research_minor.py` and `run_review_pr.py` reach their workflow through an aliased module that also carries pure prompt-assembly functions legitimately running before the bag, so treating any call on it as a side effect would fail those three falsely. The set it does not cover is **pinned in the test**, so an entrypoint dropping out of coverage fails rather than passing vacuously. Presence is still checked for all 11.
