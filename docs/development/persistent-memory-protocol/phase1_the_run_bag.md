...
8. **The bag has a stated lifecycle: open, sealed, redacted, incomplete** (§ *Bag lifecycle* below), and the validator reports each state distinctly. `incomplete` is [Phase 3](phase3_the_emit_rule.md)'s write-failure marking and it is independent of the other three.
...
11. **Opening the bag is an ACTIVITY that a parent invokes as its first step**, not a library call each workflow remembers to make. § *Why this is an activity and not a library* below.
...
Made structural, the failure mode inverts: a workflow that does not open a bag does not get a run with an absent journal, it gets a run that **does not start** (requirement 9).

**⚠ What this does NOT cover, so the guard is not over-read.** It makes the journal structurally *present*; it says nothing about whether any individual write is *emitted*. A parent that opens a bag and then writes to a store through a path nobody wrapped still produces a gap, and the activity boundary cannot see that — **[Phase 4](phase4_rebuild_is_a_test.md)'s rebuild test is the guard for that class and it is a different guard.** It also does not reach model-issued writes at all: when the child itself runs `gh pr comment`, there is no fleet call site to wrap, and that half is [Phase 3](phase3_the_emit_rule.md)'s post-exit harvest.

**And what an activity *is* here is a boundary the [Temporal Integration](../temporal-integration/temporal-integration.md) component also owns.** This phase states what it needs — a step that runs before any child, is retried and recorded, and whose failure stops the run — rather than specifying the mechanism. Where the two designs meet, the shape is settled once and the other cites it ([roadmap § *Constraints that run BOTH ways*](roadmap.md#constraints-that-run-both-ways-with-the-temporal-port)).
...
- [ ] **Build bag-open as an activity** the parent invokes as its first step, and demonstrate that a workflow which skips it fails to start rather than running journal-less (requirement 11)
- [ ] Build the validator: re-hash the payload against the manifest, report pass/fail, distinguish *missing file* from *checksum mismatch*, and report `open` / `sealed` / `redacted` / `incomplete` distinctly — **with `incomplete` reportable alongside any of the other three rather than instead of them**
- [ ] Add the validator to [`testing/run-all.sh`](../../../testing/run-all.sh) with a `tests/` directory per the [Testing Standard](../../standards/testing/README.md) — `unit/` for layout, manifest generation and the four lifecycle states (including a bag that is both `redacted` and `incomplete`), `integration/` for a real bag produced by a real dispatch
