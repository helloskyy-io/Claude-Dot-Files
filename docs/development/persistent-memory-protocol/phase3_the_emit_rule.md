...
**Component:** [Persistent Memory Protocol](roadmap.md) · **Status:** not started · **Gate:** Phase 1, Phase 9 — **its mechanism, not its closure** (scoped below)
...
**⚠ And the gate is scoped to the half of Phase 9 that can close, which is not a technicality.** Phase 9's own r2 *"stays UNCHECKED until its shape is agreed with the Temporal port"*, and [roadmap § *Open inputs*](roadmap.md) item 4 says that question *"is not answerable from here"*. **An unscoped `Gate: Phase 9` would therefore put this phase — and [Phase 4](phase4_rebuild_is_a_test.md) and [Phase 6](phase6_cpi_reads_the_journal.md) behind it — permanently behind another component's naming decision**, which would make the roadmap's *"commitments 1 through 5 … the component's whole thesis, standing on its own"* false. **What this phase actually needs is Phase 9's r1, r2's MECHANISM and r4** — one ruled authority, a run id that is an input rather than a mint, and an explicit standalone-child discriminator. **It does not need r2's closure**, which is a deadline against the port's Stage B and not a precondition of emitting.
...
- **[Phase 9](phase9_one_run_one_identity.md)** — hard dependency on **r1, r2's mechanism and r4**, and stated from both sides. It rules who names a run; this phase threads that value down to the emitting children. **Not a dependency on Phase 9 reaching complete** — its r2 closes against the Temporal port and this phase does not wait on that. Its requirements are not restated here.
