...
**It surfaces on two channels that are not the journal, and the second one is why the first is not enough.** The typed exit record plus a non-zero exit status carry it out of the process — **and both are invocation state: read within seconds and then gone**, so a failure reported only there is invisible to every consumer this component builds. **The durable half is the parent's own [working-record](../../guide/memory-model.md) write**: a pull-request comment, which is durable, addressable, and outside the journal root by construction. **Not the standup tracker** — that surface is [`tracked/operations/`](../../../tracked/operations/), and [Tracked Items §1.2](../../standards/documentation/tracked_items_standard.md) makes it human-in-the-loop only with no autonomous write, ever. This document states that prohibition itself in § *The write-path inventory*; offering it here as a destination for a **fleet-code** write would put a binding-standard violation on the one path that exists to report this component being broken.
