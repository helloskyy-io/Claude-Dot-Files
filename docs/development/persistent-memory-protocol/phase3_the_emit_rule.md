...
**Component:** [Persistent Memory Protocol](roadmap.md) · **Status:** not started · **Gate:** Phase 1, Phase 9
...
13. **The bag's tag namespace has a stated extension rule.** Three questions, one requirement: **who may add a `Journal-` tag**, **what a reader does with one it does not recognise**, and **who bumps the schema version when a field is added from outside this component**. [Phase 1](phase1_the_run_bag.md) r6 gave the schema version a home in `bag-info.txt` and requirement 8 above governs versioning for *events*; neither says what happens when a **different component** writes into the bag's tag space. **Trigger: [WD Phase 5](../workflow-decomposition/phase5_configuration_a_run_absorbed.md) r1** — a sixth `Journal-` tag beside the five that exist, and the first field this bag has ever taken from outside. **This requirement asks WHO decides, and deliberately does not answer whether that particular tag needs a bump** — nobody has verified that, and settling it here would manufacture a decision no run has made.
...
- **[Phase 9](phase9_one_run_one_identity.md)** — hard dependency, and stated from both sides. It rules who names a run; this phase threads that value down to the emitting children. Its requirements are not restated here.
...
- **⚠ [Phase 5](phase5_snapshots_then_retention.md) requirements 2–5 are a PRECONDITION of this phase, not a parallel track.** This phase is the one that makes the journal grow: from Phase 1's 496-byte floor per run to roughly 4.9 MB, on a root every run now hard-depends on. With no retention in place, a full disk stops **every** run including the one an operator would use to diagnose it — [Phase 1](phase1_the_run_bag.md) § *Why this is an activity and not a library* names that cost, and r2–r5 are labelled `(ungated)` precisely so they can land first. Recorded here as a dependency rather than left in Phase 1's prose, because **a rule written only as prose has not once prevented a write path being added without it** — which is this component's own argument for requirement 11, applied to its own sequencing.
...
- [ ] State the tag-namespace extension rule beside the bag's own metadata — who may add a `Journal-` tag, what a reader does with an unrecognised one, and who bumps the schema version for a field added from outside this component
