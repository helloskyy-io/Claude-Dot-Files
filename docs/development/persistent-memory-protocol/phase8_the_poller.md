# The poller — Persistent Memory Protocol
...
Today, work that is ready to be picked up sits in a file marked `status: open` and waits for a person to notice. The information is already there and already machine-readable; what is missing is something that looks at it on a timer and starts the run. This phase builds that: a scheduled workflow that reads a store, finds work marked as needing attention, and starts a child.

**Nothing new is invented on the memory side.** No new surface, no new marker, no new "queue" table. A [`tracked/`](../../../tracked/) item's `status: open` already is the marker, and a to-do bit is a required property of every [working record](../../guide/memory-model.md) in this fleet. Creating another one would mean two things to keep in sync, and the fleet has already measured what happens then.

**Terms used here.** A **journal** is the whole record: one folder per run, never edited after the run ends. A **store** is any place other than the journal that a run writes to — a [`tracked/`](../../../tracked/) item file, a pull-request comment, an issue. A **to-do bit** is a machine-readable flag on a record saying whether it still needs something; in this fleet's file surfaces it is the `status:` field of [Tracked Items §3](../../standards/documentation/tracked_items_standard.md)'s shared core. A **cue** is a to-do bit that has fired — a specific record that needs a specific run. An **edge** is one machine running this fleet.
...
**It is the last thing built on the memory path because it is the only phase that makes the fleet act rather than remember**, and acting on a record whose completeness is unproven is worse than not acting.  [Phase 4](phase4_rebuild_is_a_test.md) is what makes the record's completeness a test rather than a claim; a poller built before it would be dispatching work off a record nobody had checked.
...
8. **The store's own unactioned-item rule is implemented as a second cue.** [Tracked Items §0](../../standards/documentation/tracked_items_standard.md) property 3 states that *"an item that survives three consecutive triage passes without a ruling is itself a finding, reported at the next standup"* — **a rule the standard requires and nothing computes.** It is exactly this phase's shape: a scheduled read of durable state producing an action with no human trigger, on a surface that already exists. **The output is a report, never a dispatch** — the item is the operator's to rule on, so escalating it is surfacing it at the standup and nothing more. **Placed here rather than filed** because this phase owns the trigger: nothing before it can run a schedule, and after it the capability is a table row.
...
**(a) The workflow is chosen by the surface, never by the row.** The mapping from cue to dispatch is a fixed table in code: *this surface's open items start this workflow*. Item content is passed as **data** — never as workflow selection, never as an unquoted fragment of a prompt. **A poller that decides what to run by reading what an item says is a remote-code-execution path wearing a scheduler's clothes**, and it would be one the moment [Phase 7](phase7_s3_aggregation.md) makes an item's ultimate origin a machine other than this one.

**⚠ That table is now DERIVED rather than invented, and the four-store migration is what made it so.** [Tracked Items §4](../../standards/documentation/tracked_items_standard.md) already states a *triage cadence and a named runner* per store — `triage-candidates` for `candidates/`, the sprint's owner at close-out for `issues/`, a standards pass for `standards/`, the standup for `operations/`. **A ratified standard already says which actor consumes which surface**, so this phase's job is to bind the three machine-runnable rows to a dispatch rather than to decide the mapping. **A table that restates a standard is two sources of truth**, so the code cites §4 and this doc records the derivation.

**And `tracked/operations/` is excluded from that table, by construction rather than by preference.** §1.2 makes it human-in-the-loop only: *"no workflow, dispatch or agent writes into it autonomously."* A poller that started work from an operations item would be reading the operator's own private queue as a dispatch instruction — which is not a write, so §1.2 does not literally forbid it, and is exactly the reading this line exists to close. **The store whose whole purpose is that a human decided each line belongs there is not a machine trigger.**

**(b) The poller acts only on cues of local origin.** Trace the chain the plan already contains: a party with write access to a shared bucket lands a record → [Phase 4](phase4_rebuild_is_a_test.md)'s replay applies it → an item appears in [`tracked/candidates/`](../../../tracked/candidates/) → this phase reads that item and starts a run. **Every link exists in the design; none of them is hypothetical**, and the only one that does not exist *today* is the shared bucket. And the [heartbeat pollution paper](https://arxiv.org/pdf/2603.23064) measured exactly the entry point — pollution reaching durable memory at rates up to 91%, **with prompt injection not required.** **⚠ Confidence: ONE READING, NOT VERIFIED.** That paper is in neither research pool. Its only home in this repo is [`research/synthesis.md`](research/synthesis.md) § *Live-session sources*, whose own header says *"one reading, no verification"* and *"do not cite this document as evidence; cite what it points at"* — and nobody has fetched it through `research-critic`. **The requirement does not rest on the figure and must not be read as doing so**: the argument is the causal chain above, which stands on this plan's own design. The measurement is corroboration, and corroboration at this confidence is worth stating with its confidence rather than either dropping it or laundering it. *(Marked 2026-08-28; both sites carried the figure unmarked until then.)*
...
**The answer is neither, and it follows from this component's own rule that a failure is never silent** ([Phase 3](phase3_the_emit_rule.md)): a failed dispatch emits, the cue stays open, and **repeated failure against one cue is itself a condition someone is told about** rather than a loop nobody watches. What the escalation surface is — a bounded attempt count, a flag on the row — is a build-time choice; that there is one is a requirement. **The standup tracker is NOT among the options**: it is [`tracked/operations/`](../../../tracked/operations/), and [Tracked Items §1.2](../../standards/documentation/tracked_items_standard.md) admits no autonomous write to it — the same rule § *What a cue may start* already cites to exclude that store from the cue→dispatch table. A failure record written there would be a machine write into the operator's own queue, arriving through this phase's failure path rather than its dispatch path.
...
- **Any change to who may set a to-do bit.** `ready:` on a [`tracked/operations/`](../../../tracked/operations/) item and `ratification:` on a standards candidate are the operator's alone and stay that way, and [Tracked Items §1.2](../../standards/documentation/tracked_items_standard.md) forbids any machine write to `operations/` at all; this phase reads bits, it does not write them. **That read-only posture is about to-do bits specifically and not about every write this phase makes** — requirement 5's failure record is a write, and it emits like any other.
...
- [ ] Implement the unactioned-item rule (requirement 8): count triage passes an item has survived without a ruling, and surface the ones past three at the next standup — **as a report, not a dispatch**
