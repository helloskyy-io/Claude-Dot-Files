...
7. **Mutual exclusion between two SIMULTANEOUS openers of one bag is stated as a required property and its mechanism is ruled with the identity design — not here.** This is the residual property [Temporal Integration](../temporal-integration/temporal-integration.md) § *The dispatch record already exists* hands over when it retires the two-tier state store proposal: the bag replaces that store on every count except **compare-and-swap on the small part**. **It is a separate requirement rather than a clause of r3 precisely so that r3 can close.** r3 closes on the sequential property, demonstrated; this one stays open, visibly, and cannot be checked by accident — the alternative buries a live property inside a ticked box, which is the failure TI's own text says the supersession exists to prevent.
    ...
    **No mechanism is chosen here** — a lock, a create-then-rename and a compare-and-swap are all open, and picking one before the concurrency question is answered would be designing for a race nobody has shown exists. **Where it is ruled:** whichever carrier [`C-zhdm5gh1`](../../standards/architecture/research/candidates.md) resolves to — that row leaves the SHAPE open between widening §A3 and splitting identity into its own §A-section, and nothing here should assert one, because half the cross-references would then describe an artifact that does not exist. §A3 as it stands is scoped to machine-axis queue naming and cannot hold this; that scope gap is queued as [`C-zhdm5gh1`](../../standards/architecture/research/candidates.md) and **this requirement cannot be marked complete while its carrier does not exist**.
