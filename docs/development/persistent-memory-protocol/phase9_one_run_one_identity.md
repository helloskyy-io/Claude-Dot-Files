...
3. **Opening a bag twice under one run id yields one bag, not two — demonstrated.** That is the shape a retry takes, and it is the only requirement here whose failure is silent: two bags for one run reads as two runs forever after.
    ...
    **What this phase must deliver is the sequential property, demonstrated; the simultaneous property is NAMED HERE AND RULED BY THE PORT.** Sequential retry is the case an at-least-once orchestrator actually produces, and it is the case this phase can close on its own. Whether the fleet ever needs mutual exclusion depends on whether two openers can be live at once, which is a function of how the port schedules a dispatch — so it is decided with the rest of the identity design, in [the local Temporal addendum](../../standards/temporal/claude-dot-files-addendum.md), on the same argument § *The identity is a joint design* makes below. **This requirement is the residual property's stated carrier**, which is the thing [Temporal Integration](../temporal-integration/temporal-integration.md) § *The dispatch record already exists* needs from it: closing the gap means EXTENDING r3, not merely satisfying it. **No mechanism is chosen here** — a lock, a create-then-rename or a compare-and-swap are all open, and picking one before the concurrency question is answered would be designing for a race nobody has shown exists.
...
6. **The caller-supplied run id is VALIDATED against a stated allowlist before it is used as a folder name or written into `bag-info.txt`, and the allowlist is expressed as the set of PERMITTED characters rather than as a list of forbidden separators.** The reason is one clause: the id is written into a tag-line composer as `f"{label}: {value}"`, so **any character that can terminate a line can forge a subsequent label**. Today's guard refuses separators and relative segments but not newlines, and `External-Identifier` is written *before* the loop that runs `_refuse_folded_value`, so the id never passes that check at all. **Requirement 2 is what creates the exposure** — the id stops being minted inside the journal from a fixed alphabet and starts arriving from a caller — which is why the check lands in this phase rather than a later one.
    ...
    **What the permitted set actually is, is this phase's ruling and is deliberately not fixed here.** The requirement is that a set is *stated* and *enforced*, not which characters it contains.
...
*(**Cited by section TITLE rather than by number, deliberately.** That synthesis is rewritten whenever the pool is rescoped and its section numbering does not survive the rewrite — the numbered `§3` this line used to name no longer exists in it. The same rule and the same reason are stated on the other side of the seam, at [Temporal Integration](../temporal-integration/temporal-integration.md) § *The dispatch record already exists*.)*
...
- [ ] Demonstrate requirement 6: a test that a run id containing a **newline** is REFUSED at bag-open
