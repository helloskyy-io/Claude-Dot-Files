...
**The gate is the Temporal server**, and it reaches **only the recurring half**. A retention pass is a scheduled workflow plus a configuration value; there is no scheduler today, so nothing runs it on a cadence. That is a fact about the calendar.
...
1. **A recurring snapshot records every store's state into the journal**, addressable as the point a rebuild starts from. It records the same store set [Phase 4](phase4_rebuild_is_a_test.md) tests against, and a store Phase 4 could not rebuild is not silently snapshotted as though it could be.
2. **A retention dry-run refuses to delete past the most recent snapshot**, demonstrated against a real journal — and the refusal is a non-zero exit with a named reason, not a warning.
3. **The authored record and the transcript carry separate stated retention rules**, and the transcript is removable from inside a run folder without destroying the record.
4. **A rebuild from the most recent snapshot forward still passes [Phase 4](phase4_rebuild_is_a_test.md)'s test after a retention pass has run** — demonstrated by running that test on both sides of a real deletion.
...
6. **A retention pass emits its own journal events** — what it deleted, when, and under which snapshot — and leaves each affected bag validating in the `pruned` state rather than reporting missing files.
7. **A bag that is both incomplete and pruned stays distinguishable as both.** § *Retention meets a gapped bag* below.
...
- **[Temporal Integration](../temporal-integration/temporal-integration.md) → *Stand up the Temporal server*** — for the recurring half only.
...
- **Deleting the content store.** [Phase 2](phase2_content_store.md)'s cached bytes are referenced by checksum from potentially many bags, so deleting them needs a reachability pass this phase does not build. **Named rather than assumed**: a retention pass that removes a bag does **not** remove the objects it referenced, so the content store grows unbounded until something owns this. That is a real gap and it is stated as one.
...
- [ ] Specify the retention rule: whole folders oldest-first, never past the most recent snapshot, with the refusal as a non-zero exit and a named reason
