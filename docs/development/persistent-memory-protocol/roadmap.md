...
**NOT SIZED — the prior figure was measured before this phase was split and no longer describes it.** That figure was recorded with the note that *the size is itself a finding rather than just a number*: thirteen requirements, and *"roughly half again as large as the biggest phase sized anywhere else in this repo's sprint plan"*, against this component's own run-identity doc observing that adding a fourth concern here "makes the phase a project". **A finding with no remedy is a note, so the remedy was taken**: the model-issued half of the write-path inventory is now [the model-issued harvest](phase10_the_model_issued_harvest.md), on the seam this basis had already named — *an inventory whose two halves need different mechanisms (a call-site wrapper, and a post-exit harvest that has no wrapper to be)*. **What remains here** is the event contract, write-ahead ordering carried as intent-plus-completion, four write-failure cases demonstrated against a real journal, six new event fields, the capture-time secret filter, the tag-namespace ruling, the run-log cut-over where a join key changes meaning, and wiring every fleet-code path in the inventory. `plan-verify` sizes both halves cold.
