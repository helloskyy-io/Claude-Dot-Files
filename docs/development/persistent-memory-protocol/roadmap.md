...
**Status: 🟡 IN PROGRESS.** [The journal root and the run bag](phase1_the_run_bag.md) is complete — the journal root, the run bag and its validator ship today. The run-identity phase listed immediately below was **built on 2026-08-24** and is not complete: five of its seven requirements are met and two stay open by design, which its own doc states and this line records so the roadmap does not read as though nothing had happened since. Ten phases have a phase doc. Four of them wait on something that does not exist yet; that is a scheduling fact recorded below, and it is not a reason any of them is planned less completely.

**Phases are listed in logical rollout order. Phase numbers are creation-order identifiers and do not reflect rollout sequence; execution order across components lives in [`sprint.md`](../sprint.md).** [One run, one identity, one bag](phase9_one_run_one_identity.md) is listed second for that reason — it was created last and is built early.
...
| **completeness** | The property that if any store got something, the journal got it too, verbatim. It is not an assertion — [rebuildability is a test](phase4_rebuild_is_a_test.md) turns it into a test that goes red when it stops being true. |
...
**And audience did not predict behaviour where lifecycle did.** §4.3.4's second observation, taken against the two file surfaces of the time: both were markdown tables in one directory, both read by humans and by code, both in the same class — and one never deleted a row while the other rotated a ruled row out at 90 days. Audience says nothing about that difference. What ends a record's life does.

**The four-store migration of 2026-08-26 replaced those two files and the observation got SHARPER rather than stale.** [`tracked/operations/`](../../../tracked/operations/) admits no machine write at all while [`tracked/candidates/`](../../../tracked/candidates/) is filed, triaged and incremented entirely by dispatches — same substrate, same directory, same readers. And [Tracked Items §4.2](../../standards/documentation/tracked_items_standard.md)'s prune clock runs from **last activity**, so **two items in ONE store have different lifetimes**: `resolved` at 14 days, `rejected` at 6 months, and a recurring item never at all because each `count` increment restarts its clock. Audience predicts none of that. Lifecycle is still the axis.
...
| **the working record** | its to-do bit clears | the current state of one unit of work, edited in place, with the history behind it dropped | pull-request threads, and the four [`tracked/`](../../../tracked/) stores |
...
**And a class's substrate may move without the class dissolving.** [The journal root and the run bag](phase1_the_run_bag.md) § *The surface this replaces* **has now ruled it: the journal absorbs the per-run log.** So measurement samples get a new home and a new clearing rule while remaining what they are — a number that means nothing alone — and `run_log.py`'s three member event types keep their class across the move. The cut-over is [the emit rule](phase3_the_emit_rule.md)'s, which is where emitters live.
...
**The lifecycle axis keeps discriminating one level down, which is the point.** Within the working record, *how* the to-do bit clears and what happens to the item afterwards is a per-surface property and is stated per surface — and after the migration it discriminates one level further still, *within* a surface, because [Tracked Items §4.2](../../standards/documentation/tracked_items_standard.md)'s clock depends on the item's own terminal state and its own activity. That is the difference audience never predicted, and it stays visible because the axis the classes are named on is the axis it lives on.
...
| The typed exit record's envelope and its fail-safe contract | [`exit-protocol.md`](../../standards/exit-protocol.md) | `exit-protocol.md` — a routing channel, and a **separate contract** from the journal event ([the emit rule](phase3_the_emit_rule.md) r3) |
| The journal event contract | — | **[The emit rule](phase3_the_emit_rule.md)** |
| The storage budget and what deletion costs | — | **[Snapshots, then retention](phase5_snapshots_then_retention.md)**, on commitment 6 |
...
**It does not own:** the Temporal port ([`temporal-integration/`](../temporal-integration/temporal-integration.md)), the fleet's deployment automation ([`C-2xgap6o1`](../../../tracked/candidates/C-2xgap6o1.md)), or **managed configuration and the digest of what a run absorbed** — that is [Workflow Decomposition · What configuration a run absorbed](../workflow-decomposition/phase5_configuration_a_run_absorbed.md)'s in full, by operator ruling on 2026-08-19.
...
**This component depends on it in one specific, load-bearing way.** When the journal itself cannot be written, the failure cannot be recorded *in the journal* — so it surfaces on the one channel that is not the journal: this record, plus the process exit status ([the emit rule](phase3_the_emit_rule.md) § *When the journal cannot be written*, case (d)). That is the dependency running the right way round, and it is why the record is adopted rather than replaced.

**This protocol is its own component, not a phase of anything, by operator ruling on 2026-08-16** — the framework it would have been a phase of was retired into it. [`C-xh8nvwqn`](../../../tracked/candidates/C-xh8nvwqn.md) asks the question; its `decision` is the triager's to set and this is the evidence.
...
This is the commitment that keeps commitment 2 honest, and it has to be designed rather than discovered. **The rule is: a gap may exist; a silent gap may not.** Where a journal write has a paired store write, the emit goes first and a failed emit means the store write does not happen — so the invariant in commitment 2 is preserved by *both* sides being absent, which is loud and recoverable. Where there is no paired store write, a failure appends a typed gap event and marks the bag `incomplete`, and everything downstream treats an `incomplete` bag as a known-gap input rather than a clean one. [The emit rule](phase3_the_emit_rule.md) owns the rule; [the journal root and the run bag](phase1_the_run_bag.md) owns the bag state; [rebuildability is a test](phase4_rebuild_is_a_test.md) and [CPI reads the journal](phase6_cpi_reads_the_journal.md) honour it.
...
**The arrangement is settled and it is short: each edge has access to an S3 bucket — shared or its own — and uses it as it sees fit.** Today there is one edge and it has access to its own data. What a *shared* bucket would additionally require of a reader is decided by the need that produces one, not now — and [cross-machine aggregation](phase7_s3_aggregation.md) states the shape it would take so nobody re-derives it.

**9 · What goes in is filtered at capture, and the one exception to immutability is a redaction.**
The transcript carries the literal input of every command the fleet runs, and this fleet runs with permissions bypassed. A secret that reaches the journal is sealed into a checksum-covered file that [rebuildability is a test](phase4_rebuild_is_a_test.md)'s test then makes expensive to remove — so the filter runs **at capture, before any byte lands**, and it emits a placeholder so the record is honest about the removal rather than silently shorter. And because a filter is best-effort, there is **one stated exception to never-changing-a-written-event**: a redaction is a new appended event that supersedes, the file is replaced by a marker, the manifest is regenerated and that regeneration is itself recorded — tagged distinctly from a **gap event**, which shares its shape — both leave a bag whose payload differs from what was first written — so a deliberate redaction is never mistaken for bytes lost to a full disk. **Nothing is ever silently edited.** [The emit rule](phase3_the_emit_rule.md) owns the filter; [the journal root and the run bag](phase1_the_run_bag.md) owns the exception.

**Where the design is deliberately incomplete, and it is named rather than hidden.** One input is not derivable from any amount of work: **what a machine actually is**, when the second one is not a full Linux environment. It is listed in § *Open inputs*, carried by [the journal root and the run bag](phase1_the_run_bag.md), and it does not block that phase — [the journal root and the run bag](phase1_the_run_bag.md)'s first requirement is that the root is a config value and nothing depends on a home directory.
...
**⚠ And the activity boundary alone does not deliver that, which is why each of the three requirements carries a test rather than a placement.** Nothing in an orchestrator forces a workflow to invoke a particular activity first — a workflow that omits the call simply omits it. So *"the journal code lives in the activities layer"* is tidiness, not a guarantee; and [the journal root and the run bag](phase1_the_run_bag.md) r9's refuse-to-start cannot supply it either, because **r9 fires only once bag-open has already been invoked.**

**What delivers it is an enumerating test, and this repo already ships the exemplar** — `test_every_entrypoint_actually_calls_preflight` discovers every entrypoint under `scripts/workflows/temporal/scripts/` and asserts each one calls `preflight(`. [The journal root and the run bag](phase1_the_run_bag.md) r11, [the content store](phase2_content_store.md) r8 and [the emit rule](phase3_the_emit_rule.md) r12 each take that shape for the surface they own. **A parent added without the call goes red on the merge path**; a parent that opens a bag against an unresolvable root gets a run that does not start ([the journal root and the run bag](phase1_the_run_bag.md) r9). Two guards, two failures, neither substituting for the other.
...
**What this does NOT cover, stated so the guard is not over-read.** It makes the journal *structurally present*; it does not make any individual write *complete*. A parent that opens a bag and then writes to a store through a path nobody wrapped still produces a gap, and neither the boundary nor the sweep can see that — **[Rebuildability is a test](phase4_rebuild_is_a_test.md)'s rebuild test is the guard for that class.** It does not reach model-issued writes at all, which are [the emit rule](phase3_the_emit_rule.md)'s post-exit harvest and are structurally outside any wrapper. And **an enumerating test is only as good as its discovery predicate** — a parent living outside the swept directory is invisible to it.

**⚠ Making the journal mandatory has a cost, and it is named rather than discovered.** Once [the emit rule](phase3_the_emit_rule.md) lands, a full disk stops *every* run including the one you would use to diagnose it. Two things bound that, and both are stated: [snapshots, then retention](phase5_snapshots_then_retention.md)'s requirements 2–9 are ungated and should land with or before [the emit rule](phase3_the_emit_rule.md) rather than waiting for the server only its requirement 1 needs; and [the journal root and the run bag](phase1_the_run_bag.md) r9's refusal names the resolved path and the failing property, so recovery does not need a working journal. **Why that phase is nonetheless listed and scheduled later than the phase it precedes is reconciled in § *The order, and what each part waits on*** — the gate is on its requirement 1 alone, and r2–r9 are what this sentence is about.
...
**[The journal root and the run bag](phase1_the_run_bag.md) is built.** The journal root, the run bag, its BagIt manifest and its validator ship today, and eleven entrypoints open a bag before their first side effect.

**Five phases have no EXTERNAL gate**: 9 (run identity), 2 (the content store), 3 (the emit rule), 10 (the model-issued harvest), 4 (the rebuild test). Four of them could start today; 9 was built on 2026-08-24 and its two open requirements are the residual described below. They depend only on each other and on [the journal root and the run bag](phase1_the_run_bag.md) — 1 → 2, and 1 → 9 → 3 → 10 → 4, as each phase doc's own header says, so *ungated* means *waiting on nothing outside this component* rather than *runnable in parallel*. Together with [the journal root and the run bag](phase1_the_run_bag.md) they deliver commitments 1 through 5 — which is the component's whole thesis, standing on its own.

**[The model-issued harvest](phase10_the_model_issued_harvest.md) is new, and it is a SPLIT of [the emit rule](phase3_the_emit_rule.md) rather than added scope.** [The emit rule](phase3_the_emit_rule.md)'s own sizing basis named the seam — an inventory whose two halves need different mechanisms, a call-site wrapper and a post-exit harvest that has no wrapper to be — and recorded that the phase's size *"is itself a finding rather than just a number"*. A finding with no remedy is a note. **Nothing moved into this component and nothing left it**; one phase became two on a boundary the plan had already drawn, and [`C-6i1u3b1f`](../../../tracked/candidates/C-6i1u3b1f.md) is the filed evidence that the moved half needed a deliverable of its own.

**⚠ Ungated is not the same as unpressed, and [one run, one identity, one bag](phase9_one_run_one_identity.md) is the one that shows the difference.** It waits on nothing, and two other components have already set clocks on it: [Workflow Decomposition · Dual-mode children](../workflow-decomposition/phase3_dual_mode_children.md) adds nine entrypoints into the directory this component's bag-open sweep enumerates, and the Temporal port's Stage B wraps the functions the run id is generated inside. **Neither is a gate on this component; both are deadlines, and a deadline nobody schedules against is missed by default.**

**Four wait on something named:**

| Phase | Waits on | Why that thing and not another |
|---|---|---|
| **[Snapshots, then retention](phase5_snapshots_then_retention.md)** | the Temporal server (*Stand up the Temporal server*, a milestone of the [Temporal Integration](../temporal-integration/temporal-integration.md) component, tracked as a checkbox in [`sprint.md`](../sprint.md) § *Sprint: Temporal Integration*) — **and only its recurring half** | Running the retention pass *on a cadence* is a scheduled workflow. Only requirement 1's recurrence needs that; the other eight are a policy and a command, and [rebuildability is a test](phase4_rebuild_is_a_test.md) already builds the one-off snapshot. |
| **[CPI reads the journal](phase6_cpi_reads_the_journal.md)** | *Port `review-runs`* — same component, same tracking surface | The evidence sweep exists today only in the frozen bash fleet, which may not be modified. **Not the Temporal server** — a sweep is a batch read, not a schedule. |
| **[Cross-machine aggregation](phase7_s3_aggregation.md)** | a second machine that actually produces runs | Building this before a second machine exists is the speculative-generality trap [`state_passing`](research/raw/state_passing_between_workflow_children.md) §5.2 warns this fleet against. **One gate, not three** — the storage arrangement is settled (commitment 8) and is not a gate. |
| **[The poller](phase8_the_poller.md)** | Temporal schedules (same server) | Reading a to-do bit and starting a child with no human trigger is what a scheduler does. |

**⚠ [Snapshots, then retention](phase5_snapshots_then_retention.md) is listed AFTER [the emit rule](phase3_the_emit_rule.md) while three documents call its requirements 2–9 a precondition of that phase. Both are true, and this is the sentence that says how.** *(Ruled by the operator, 2026-08-28: the phase is **not** split, and it stays where [`sprint.md`](../sprint.md) puts it. Recorded here because the apparent contradiction is what a reader hits first, and it has been escalated once already.)*

- **[`sprint.md`](../sprint.md) sequences PHASES; a phase doc tracks REQUIREMENTS.** Those are different granularities, deliberately. The sprint can only place a phase where the *whole* phase can close, and requirement 1 — running the retention pass on a cadence — needs the Temporal server.
- **So this phase's position, here and in the sprint, is correct.** Listing it earlier would state a schedule nobody can execute.
- **`(ungated)` on requirements 2–9 already carries the precondition, and it is not decoration.** Those eight wait on nothing outside this component. They MAY be built before the phase is scheduled and they SHOULD be, because they are what stops a full disk halting every run once [the emit rule](phase3_the_emit_rule.md) takes a run from [the journal root and the run bag](phase1_the_run_bag.md)'s 523-byte floor to roughly 4.9 MB.
- **A half-built phase is a normal state, not a defect to design around.** A phase does not close until all of it is done, and its own checkboxes are what show which half is standing. That is the whole reconciliation: the **phase** is gated, eight of its **requirements** are not, and the two facts live in different documents because they answer different questions.

**⚠ EVERY ONE OF THOSE GATES IS DEEPER THAN IT READS, and the depth was added after this table was written.** Both named milestones — *Stand up the Temporal server* and *Port `review-runs`* — sit in [`sprint.md`](../sprint.md) § *Sprint: Temporal Integration* **below** a milestone that says of itself that it *"gates everything below it"*: proving a dispatched invocation is indistinguishable from an operator at a terminal. That component is in turn gated on [Workflow Decomposition](../workflow-decomposition/roadmap.md), which has four open phases. **So [snapshots, then retention](phase5_snapshots_then_retention.md), [CPI reads the journal](phase6_cpi_reads_the_journal.md) and [the poller](phase8_the_poller.md) are all transitively behind a viability question and a whole sibling component**, and this document previously implied they were behind one checkbox each.

**What that does NOT change:** the gates are still correctly *named*, [CPI reads the journal](phase6_cpi_reads_the_journal.md)'s is still the port and not the server, and [CPI reads the journal](phase6_cpi_reads_the_journal.md) can still be pulled forward relative to [snapshots, then retention](phase5_snapshots_then_retention.md) and [the poller](phase8_the_poller.md). **What it changes is the planning assumption** — *"waiting on one checkbox"* and *"waiting on a component plus an unanswered question"* schedule very differently, and the second is the true one.

**[Cross-machine aggregation](phase7_s3_aggregation.md)'s gate is unaffected and is still a fact about the world:** a second machine that actually produces runs. Nothing in the Temporal port opens it.
...
|---|---|---|
| [The poller](phase8_the_poller.md) r7 | every event carries a **provenance class**, so a reader that *starts work* off a row can filter it by origin | [The emit rule](phase3_the_emit_rule.md) r7(d) |
| [Cross-machine aggregation](phase7_s3_aggregation.md) § *Where a shared bucket would change things* | every event carries a **credential epoch**, so a replay can exclude events authored under a credential known to have leaked | [The emit rule](phase3_the_emit_rule.md) r7(c) |
| [Cross-machine aggregation](phase7_s3_aggregation.md) r2 | the content store's shape is ruled such that a shipped bag's cited bytes are resolvable at the destination | [The content store](phase2_content_store.md) r7(a) |
| [Cross-machine aggregation](phase7_s3_aggregation.md) r3 | the sweep reads through a storage interface, so pointing it at a bucket is not a rewrite | [CPI reads the journal](phase6_cpi_reads_the_journal.md) r7 |
| [Cross-machine aggregation](phase7_s3_aggregation.md) § *The storage arrangement* | the payload spec can express a **per-field** classification, because a classification the payload shape cannot express is one nobody can act on later | [The journal root and the run bag](phase1_the_run_bag.md) r7 |
| [The poller](phase8_the_poller.md) r7 | the provenance class survives the rebuild, so a rebuilt row can be filtered by origin | [Rebuildability is a test](phase4_rebuild_is_a_test.md) r9 |
...
| Stated by | The constraint | Discharged by |
|---|---|---|
| [WD · What configuration a run absorbed](../workflow-decomposition/phase5_configuration_a_run_absorbed.md) r1 | a **sixth `Journal-` tag** is written into `bag-info.txt` by another component — so the bag's tag namespace needs a stated extension rule: who may add a tag, what a reader does with one it does not know, and **who bumps the schema version**, since a field absent from v1 records is absent for good | **[The emit rule](phase3_the_emit_rule.md) r13** — the extension rule is its own requirement, stated there because this is the first field the bag has taken from outside this component; **r8 stays the event-versioning rule r13 has to answer to**. **The requirement is written; the phase is not built** — [the emit rule](phase3_the_emit_rule.md) has not started, and that phase has no gate stopping the sixth tag landing first. **Neither gates the other; whichever lands first sets the contract** — the same shape as row 3. If that phase lands first it states its own answers to r13's four questions in its PR, and r13 ratifies or overrides them |
| [WD · What configuration a run absorbed](../workflow-decomposition/phase5_configuration_a_run_absorbed.md) r3 | the config-digest reader is a **consumer of bag content**, so it belongs in the consumer enumeration this component owes | [CPI reads the journal](phase6_cpi_reads_the_journal.md) r3, which enumerates consumers rather than asserting them |
| [WD · Dual-mode children](../workflow-decomposition/phase3_dual_mode_children.md) | nine **new entrypoints** land in the directory the bag-open sweep enumerates, so what a standalone child does about the journal is decided by whoever lands them unless it is decided first | **[One run, one identity, one bag](phase9_one_run_one_identity.md)** r4, r5 |
| [WD · Nothing a run relies on is invisible](../workflow-decomposition/phase4_nothing_invisible.md) | the producer-with-no-consumer gate is extended **beyond one directory**, which brings this component's producers under it | [CPI reads the journal](phase6_cpi_reads_the_journal.md) r3 — **and see the sequencing warning below, because the two can collide** |
| [Temporal Integration](../temporal-integration/temporal-integration.md), Stage B | the run id must be **supplied to** the work rather than generated inside it, or a retry and a replay each produce a new one | **[One run, one identity, one bag](phase9_one_run_one_identity.md)** r2 |

**⚠ [WD · Nothing a run relies on is invisible](../workflow-decomposition/phase4_nothing_invisible.md) and [CPI reads the journal](phase6_cpi_reads_the_journal.md) can collide on schedule, and the collision is one-directional.** Phases 1–5 here ship producers — bag tags, journal events, gap events, snapshots — whose only planned consumer is [CPI reads the journal](phase6_cpi_reads_the_journal.md), which is gated on the `review-runs` port. If a fleet-wide producer/consumer gate lands while that gate is still shut, this component's own producers are what turn it red. **The resolution is not to weaken either**: it is that whichever lands second states the other's producers as a named, reasoned exemption with the phase that discharges it. Recorded here so it is a scheduling input rather than a surprise.

**The phases are listed in LOGICAL ROLLOUT ORDER, and the numbers are identifiers rather than that order.** [One run, one identity, one bag](phase9_one_run_one_identity.md) is listed second and was created last. A phase number names a phase for life, the way a ticket number does. **What gets built *when* across components is the sprint's decision and not this document's** — the gate table above says only what each phase waits on.

**Why [CPI reads the journal](phase6_cpi_reads_the_journal.md) is its own phase, split from [the poller](phase8_the_poller.md).** At draft the two were one phase gated on the Temporal server, which would have put this component's **only consumer** behind a server nobody has stood up, for four phases of producers. This fleet has the measured record of what happens then — three phases each appended a parent-written observable to one log and no committed tool read any of the three. Splitting them is why [CPI reads the journal](phase6_cpi_reads_the_journal.md) waits on the `review-runs` port rather than on the server.

**The checkboxes under each phase below are a reading aid, not a completion record.** They summarise each phase doc's numbered requirements without reproducing them, and **the phase doc is authoritative** — so a fully-checked list here never means a phase is done. A dispatch briefed from this section alone will under-size every phase; most of all [the emit rule](phase3_the_emit_rule.md), whose write-path inventory has no checkbox here. **Brief from the phase doc, and track completion against its numbered requirements.**
...
**~36 hrs, and it is the cost this phase CONTAINED rather than work remaining.** Basis: `modules/journal/` is 2,226 lines across five modules against roughly 5,460 lines of test in twelve modules, plus the `journal:` config block, eleven entrypoints wired to open a bag before their first side effect, and the enumerating sweep that makes that structural rather than remembered. The expensive half is not the package — it is the containment work, which took three passes to converge: four escapes of one shape, closed as a named function plus a sweep plus a behavioural battery carrying mutation evidence in both directions, and the suite-pollution defect that no assertion could see. Placed above the typed exit record's figure for that reason and not on line count alone.

**⚠ This replaces a stated zero, and the correction changes what the component reports.** The prior entry read *the cost this phase actually took is history … and it belongs to no total*, which is not how the figure is consumed: `plan_activities.phase_sizing` sums every estimate into TOTAL and subtracts only the phases the roadmap marks `✅` into TO-DO, so a complete phase has to carry its real cost for the two figures to differ at all. Under the zero this component reported an identical TOTAL and TO-DO, alone among the four components that have a roadmap. *(Corrected cold by `plan-verify`, 2026-08-28.)*
...
- [ ] Mutual exclusion between two **simultaneous** openers of one bag — **unchecked, and its carrier does not exist yet**: the Temporal addendum has no identity section, tracked as [`C-zhdm5gh1`](../../../tracked/candidates/C-zhdm5gh1.md)
...
**~16 hrs** *(re-sized cold by `plan-verify`, 2026-08-28; raised from the prior figure)*. Basis: one new package — the citation record, a content-addressed layout, a capture path, one re-hashing resolver and a three-exit-code `verify` — plus two activities and a unit-and-integration tier. Most of it is new mechanism rather than applying an existing one. The spread lives in r7(c): teeing an existing tool's output is the low end; a new fetcher carrying its own https-only, redirect-revalidation, private-address-refusal and size-cap policy is the high one, and that sub-decision is unmade. **What the prior figure priced as a sentence is r7(a)** — per-run versus root-level shared is not a local choice: it decides whether a shipped bag is self-contained ([cross-machine aggregation](phase7_s3_aggregation.md) r2) and whether [snapshots, then retention](phase5_snapshots_then_retention.md) owes a reachability pass, so ruling it means reconciling two other phases rather than writing a line. r7(d)'s all-reads-through-one-fail-closed-resolver is the other item that constrains the package's shape from the first commit rather than being added later.
...
**~52 hrs** *(sized cold by `plan-verify`, 2026-08-28, on the post-split scope)*. Basis, taken bottom-up over what the phase doc actually asks for: the journal event contract stated as its own contract plus the one-declaration-per-shared-concept module (r2, r3, r7, r8); write-ahead ordering carried as intent-plus-completion with replay's apply-only-a-completed-intent rule and the store deriving its idempotency key from the same identity (r4); **six** demonstrations against a real journal, not four — the four write-failure cases plus the store-write-failure and retried-activity cases § *Why one event per write is not enough* adds; seven new event fields each with its own rule and two of them carrying stated non-guarantees; the capture-time secret filter running over the transcript before any byte lands, which is the highest-volume surface in the component; r13's four-clause tag ruling with a demonstrable reader half; the run-log cut-over, where `run_log.py`'s 470 lines of member event types, join key and `PUBLISHABLE_FIELDS` move and the join key's **meaning** changes while an existing archive keeps the old one; the inventory enumerated from the tree with its third category ruled or handed on; and then wiring every inventoried fleet-code path, which is cheaper than its site count because the `tracked/` writes already funnel through shared helpers.

**⚠ THE SPLIT DID NOT CLOSE THE SIZE FINDING, AND THAT IS THE MOST USEFUL THING THIS NUMBER SAYS.** The retired figure's own words were *"roughly half again as large as the biggest phase sized anywhere else in this repo's sprint plan"*. The biggest phase sized anywhere in the plan today is **34**, so that characterisation described the pre-split phase at roughly fifty. **Removing the harvest — separately sized below — leaves a phase that is still half again the largest, which means the pre-split phase was materially larger than its own basis said.** The remedy was taken on a real seam and it was the right seam; it was not sufficient. The residual splits cleanly in three, each with its own demonstration: **(i) the event contract and its admission fields** — an event validates, and every concept shared with the typed exit record is declared once; **(ii) the failure discipline** — r4 and r11, six demonstrated failure paths and the unwritable-journal signal landing with both its readers; **(iii) the wiring and the cut-over** — r1, r9, r10, r12, r13, ending at one real cycle whose fleet-code writes are in the journal verbatim. **That is a boundary ruling and this run does not take it**; it is reported so somebody can.

The core rule, and it is absolute: if any store gets it, the journal gets it, verbatim. The destination is a field rather than a format, which is the property cross-machine work later depends on. Every emitted item records which input produced it and carries a stable machine id. **And this phase answers what happens when the journal cannot be written** — the question that decides whether [rebuildability is a test](phase4_rebuild_is_a_test.md)'s guarantee means anything.
...
### The model-issued harvest 🟠 PLANNED

**Implementation:** [`phase10_the_model_issued_harvest.md`](phase10_the_model_issued_harvest.md)

**~26 hrs** *(sized cold by `plan-verify`, 2026-08-28)*. Basis: a fetch-and-emit over three GitHub surfaces — the pull-request body, every comment on it, and issue bodies the run authored — landing on a contract [the emit rule](phase3_the_emit_rule.md) already supplies, so the mechanism is bounded and adds no second contract. Roughly a third of the figure is not the harvest at all: r4's **standing check** is a build decision between an enumerating sweep and a per-run reconciliation, plus proving it goes red when the harvest is disabled, and r3's window has to be measured against what the surface holds now rather than asserted small. The remainder is run-id-to-bag resolution that fails loudly, the gap-event path, routing harvested bytes through the capture-time filter, the activity boundary, and a unit-and-integration tier. **The vendor half is the residual uncertainty and it is real** — this is the only phase in the component that reads an API it does not own, and pagination, rate limits and the edited-or-deleted-comment representation are facts the phase doc correctly refuses to plan from memory.

Covers the half of the write-path inventory that has no call site to wrap. A child runs `gh pr comment` because a prompt told it to, and by the time anything could react the process has exited — so this phase fetches what the run wrote to its GitHub surfaces after it ends, keyed by run id, and emits it verbatim like any other event. **These are the writes the component exists for**: the pull-request body, the decision log and the reflection comment are where the reasoning lives, and a record holding every file and none of the argument answers *what changed* but never *why*. It also builds the standing check that keeps the harvest honest, which is what a one-time demonstration cannot supply — [`C-6i1u3b1f`](../../../tracked/candidates/C-6i1u3b1f.md) is that gap already filed.

**Gate: none beyond [the emit rule](phase3_the_emit_rule.md).** It is listed before [rebuildability is a test](phase4_rebuild_is_a_test.md) so the completeness test does not run over a journal knowingly missing its highest-value half — a green check over a half-record is the *half-readable reads as coverage* failure commitment 6 forbids.

- [ ] Everything a run wrote to a GitHub surface is in its bag verbatim — body, every comment, the review verdict, any issue body it authored — on the emit rule's contract and not a second one
- [ ] The harvest resolves run id to bag, and **fails loudly rather than guessing** when it cannot
- [ ] The harvest window's failure mode is stated, bounded and **measured** — comments captured against comments the surface holds now, with the denominator
- [ ] **A standing check covers this half**, in the family of the enumerating sweeps, and goes red when the harvest is disabled
- [ ] A failed harvest emits a typed gap event and marks the bag `incomplete`; harvested bytes pass through the capture-time secret filter on the same path
...
Replays the journal into a scratch directory and diffs the result against the live store. This is what makes [the emit rule](phase3_the_emit_rule.md) enforceable: without it, completeness decays silently the first time a write path is added and its emit is forgotten. The test belongs on the merge path, so a missing emit goes red rather than unnoticed.

- [ ] Replay reproduces the **ruled test set** — [`tracked/candidates/`](../../../tracked/candidates/) as the positive control and [`tracked/operations/`](../../../tracked/operations/) as the negative one — byte-identical or under a stated normalisation, from a starting snapshot forward, with the store contract version it rebuilt against recorded
...
- [ ] A rebuild from the last snapshot forward still passes [rebuildability is a test](phase4_rebuild_is_a_test.md)'s test after a deletion pass
...
- [ ] The wall-clock of the cross-run sweep is measured against journal size, and reported as the first real test of [the journal root and the run bag](phase1_the_run_bag.md)'s no-database decision
...
Folders sync to object storage under machine id and run id. The local file is the truth at write time and ships asynchronously, so a machine keeps working when the bucket is unreachable. CPI then reads the bucket instead of one local journal — same reader, different input, which is why [CPI reads the journal](phase6_cpi_reads_the_journal.md) must not be built as throwaway.
...
- [ ] CPI reads the bucket with no change to the reader written in [CPI reads the journal](phase6_cpi_reads_the_journal.md)
...
No new to-do surface is needed — a [`tracked/`](../../../tracked/) item's `status: open` already is one, and a to-do bit is a required property of every working record. What is missing is the thing that reads it: a scheduled workflow that queries state and starts children with no human trigger. **The cue→dispatch table is now DERIVED** from [Tracked Items §4](../../standards/documentation/tracked_items_standard.md)'s triage-cadence column rather than invented, with `tracked/operations/` excluded because §1.2 reserves it to the operator.
...
- [ ] The store's own unactioned-item rule — *an item surviving three triage passes without a ruling is itself a finding* — is computed and **reported** at the standup, never dispatched
...
| 2 | Every write also emits, completely; the journal rebuilds the store; rebuildability is a test | 1 (r6, versioning), 3 (r1, r9), **10** (r1, r4 — the model-issued half), 4 (r1, r2) |
| 3 | One typed return per step, modality-neutral | **3** (§ *One typed return per step, modality-neutral*, **stated**) · 3 (r3, the vocabulary half only) |
...
- **The run id is supplied by the caller, and one authority names a run** — [one run, one identity, one bag](phase9_one_run_one_identity.md) r1–r7. It is here because the synthesis settled that a bag is keyed by run id (§14) and said nothing about *who mints it*, which was a complete answer for the one invocation shape that existed when it was written. Two decisions taken since — the Temporal port's retry-and-replay semantics, and [Workflow Decomposition · Dual-mode children](../workflow-decomposition/phase3_dual_mode_children.md)'s nine standalone child runners — make the minting site load-bearing. Its evidence is not this component's pool: it is [`temporal-integration/research/synthesis.md`](../temporal-integration/research/synthesis.md) § *"The restart-recovery contract's prior findings still apply unchanged"*, from `raw/durable_dispatch_identity.md` §5.2d and §6 item 1, which surveyed six systems that name a unit of work and found none that mints the name inside the work. **Cited by section title rather than by number** — that synthesis is rewritten whenever its pool is rescoped and the numbering does not survive; see [one run, one identity, one bag](phase9_one_run_one_identity.md) § *Why an identity minted inside the work is the defect*.

- **A failed journal write is never silent** — carried by [the emit rule](phase3_the_emit_rule.md) r4, and honoured by [the journal root and the run bag](phase1_the_run_bag.md) (**r8** for the `incomplete` state, **r9** for the refuse-to-start case), [rebuildability is a test](phase4_rebuild_is_a_test.md) (r7), [snapshots, then retention](phase5_snapshots_then_retention.md) (r9) and [CPI reads the journal](phase6_cpi_reads_the_journal.md) (r6). It is here because [rebuildability is a test](phase4_rebuild_is_a_test.md)'s guarantee is meaningless without it — a journal with silent gaps cannot rebuild anything — and the synthesis does not address it.
- **What goes in is filtered at capture, with redaction as the single stated exception to immutability** — [the emit rule](phase3_the_emit_rule.md) r10 and [the journal root and the run bag](phase1_the_run_bag.md) r6. It is here because the component is otherwise **strictly weaker than what exists today**: `.claude/logs/` is equally unfiltered but is machine-local and deletable with no consequence, and after [rebuildability is a test](phase4_rebuild_is_a_test.md) the journal is neither.
- **The protocol is built as activities with an enumerating test behind them, not as a library** — [the journal root and the run bag](phase1_the_run_bag.md) r11, [the content store](phase2_content_store.md) r8, [the emit rule](phase3_the_emit_rule.md) r12, [the model-issued harvest](phase10_the_model_issued_harvest.md) r7. It is here because a protocol each workflow has to remember to call is optional, and § *Why the protocol is built as activities* names the three controls this fleet has already lost that way.
...
- **[Temporal Integration](../temporal-integration/temporal-integration.md)** — two separate items, and conflating them is what gated this component's consumer behind a server at draft:
  ...
  - *Port `review-runs`* gates **[CPI reads the journal](phase6_cpi_reads_the_journal.md)**. The evidence sweep exists today only as `scripts/workflows/review-runs.sh` in the frozen bash fleet, which may not be modified.
- **[Workflow Decomposition](../workflow-decomposition/roadmap.md)** — **a two-way dependency that neither roadmap carried until 2026-08-19**, and the traffic runs mostly toward us:
  - **[WD · What configuration a run absorbed](../workflow-decomposition/phase5_configuration_a_run_absorbed.md) depends on [the journal root and the run bag](phase1_the_run_bag.md)** — it writes a sixth `Journal-` tag into the bag. **That dependency is satisfied**, and WD's own roadmap says so. The extension rule for the tag namespace it writes into **is stated — [the emit rule](phase3_the_emit_rule.md) r13 — but [the emit rule](phase3_the_emit_rule.md) is not built**, so nothing enforces it yet. **What remains genuinely open is narrower than the rule**: r13 clause (c) deliberately does not rule whether bag metadata carries a version of its own, distinct from the per-event version, because the fleet declares one version field today and [the journal root and the run bag](phase1_the_run_bag.md) describes it two different ways. See § *What a SIBLING COMPONENT requires*.
  - **[WD · Dual-mode children](../workflow-decomposition/phase3_dual_mode_children.md) collides with [one run, one identity, one bag](phase9_one_run_one_identity.md)** — nine new entrypoints in the swept directory. Neither gates the other; whichever lands first sets the contract.
  - **[WD · Nothing a run relies on is invisible](../workflow-decomposition/phase4_nothing_invisible.md) extends a gate over our producers** while our consumer is gated. A scheduling input, recorded above.
- **[`C-2xgap6o1`](../../../tracked/candidates/C-2xgap6o1.md)** — deployment automation is a dependency of synthesis §13's redeploy path, sized as its own sprint, and explicitly not in this component.
...
|---|---|---|
| **A machine identity** that is stable across credential rotation, never derived from a key, and present on every event | The port addresses workers, task queues and schedules, and has its own reasons to name a machine. `temporal-integration.md` names no machine or edge id today, so nothing has been decided in either direction | commitment 7; [the emit rule](phase3_the_emit_rule.md) r6 |
| **Atomic create-if-not-exists on the bag** — mutual exclusion between two simultaneous openers, the one property the superseded two-tier state store had that the bag does not | The port retired that store's proposal, so it owns whether the property is still needed: if its workflow id **is** the run id, the orchestrator supplies exclusion and we build nothing. **A supersession that leaves the property unowned is what its own text warns against** | [One run, one identity, one bag](phase9_one_run_one_identity.md) r7, which stays unchecked until the addendum has a section that can hold the ruling ([`C-zhdm5gh1`](../../../tracked/candidates/C-zhdm5gh1.md)) |
| **Event identity that survives an at-least-once retry**, so a retried activity cannot double-append | Idempotency under retry is the port's own rule ([Temporal Standard](../../standards/temporal/temporal_standard.md) §7.1), and the journal is a side effect it has to hold for | [The emit rule](phase3_the_emit_rule.md) r7(a) |
| **A RUN identity supplied by the caller** — stable across the whole run, reproduced by a retry and by a replay, and not generated inside the work | The port already names a dispatch (workflow id plus run id, with two orthogonal reuse policies), and the answer is either that name or a second one that has to be joined to it. **Its reliability pool has already ruled that generation moves to an activity input before Stage B**, so this is the port's own finding rather than a request from here | commitment 1; **[One run, one identity, one bag](phase9_one_run_one_identity.md) r2** |
...
| **A storage interface** the evidence sweep reads through, so pointing it at object storage is a change of input | The sweep becomes a ported workflow, and its I/O boundary is the port's activity boundary | [CPI reads the journal](phase6_cpi_reads_the_journal.md) r7 |
...
1. **What a machine actually is** ([the journal root and the run bag](phase1_the_run_bag.md)). Home-directory placement suits the machine we have because Claude Code itself requires a user context. A machine that is not a full Linux environment — HAOS is the live example — may have no user account, and may need a sidecar to run at all. **This does not block the build**: [the journal root and the run bag](phase1_the_run_bag.md)'s first requirement is that the root is a config value and nothing depends on a home directory. The definition is the unbuildable half.
...
3. **Journal format at this volume.** A genuine research question and the strongest remaining candidate for a full research cycle. Placed as [`C-l2doi8ox`](../../../tracked/candidates/C-l2doi8ox.md).
4. **Whether the port's dispatch name can BE the run id, or has to be joined to it** ([one run, one identity, one bag](phase9_one_run_one_identity.md)). The three properties this component needs are stated — supplied by the caller, stable across the whole run, mapped onto whatever the orchestrator already calls a dispatch — and the third decides whether the phase is a parameter change or a migration. **It is not answerable from here**: it needs the port's own naming decision, and that component has its own criteria. [One run, one identity, one bag](phase9_one_run_one_identity.md)'s requirement 2 stays unchecked until it exists.
...
**Surfaced, never written — the documents below are human-in-the-loop under [`standards-governance.md`](../../../config/rules/standards-governance.md).** **One amendment this component depends on is NOT listed below and is tracked elsewhere on purpose:** the widening of [the local Temporal addendum](../../standards/temporal/claude-dot-files-addendum.md) §A3 to carry the run-identity ruling — [one run, one identity, one bag](phase9_one_run_one_identity.md) r2 and r7 both refer readers there, and §A3 as it stands is scoped to machine-axis queue naming. It is queued as [`C-zhdm5gh1`](../../../tracked/candidates/C-zhdm5gh1.md) rather than restated here, because the addendum is [Temporal Integration](../temporal-integration/temporal-integration.md)'s standards surface rather than this component's. Recorded so that a reader following either referral can find out that the widening is already filed instead of re-filing it. Every entry names when it lands, which is what keeps this list from becoming a carried-work ledger. **This roadmap is the writer for these entries.**
...
2. **[`memory-model.md`](../../guide/memory-model.md) §2.4 needs an authority note once the file surfaces become things a journal rebuilds.** It checks [`tracked/candidates/`](../../../tracked/candidates/) against §1's five properties and describes it as where the answer lives. *(Was §2.4 and §2.5 over the two pre-migration file stores; they merged when the stores did, 2026-08-26. The obligation is unchanged.)* After [rebuildability is a test](phase4_rebuild_is_a_test.md), **a hand edit that does not also emit is reverted by the next rebuild** — and the highest-value content in the store is hand-written by the operator. **Trigger: [rebuildability is a test](phase4_rebuild_is_a_test.md) landing.** **Consequence if it never lands:** someone edits an item, watches the edit disappear, and concludes the tool is broken. [Rebuildability is a test](phase4_rebuild_is_a_test.md) r6 is that phase's own statement of this and it explicitly cannot satisfy itself — the warning has to live where readers of those files are.

3. **[`exit-protocol.md`](../../standards/exit-protocol.md) §2 and §2.5 should say they govern the ROUTING CHANNEL, not every typed record the fleet emits.** §2 states *"No field is added on behalf of a consumer that does not exist"* and §2.5 bounds the envelope's fixed part at **4096 bytes**. Both are right for a channel a parent reads in code within seconds. **Read as governing any typed record, they forbid the durable one**, and the argument rests on the two clauses that actually bite rather than on a byte comparison: **§2's no-speculative-fields rule rejects all six** of the journal event's fields, unambiguously, because no parent branches on any of them; and **§2.5's bound is on the fixed part *and explicitly exempts `findings[]` "because it carries two short fields per finding and no prose"** — which is precisely the clause that excludes a record carrying authored content verbatim. *(Resting this on "39,772 bytes exceeds 4096" would be rebuttable at ruling time, since §2.5 already exempts the growing part; the no-prose clause is the one that holds.)* [The emit rule](phase3_the_emit_rule.md) r3 resolves this on the protocol's side — *one vocabulary, two contracts* — and the standard's side is this amendment.

**This amendment also carries two stale rollout pointers in the same file**, because they now point into a retired component: §3's `artifact` marker and §1's cross-reference both name that component's phase docs as the carrier of an obligation. **Re-home both onto this component's [the emit rule](phase3_the_emit_rule.md) checklist under the standard's own existing trigger** — *the first parent other than `review-pr` that routes on a typed record* — so an obligation with a live trigger stops being carried by documents nobody opens. **Trigger: that draft's ratification** — it binds nothing today, so there is nothing to fix yet, only something to fix before it binds. **Consequence if it lands unscoped:** the next durable record is designed against a 4 KB routing bound, or the rule is quietly ignored and stops meaning anything for the channel it *is* right for.

4. **[`memory-model.md`](../../guide/memory-model.md)'s run-log surface is superseded by the journal, and the document still describes it as current.** [The journal root and the run bag](phase1_the_run_bag.md) § *The surface this replaces* ruled the open question that phase carried: **the journal is the run log's new home**, not a second surface beside it. `run_log.py`'s three member event types, its join key and its `PUBLISHABLE_FIELDS` classification are carried into the journal's vocabulary rather than discarded — but the join key **changes meaning**, from per-model-invocation to per-run, which is the substantive half of the amendment and not a rename. **The existing `.claude/logs/` archive is deliberately not migrated** (it is gitignored, it is a moving denominator, and it holds no identifier addressing a whole run — a migration would have to synthesise records, which is the one thing a journal must never do). **Trigger: [the emit rule](phase3_the_emit_rule.md) landing**, which is when the cut-over actually happens; before that both surfaces exist and only one is written to, so nothing in the document is yet wrong. **Consequence if it never lands:** the operating manual documents a surface the fleet has stopped writing to, and the next author to add an observable adds it to the dead one.

5. **RETIRED 2026-08-28 — already discharged by a rewrite of the target, and the entry is kept with its reasoning rather than deleted.** The claim this entry was addressed to is **no longer in [`problem-statement.md`](../../standards/architecture/problem-statement.md)**. Element 3 today reads, verbatim at `:102`: *"**3. The first edge builds the others, then operates inside them.** See below. *Evidence: no trace of anything comparable.*"* — it carries no assertion about routing on values the model did not author, and `grep` for *"did not author"*, *"code-routed"* and *"non-model"* over the live file returns nothing. That document's own preamble records the rewrite pattern in the adjacent element (*"An earlier version of this claim said … **Research knocked that down.**"*), so the weakening this entry surfaced was absorbed by the document rather than left standing. **Nothing addresses the operator here any longer**, and there is nothing left to dispose of. *The original entry, verbatim:* **`docs/standards/architecture/problem-statement.md` element 3 is weakened by evidence in the product pool, and the ruling is the operator's.** The N6 gap and its *re-cut (a)/(b)* framing are [`code_routed_control_flow.md`](../../standards/architecture/research/raw/code_routed_control_flow.md)'s, not this component's pool's — **cited explicitly here because an operator disposing of this item otherwise has no traceable source.** The claim: that pool closes N6 *from outside the agent corpus*, which retires re-cut (b) ("routing on values the model did not author") as a novelty claim and leaves re-cut (a) — cross-process, cross-run, disjoint-context — as the only reading still uncovered by the located literature. It touches a product-altitude document this component does not own.

   *The original trigger, preserved with the entry above and no longer live:* **Trigger: none; it lands when the operator disposes of it.**

6. **The vendored [Testing Standard](../../standards/testing/README.md)'s applicability table still classes the integration tier as *"not yet"*, one directory from where it now exists.** Its row reads *"Service / integration tiers | **not yet** | No service runs here. `run-all.sh` reports these as `SKIP (no tests/integration)`"* — true when vendored, false since [the journal root and the run bag](phase1_the_run_bag.md) added `scripts/workflows/temporal/tests/integration/`. **The amendment carries a second clause, and it is the one worth the operator's time:** the tier that now exists is `skipif`-gated on machine-local state, so it reports **PASS having asserted nothing** on a clean runner — the category-present-but-nothing-ran case, which `run-all.sh` cannot tell from a real pass because no pytest exit code carries an executed-test count. That is the same gap [`C-abieu0fg`](../../../tracked/candidates/C-abieu0fg.md) already describes for `mutate.sh`, reached through a second tool, so the standard should say what an integration test is and is not evidence for rather than merely that the tier exists. **Trigger: none — a ruling to take.** It is vendored, so it is amended upstream in MDC-Master-Planning and re-vendored, never edited here. **Consequence if it never lands:** the binding document tells the next author the tier does not exist while the tree has one, and the interim answer — [`testing/README.md`](../../../testing/README.md), amended in PR #99 — is repo-local operational doc that does not bind.

7. **[`workflow-scripts.md` § *Location*](../../standards/workflow-scripts.md) has no row for a fleet-wide, activity-only module, and this component is about to land three of them.** The standard's target layout says `{module}/  ONE PER EDGE — assistant/, home_automation/, robotics/, …` with `modules/common/` for *workflows* owned by no edge. `modules/journal/` is neither: it is not an edge, and it holds no workflow — so it opens a third, unruled meaning for a child of `modules/`. [The content store](phase2_content_store.md)'s content store and [the emit rule](phase3_the_emit_rule.md)'s emitters are the next two, and each will land the same way by precedent, at which point *"a `modules/` child names a domain"* stops discriminating and a reader cannot tell an edge from a capability without opening files. **Two readings are available and the operator picks one:** amend § *Location* to admit `modules/<capability>/` for a fleet-wide activity-only package, or relocate to `modules/common/journal/` and widen `common/`'s definition beyond workflows. **Trigger: [the content store](phase2_content_store.md) starting**, which is when the precedent stops being a single case. **Consequence if it never lands:** the layout rule quietly becomes descriptive instead of binding, and the ruling gets made by whichever dispatch happens to land [the content store](phase2_content_store.md). *(Surfaced by the standards-auditor on PR #99 and deliberately not acted on there — eleven entrypoint imports and a pinned sweep predicate ride on the current path, so moving the package on an unruled reading is the expensive half.)*
...
- **A database.** `state_passing` §4.3.3's format table has one empty row — *queries over accumulated history* — and the reflex is to fill it with SQLite. A per-run folder tree with a checksum manifest answers the questions we actually have. And a database would be something the journal rebuilds, which [rebuildability is a test](phase4_rebuild_is_a_test.md) already makes cheap — so this is a future build opportunity with no rework cost. Revisit on a real query, not on a feeling that a record ought to live in a database. [CPI reads the journal](phase6_cpi_reads_the_journal.md)'s measurement is that trigger's first evidence.
