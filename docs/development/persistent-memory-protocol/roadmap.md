...
**And a class's substrate may move without the class dissolving.** [Phase 1](phase1_the_run_bag.md) r7 has an open ruling on whether the journal absorbs the per-run log; if it does, measurement samples get a new home and a new clearing rule while remaining what they are — a number that means nothing alone. That ruling names which class each absorbed artifact belongs to, so the taxonomy survives the move.
...
**Four phases have no EXTERNAL gate** and could start today: 1 (the bag), 2 (the content store), 3 (the emit rule), 4 (the rebuild test). They depend only on each other — 1 → 2, and 1 → 3 → 4, as each phase doc's own header says, so *ungated* means *waiting on nothing outside this component* rather than *runnable in parallel*. Together they deliver commitments 1 through 5 — which is the component's whole thesis, standing on its own.
...
- [ ] The bag's state is reported as `open`/`sealed` plus two independent flags — **`redacted`** and **`incomplete`** — and the validator always reports all three fields
...
- [ ] **Opening the bag is an activity a parent invokes as its first step, and a test enumerates every parent and fails when one does not** — the activity boundary alone does not make the call happen
...
The other three rows have no equivalent open artifact yet; each is settled in whichever of the two components is being built at the time, and the other cites it rather than restating it.
