...
**It does not own:** the Temporal port ([`temporal-integration/`](../temporal-integration/temporal-integration.md)), the fleet's deployment automation ([`C-2xgap6o1`](../../../tracked/candidates/)), or **managed configuration and the digest of what a run absorbed** — that is [Workflow Decomposition Phase 5](../workflow-decomposition/phase5_configuration_a_run_absorbed.md)'s in full, by operator ruling on 2026-08-19.
...
**This protocol is its own component, not a phase of anything, by operator ruling on 2026-08-16** — the framework it would have been a phase of was retired into it. [`C-xh8nvwqn`](../../../tracked/candidates/) asks the question; its `decision` is the triager's to set and this is the evidence.
...
- **[Workflow Decomposition](../workflow-decomposition/roadmap.md)** — **a two-way dependency that neither roadmap carried until 2026-08-19**, and the traffic runs mostly toward us:
  - **[WD Phase 5](../workflow-decomposition/phase5_configuration_a_run_absorbed.md) depends on [Phase 1](phase1_the_run_bag.md)** — it writes a sixth `Journal-` tag into the bag. **That dependency is satisfied**, and WD's own roadmap says so. The extension rule for the tag namespace it writes into **is stated — [Phase 3](phase3_the_emit_rule.md) r13 — but Phase 3 is not built**, so nothing enforces it yet. **What remains genuinely open is narrower than the rule**: r13 clause (c) deliberately does not rule whether bag metadata carries a version of its own, distinct from the per-event version, because the fleet declares one version field today and Phase 1 describes it two different ways. See § *What a SIBLING COMPONENT requires*.
...
- **[`C-2xgap6o1`](../../../tracked/candidates/)** — deployment automation is a dependency of synthesis §13's redeploy path, sized as its own sprint, and explicitly not in this component.
...
3. **Journal format at this volume.** A genuine research question and the strongest remaining candidate for a full research cycle. Placed as [`C-l2doi8ox`](../../../tracked/candidates/).
...
2. **[`memory-model.md`](../../guide/memory-model.md) §2.4 needs an authority note once the file surfaces become things a journal rebuilds.** It checks [`tracked/candidates/`](../../../tracked/candidates/) against §1's five properties and describes it as where the answer lives. *(Was §2.4 and §2.5 over the two pre-migration file stores; they merged when the stores did, 2026-08-26. The obligation is unchanged.)* After [Phase 4](phase4_rebuild_is_a_test.md), **a hand edit that does not also emit is reverted by the next rebuild** — and the highest-value content in the store is hand-written by the operator. **Trigger: Phase 4 landing.** **Consequence if it never lands:** someone edits an item, watches the edit disappear, and concludes the tool is broken. [Phase 4](phase4_rebuild_is_a_test.md) r6 is that phase's own statement of this and it explicitly cannot satisfy itself — the warning has to live where readers of those files are.
