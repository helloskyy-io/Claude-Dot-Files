...
|---|---|---|---|
| **invocation state** | the child process exits | everything a parent and a child pass each other in flight — the prompt, the worktree path and its contents, the completion line, and **the typed exit record**, which is the one contracted member | in the process, and on the exit-record channel |
...
**Those four cover all eight boundary-crossing channels** — invocation state takes rows 1–5 of the enumeration, the working record takes rows 7 and 8, measurement samples take row 6, and the journal is the ninth thing, which is what this component builds. The count is `state_passing` §2.1's own and is re-derivable from its table.
...
**The origin of the interface, cited rather than deferred to.** The five properties a durable record has to satisfy — durable, readable by humans and later runs, carries the outcome *and* its reasoning, has a to-do bit, retrievable by address rather than by replay — were first written down in [`memory-model.md`](../../guide/memory-model.md) §1, and that document remains the operating manual for the working record's five surfaces. **This roadmap is the writer for the taxonomy itself**; where the two disagree, this is the one that changed and the amendment is listed in § *Standards-amendment candidates* below.
...
**It does not own:** the Temporal port ([`temporal-integration/`](../temporal-integration/temporal-integration.md)), or the fleet's deployment automation ([`C-076`](../../standards/architecture/research/candidates.md)).
...
**Nothing in this component points at the retired framework as a live authority.** Its documents are left exactly as they are, as the record of what was built and why.
...
The journal has **one size limit: 1 GB by default, and it is a configuration value an operator changes without editing code.** It governs the whole journal. **Nothing is exempt from it** — there is no half kept forever and no reserved floor. When the journal is over budget, a retention pass removes **whole run folders, oldest first, until it is back under**, and never past the most recent snapshot.
...
So the structure carries it instead. [Phase 1](phase1_the_run_bag.md) r11, [Phase 2](phase2_content_store.md) r8 and [Phase 3](phase3_the_emit_rule.md) r12 each state this for the surface they own: the journal root is resolved and the bag opened by an activity that runs before any child does, and an emit is an activity rather than a call a prompt asks a model to remember. A workflow that skips it does not get a run with no journal — it gets a run that does not start ([Phase 1](phase1_the_run_bag.md) r9).

**What this does NOT cover, stated so the guard is not over-read.** It makes the journal *structurally present*; it does not make any individual write *complete*. A parent that opens a bag and then writes to a store through a path nobody wrapped still produces a gap — and nothing about the activity boundary catches that. **[Phase 4](phase4_rebuild_is_a_test.md)'s rebuild test is the guard for that class, and it is a different guard**; the two are complementary and neither substitutes for the other. It also does not reach model-issued writes at all, which are [Phase 3](phase3_the_emit_rule.md)'s post-exit harvest and are structurally outside any wrapper.
...
|---|---|---|
| [Phase 7](phase7_s3_aggregation.md) § *Where a shared bucket would change things* | every event carries a provenance class and a credential epoch, so a reader can say where content came from | [Phase 3](phase3_the_emit_rule.md) r7 |
...
- [ ] The bag has four lifecycle states — open, sealed, redacted, **incomplete** — and the validator reports each distinctly
- [ ] The payload spec is stated as a table with a reason per row — authored output, transcript and execution facts in; code diffs out (commit SHA); Temporal history out (it expires) — **and every row carries a per-field classification slot**
...
- [ ] **Opening the bag is an activity a parent invokes as its first step**, so a run cannot execute without a journal
...
- [ ] A recurring retention pass runs on a cadence, and writes a snapshot when it needs a newer barrier to delete against
- [ ] A deletion dry-run **refuses** to cross the last snapshot, demonstrated against a real journal
- [ ] The budget is one configuration value, 1 GB by default, governing the whole journal — **no half exempt, no reserved floor, and no second number**
- [ ] Deletion is **whole run folders, oldest first, until under budget** — never a partial run
- [ ] A rebuild from the last snapshot forward still passes Phase 4's test after a deletion pass
- [ ] A retention pass emits its own journal events — folded into the snapshot, so the explanation of a deletion survives the next one — and a bag that is both incomplete and redacted stays distinguishable as both
...
| 8 | The accumulated log is an asset; deleting is small, planned work | **5** (r1–r5) |
...
| 12 | Temporal's own store is telemetry, not memory | 1 (r7, contents table), 5 (r3, the budget) |
...
**Three concepts are not from the synthesis and are added by this plan:**

- **A failed journal write is never silent** — carried by [Phase 3](phase3_the_emit_rule.md) r3, and honoured by Phase 1 (**r5** for the `incomplete` state, **r7** for the refuse-to-start case), Phase 4 (r7), Phase 5 (r6) and Phase 6 (r4). It is here because Phase 4's guarantee is meaningless without it — a journal with silent gaps cannot rebuild anything — and the synthesis does not address it.
...
- **The protocol is built as activities, not as a library** — [Phase 1](phase1_the_run_bag.md) r11, [Phase 2](phase2_content_store.md) r8, [Phase 3](phase3_the_emit_rule.md) r12. It is here because a protocol each workflow has to remember to call is optional, and § *Why the protocol is built as activities* names the three controls this fleet has already lost that way.
...
**Where the two disagree, neither wins by default.** This plan states what it needs and why; the port states what it needs and why; the shape that satisfies both is settled once, in whichever of the two is being built at the time, and the other cites it.
