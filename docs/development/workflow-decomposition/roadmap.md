...
**Revised on 2026-08-27 by a follow-up design pass over the three unbuilt phases — 3, 4 and 5 — and the phase list did not change.** What changed in the interval is that the **four tracked stores** landed on 2026-08-26 (`tracked/issues/`, `tracked/operations/`, `tracked/candidates/`, `tracked/standards/`, under [Tracked Items Standard](../../standards/documentation/tracked_items_standard.md)), retiring GitHub Issues for tracked work and adding INTAKE as a third way a run reaches a surface. **Three things follow, one per phase**, and each is written up in that phase's own doc:

- **[Phase 3](phase3_dual_mode_children.md) gained a requirement it should always have carried** — [Phase 2](phase2_family_alignment.md)'s blind trial measured its per-pair ruling procedure at κ = 0.000 and moved ruling to per-FAMILY granularity. `sprint.md` records that Phase 3 inherits that as an open question and Phase 3's doc never said so. Nine adapters from one template are one family, ruled once, before they are written.
- **[Phase 4](phase4_nothing_invisible.md)'s weakest half got a first-party definition and a named population.** Its central deliverable — *what is a producer* — no longer has to be invented: [Tracked Items Standard §0](../../standards/documentation/tracked_items_standard.md) states the property and `modules/assistant/tracked/intake.py` is the worked instance. And `scripts/helpers/` holds six tools outside `measure/` that no gate reaches, one of which is a stated *condition* of an exemption.
- **[Phase 5](phase5_configuration_a_run_absorbed.md)'s deferred successor was placed.** The managed/user tier it defers is now [`C-mq7v3z8k`](../../../tracked/candidates/C-mq7v3z8k.md) rather than a sentence in a document that will be marked complete.

**No checkbox below was reworded and none was removed** — the same rule as 2026-08-19. One unchecked box was added, to [Phase 3](phase3_dual_mode_children.md). **Measurements and rulings dated August 2026 that cite surfaces since deleted are left standing**: they describe what was true when written and are records, not defects.
...
**Est: ~30 hours** *(sized cold by `plan-verify`, 2026-08-27 — the first pass to size it, and the lowest-confidence figure here)* — it is the one phase with no doc, so it is sized off its five boxes and the tree they produced rather than off a scoped requirement set. What it contained was a structural refactor of the fleet's largest family while that family was running: two monoliths became the six workflow modules under `modules/assistant/build/` (1,273 lines on disk today), `build-phase` was absorbed as a flag rather than kept as a seventh module, the activities layer was lifted out into what is now `assistant_activities.py`, and the shape was written into [`workflow-scripts.md` § Composition](../../standards/workflow-scripts.md).

**Why it carries a figure at all, reversing what this line used to say.** It read *"No estimate, and the absence is deliberate — this phase shipped; its boxes are a record of what was built rather than work to be sized."* **That reason applies word-for-word to [Phase 2](phase2_family_alignment.md)**, which also shipped and which the same pass sized — so the stated rule was already not the rule being followed, and a reader could take either half. A completed phase's cost is part of what the component cost; it is the **to-do** figure that subtracts a complete phase, and `plan-sprint` derives that separately from the total.

Every phase here carries one figure. Each is **hours of focused development, not elapsed time**, and each states what it rests on so it can be argued with. **No total is written here** — a total is derived from the parts, and a derived figure restated where nothing derives it goes stale in one direction only.
...
**Est: ~20 hours** *(re-sized cold by `plan-verify`, 2026-08-27, against what SHIPPED rather than against what was scoped; the 2026-08-19 pass read ~15)* — still almost none of it code the phase had to invent, since the mechanism, the ratchet and the standard's wording all pre-dated it. The cost is the judgement pass over the frozen baseline's thirteen rows, grouped into eight consumer-sets, each needing two prompt files read plus the git history behind them; the blind trial, sealed before any history was consulted and then scored against a co-evolution audit, whose write-up runs to twenty kilobytes and returned two structural findings that were not the question asked; and the `_minor` tier contract. **The contract is why the figure moved:** the earlier note sized it as *"one document that does not exist anywhere today"*, and what shipped is not a document — it is `TIER_INVARIANT` / `TIER_SCOPED` and a ruling validator in the 394-line `tests/unit/fork_vs_parameterize.py`, which rejects a ruling that names no category, and which gained a second consumer in the render guard.
...
**Est: ~26 hours** *(re-sized cold by `plan-verify`, 2026-08-27; the 2026-08-19 pass read ~20)* — the adapters are the cheap half (~12h) and the demonstrations are the expensive one (~8h). Nine runners written against eleven existing pairs, the five divergences ruled once rather than nine times, a 53-line naming guard widened from eleven subjects to twenty, and each of the nine then run alone until it behaves — this phase's own evidence is that a child takes several fix rounds, which is why the proof outweighs the construction. **Three things moved the figure, and none of them is the adapter count.** The measured runner range in the previous note — *"88–172 lines"* — is stale: re-counted 2026-08-27 the eleven runners span **116–372 lines** (median ~135, shims still 13–18), because `plan_verify` and `triage_candidates` landed in the interval. The requirement-6 family ruling is a box this phase gained on 2026-08-27, after that sizing was written. And ruling the nine as one family costs a design decision rather than a lookup — see § *What this phase inherits from Phase 2's blind trial*, whose 2026-08-27 note records that [Phase 2](phase2_family_alignment.md)'s apparatus does not reach a Python runner.
...
*The last box is new on 2026-08-27 and is the only box this component gained in that pass.* It carries what [`sprint.md`](../sprint.md) already said Phase 3 inherits from [Phase 2](phase2_family_alignment.md) — a ruling procedure demoted to advisory after scoring κ = 0.000 against the field's 0.271 benchmark — and which this phase's doc did not name. It bears here because this phase's own notes call nine adapters written from one template *"a copying event waiting to happen"*, and ruling that population afterwards is ruling at chance. Detail and the limits of the measurement: [Phase 3](phase3_dual_mode_children.md) § *What this phase inherits from Phase 2's blind trial*. **The counts the phase is scoped on — twenty workflows, eleven pairs, nine without — were re-verified on disk on 2026-08-27 and are unchanged.**
...
**Est: ~30 hours** *(re-sized cold by `plan-verify`, 2026-08-27; the 2026-08-19 pass read ~24)* — the largest phase here, and the two halves are not alike: **derived ~14h, produced ~16h**, split out because [`C-v4k9pz2h`](../../../tracked/candidates/C-v4k9pz2h.md) proposes ruling them apart and a split is easier to rule on with a figure against each half. The derived half finishes three properties on code that already anchors on a marker and already takes an override; it is bounded by the number of derivation sites, and its one hard item is the check that DERIVES that population from the tree rather than reading a list. The produced half is bounded by nothing yet: it has to be ruled across the whole fleet before a line of gate code is safe, and while the 2026-08-27 pass gave its definition a ratified anchor, a better-specified deliverable is a larger one rather than a smaller one. **The produced half is what makes this phase large, and it is the half that could be its own.**
...
*This phase is a merge of two that were planned separately on 2026-08-18 — a derivation audit and a producer/consumer gate. They share one shape and neither carried enough work to be its own document.*

***Revised 2026-08-27.*** *The definition in the fifth box still does not exist as a check, and that box stays unchecked — but it no longer has to be invented. The tracked stores shipped the same property, ratified: [Tracked Items Standard §0](../../standards/documentation/tracked_items_standard.md) requires every store to have an admission test, a triage cadence with a named runner, and an exit, and `modules/assistant/tracked/intake.py` states the three conditions in code as the whole of an exemption it depends on. The requirement is now* rule it against the fleet, *not* write it from scratch. *The **extend-the-gate** box gained a named first population too, replacing a direction: six tools in `scripts/helpers/` sit outside `measure/` and are in no gate today, and the intake→harvest pair's reader is invoked only by a line of prose in a command file — the exact shape this half already ruled insufficient. **`tracked/operations/` is the one surface that must be excluded by name**: its consumer is a human, and no check can assert that a person read something.*
...
*Two notes added 2026-08-27.* **The tier half of the first box now has a home that outlives this phase** — [`C-mq7v3z8k`](../../../tracked/candidates/C-mq7v3z8k.md), filed against this component. The deferral itself is unchanged and is not reopened; what changed is that a successor named only in the prose of a phase about to be marked complete stops existing when the box is ticked, and there was no store to place it in when this was written. **And the last box's ANSWER now has a destination too**, one per outcome — see [Phase 5](phase5_configuration_a_run_absorbed.md) § *Requirements for completion*. **The facts this phase is scoped on were re-read on disk 2026-08-27 and are unchanged: five `Journal-` tags, seven installer symlink targets.**
...
- **The managed/user configuration tier mechanism itself — deferred, not declined, and PLACED at [`C-mq7v3z8k`](../../../tracked/candidates/C-mq7v3z8k.md).** Managed configuration is this component's in full (operator ruling, 2026-08-19) and [Phase 5](phase5_configuration_a_run_absorbed.md) builds the record it needs. What is not built is the tier itself, because the precedence direction is a policy choice and the digest is what supplies the evidence for it. **The tracked item is what makes the deferral survive Phase 5 closing**; whether it returns as a sixth phase or a later sprint item is a triage call, not one this roadmap makes.
