...
The line this replaces read: *"No estimate, and the absence is deliberate. This phase shipped; its boxes are a record of what was built rather than work to be sized."* **That reason applied word-for-word to [Phase 2](phase2_family_alignment.md)**, which also shipped and which the same pass *did* size — so the stated rule was already not the rule being followed, and a reader could take either half. **A completed phase's cost is part of what the component cost**; it is the *to-do* figure that subtracts a complete phase, and that is derived separately from the total.
...
**Est: ~22 hours** *(sized cold by `plan-verify`, 2026-08-28 — the first figure written against this phase since the split; the one it replaces covered both halves and was removed rather than divided)* — the enumeration and the echo are both work on code that already runs, which is what keeps this below [Phase 3](phase3_dual_mode_children.md). Half a dozen derived values each need a marker, a one-sentence algorithm, an override and a scope of effect written for them; the echo has to move from the one mode where nothing is at stake onto the live path across eleven runners, without the rehearsal and the live run drifting into two assemblies; and the proof is one deliberately-wrong run captured verbatim.

> **The variance is all in one place, and it is requirement 1's population check.** That requirement asks for the set of derivation sites to be read off the tree *"the way [Phase 6](phase6_every_producer_names_its_consumer.md)'s gate reads its population off disk"* — but that gate's population is *files in a directory*, which has a syntactic definition, and **a derivation site has none in this tree today**: there is no decorator, no registry and no naming convention marking one. Establishing a marker before the enumeration can be checked against it is a change across the tree rather than an enumeration, and it is not in the figure above. **Sized on the assumption that a cheaper answer is found; if a marker convention is required, this phase is materially larger and should be re-sized before it is scheduled.**
...
**Est: ~21 hours** *(sized cold by `plan-verify`, 2026-08-28 — the first figure this phase has carried; the one covering both halves before the split was removed rather than divided)* — the definition and the ruling pass are the bulk of it, and neither is code. Requirement 2 has to be settled before a line is written; the definition then has to be checked backwards against the three observables that motivated the existing gate and against an intake whose harvest stopped being invoked; the seven tools in `scripts/helpers/` outside `measure/` get ruled in or out one at a time with a reason recorded for each; and the gate itself generalises from one directory with one named exclusion to several populations each read off disk. The proof is two mutations in opposite directions.
