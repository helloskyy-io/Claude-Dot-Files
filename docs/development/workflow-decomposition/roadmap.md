...
**Status: 🟡 IN PROGRESS.** Two phases are complete, four are `🟠 PLANNED`, and one — *Managed configuration, and whose tier wins* — is `🔵 NOT SCHEDULED` behind four named gates. **Five phases remain, but only four are workable**, and that distinction is the point of the two markers: `🔵 NOT SCHEDULED` means exactly *no plan exists*, so counting it as queue overstates what can be picked up. **Phases are listed in logical rollout order. Phase numbers are creation-order identifiers and do not reflect rollout sequence; execution order across components lives in [`sprint.md`](../sprint.md).**
...
**Est: ~34 hours** *(re-sized cold by `plan-verify`, 2026-08-29)* — the largest single unit of work here, and most of the distance from the phases below is the verification cost a refactor performed on a running family carries and a greenfield one does not. Measured against what it left behind: an activities layer of 1,972 lines and a build family of roughly 1,275 across six modules, lifted out of two monoliths while that family was serving daily dispatches.
...
**Est: ~28 hours** *(re-sized cold by `plan-verify`, 2026-08-29)* — the code half re-counted on disk at 2,141 lines of test surface across the ratchet and its two complements, against a shared-fragment pool of 35 blocks; the judgement half is 13 baseline rows in 8 consumer-sets, each needing two prompt files and their git history read, plus a blind trial sealed before any history is consulted and scored against a co-evolution audit.
...
**Est: ~26 hours** *(re-sized cold by `plan-verify`, 2026-08-29, and the figure did not move)* — one consolidation with a measured precedent (`base_ref` collapsed a value of the same shape at the same eleven call sites), one object, one echo. **Every population this phase is scoped on was re-counted on disk and still holds** — eleven name-assembly sites in three spellings, `run_build_minor` still naming its worktree `build-`, and `test_isolation_invariants.py` still keying on the literal `act.worktree_add(` that `review_pr_workflow.py` reaches under a different alias. The widest line is the tail, where the suite meets eleven changed call sites, a renamed `build-minor` worktree and a new unconditional stderr line at once.
...
*Every child runs standalone and under a parent, equally well.* **Seventeen workflow modules exist; eleven can be started by a person and six cannot** — `build_draft`, `build_draft_minor`, `build_refine`, `build_refine_minor`, `research_draft` and `research_verify` (re-counted on disk 2026-08-29: 17 `*_workflow.py` modules against 11 runner/shim pairs). The six are all children whose core function already works — what is missing is the outer half of the shape the other eleven have: a runner that owns the CLI contract, and a thin shim beside it. This phase rules them as one family, extracts the mechanism they share before writing any of them, builds them, widens the shim-naming guard to cover the fleet, and proves each child by running it alone.

**This paragraph said *nine* until 2026-08-29, and the checkboxes below still do.** The population fell because three research modules were deleted rather than because the phase narrowed: `research_refresh` and `research_refresh_parent` went when revalidation stopped being a workflow and became a row state, `research_write_minor` went with them, and `research_write` was renamed `research_draft`. **The prose is corrected here and the completion criteria are not**, because reconciling them is an operator ruling rather than a planning pass's call — see the note under the boxes.

**Est: ~28 hours** *(re-sized cold by `plan-verify`, 2026-08-29, and the figure MOVED DOWN because the population did)* — **this phase is scoped on nine children and only six of them still exist.** Re-counted on disk: seventeen workflow modules across the four families, eleven runner/shim pairs, so the arithmetic is 17 − 11 = 6. `research_write_minor`, `research_refresh` and `research_refresh_parent` were deleted from the tree after this phase was written, and `research_write` was renamed `research_draft`. **The estimate prices the six that exist; the phase text still names the nine, and reconciling it is a re-plan rather than a sizing note** — this pass reports it and does not rewrite the phase's requirements. The adapters are the cheap half against a re-verified 117–379 line, median-135 corpus, and the demonstrations are the expensive one, priced per child because this phase's own evidence is that a child takes several fix rounds before it behaves. **The figure prices requirement 5's demonstration at the bar requirement 5 itself states — *"the cheapest invocation that reaches real work"* — which [`phase3_dual_mode_children.md`](phase3_dual_mode_children.md) § *Runtime Verification* reads more cheaply as a `--dry-run`. That is the widest single line in this component, and it is unruled rather than settled.**
...
*Two boxes above are wrong as measured, and are carried verbatim because a planning run does not reword a completion criterion.* **"The box above"** in the second means the managed-config box, which now lives at [What configuration a run absorbed](phase5_configuration_a_run_absorbed.md). And its premise has now failed twice over. It was first measured false on 2026-08-19 — `research_refresh_parent` **was** invocable via `run_research.py --refresh`, so the real defect was narrower: no entrypoint **of its own**. Since then the module has been **deleted outright** (verified 2026-08-29: no `research_refresh*` module survives under `modules/assistant/research/`), so the box now names a subject that does not exist and **cannot be closed as written by any build.** Source: [`research/raw/invocation_contract.md`](research/raw/invocation_contract.md) §5.1 for the first measurement.

**Retiring that box, and reconciling the five that count nine or twenty against six children, is a SCOPE CHANGE and therefore an operator ruling — this planning pass deliberately did not make it.** Two things follow that a reader should not mistake for drift. The criteria above are carried verbatim per this file's own norm, so the entry states six in prose and nine in its boxes on purpose. And the count is stale throughout [`phase3_dual_mode_children.md`](phase3_dual_mode_children.md), which a planning pass cannot edit — **so the correction has to land in one ruling across both files, not in halves.** **This sentence said *seven further sites* until 2026-08-29 and the measured figure is 50 lines, spanning all seven of that document's sections** — swept exhaustively on the terms that carry the population (`nine`, `twenty`, `20 modules`, and the deleted module names). Five of the 50 sit in § *Runtime Verification* and are **dated observations to be superseded rather than rewritten**; the other 45 are live text, including the enumerated table in § *The nine, counted off the tree* — which names `research_write_minor`, `research_refresh` and `research_refresh_parent`, **all three since deleted**, plus `research_write`, since renamed `research_draft`. The six children that do exist are the ones named in this entry's own opening paragraph above; the phase doc does not yet name them anywhere. **The figure is corrected here because it is a measurement rather than a completion criterion**, and an operator scoping the ruling from *seven sites* would budget for a footnote and meet a rewrite of the phase's whole scope claim. The last two boxes are new on 2026-08-28; the ruling behind them and what was rejected are in [`phase3_dual_mode_children.md`](phase3_dual_mode_children.md) § *The runner corpus has no duplication guard*, filed as [`C-7q2m4xzb`](../../../tracked/candidates/C-7q2m4xzb.md).
...
**Est: ~18 hours** *(re-sized cold by `plan-verify`, 2026-08-29)* — still the smallest phase with a doc, because every surface it touches already exists and both were re-verified on disk: the bag writes exactly five `Journal-` tags in one call, and `SYMLINK_TARGETS` is still seven entries. The fiddly part is reading the digest's input set out of the installer's own bash array rather than copying it beside them, plus the check that fails when the two disagree. **The figure went up rather than down because requirement 5 is priced as what it is — a live-CLI measurement that has to declare a hook in a machine-wide managed tier and put it back**, which is a slower and more careful hour than the desk work around it.
...
**Est: ~24 hours** *(re-sized cold by `plan-verify`, 2026-08-29)* — the definition and the ruling pass are the bulk of it and neither is code; the seven tools in `scripts/helpers/` outside `measure/` were re-counted on disk and are still seven. **The variance driver is unchanged and is the largest in this component: a fleet-wide producer sweep nobody has counted**, so the figure assumes the ratchet-as-backlog shape this phase's own notes propose rather than a green-before-merge one, and it carries requirement 6's expensive branch rather than its empty one.
...
**Est: ~24 hours** *(re-sized cold by `plan-verify`, 2026-08-29, and the figure did not move)* — the coarsest figure here because there is no doc to read, and it prices what the work costs **when the gate opens** rather than claiming it can start. It buys two tiers resolved over the seven targets the installer links, `install.sh` reworked to place them, precedence demonstrated in whichever direction the operator rules, and the safety hook's guard shown still holding afterwards. **The research pass the fourth gate opens on is NOT in this figure** — it is a gate, not this phase's work, and nothing currently commissions it. **And the figure SWINGS ON the third gate, which is why it is one number and not a confident one:** if the operator rules precedence toward Claude Code's own Managed tier, most of the mechanism is vendor-supplied and the work collapses to configuration plus the demonstration; if they rule toward a user tier that can override, the resolver, the installer rework and a two-direction demonstration are all ours and the figure is materially larger. The number above sits between them because the ruling has not been made — a reader deciding whether to unblock this phase should read the gate first and the figure second.
...
**[Nothing a run relies on is invisible](phase4_nothing_invisible.md) sits before [Dual-mode children](phase3_dual_mode_children.md).** That phase adds six runners, and **six runners written before the run context exists are six new members of the eleven-site class the context is collapsing** — this fleet has measured what that costs, since `base_ref`'s docstring records the same consolidation at the same eleven runners, where the first hand sweep of eleven found ten. **The argument does not weaken as the population falls**, because it turns on new sites being added to a class mid-collapse rather than on how many. *Dual-mode children* is the *enabler* and used to sit first for that reason: a child with no standalone entrypoint can only be debugged at parent prices. **The enabler argument survives and is simply outweighed** — *Nothing a run relies on is invisible* needs nothing from the six, because its own proof runs against `plan-draft` (the workflow `plan-feature` was renamed to), which exists. **No phase number changed and no file moved**, which is what makes a re-sequencing cheap here.
