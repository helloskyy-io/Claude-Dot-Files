...
**Est: ~34 hours** *(re-sized cold by `plan-verify`, 2026-08-28)* — the adapters are the cheap half against a verified 116–372 line, median-135 corpus, and the demonstrations are the expensive one, priced per child because this phase's own evidence is that a child takes several fix rounds before it behaves.
