...
| **It does not survive, or the tier is not reachable** | the existing guard stands and one design route is closed | recorded in § Runtime Verification below, **and reported as a constraint on Phase 7's tier design** — the recurrence goes on [`C-mq7v3z8k`](../../../tracked/candidates/C-mq7v3z8k.md) rather than into a second item |
...
- [ ] Name the reader as the digest's consumer wherever [Phase 6](phase6_every_producer_names_its_consumer.md)'s gate expects it — this phase pairs its producer with its consumer deliberately, and that pairing should be visible rather than incidental.
- [ ] **Record, plainly, that this reader is invoked ON DEMAND and has no cadence and no termination** — that is the fact [Phase 6](phase6_every_producer_names_its_consumer.md) requirement 2 has to rule on, and stating it here is what turns this pair from an awkward discovery into that phase's worked test case. **Do not invent a schedule for the reader to make it look conformant.**
