# What configuration a run absorbed
...
The roadmap's own gate for this problem already names the cheap half of the answer: *if the run bag records the config a run used, the divergence half shrinks to a reader.* **That gate is open.** The run bag exists and carries five `Journal-` tags today — the workflow key, the origin repo, its remote, its commit, and the worktree — and **none of them names the configuration the run absorbed.** *(Re-read on disk 2026-08-28 at `modules/journal/journal_activities.py:342-346`: still exactly those five, written together in one call. The bag's own lifecycle labels — `Journal-Redaction`, `Journal-Incomplete`, `Journal-Gap`, `Journal-Sealed-At` — are a separate set declared in `modules/journal/bag.py` and are **not** part of the five this phase joins; a builder who counts nine and re-scopes has counted two different things together.)*
...
2. **What the digest covers is stated explicitly**, and what it deliberately excludes is stated by name. A digest whose inputs are unnamed cannot be reasoned about when two runs disagree, and cannot be recomputed by anyone checking it. **The input set is READ from the installer's `SYMLINK_TARGETS`, never copied beside it** — see § *The digest's inputs are the hard part*.
...
|---|---|---|
| **The Managed tier survives both flags** | a proposed change to how the fleet declares its safety hook | **surfaced in the PR body** as a standards amendment against the safety-layer invariant, with the target and anchor named — [`finding-routing.md` § 7](../../standards/finding-routing.md) gives a producing run the surfacing and `review-pr` the filing, and `ratification` is the operator's alone |
| **It does not survive, or the tier is not reachable** | the existing guard stands and one design route is closed | recorded in § Runtime Verification below, **and reported as a constraint on the tier design in *Managed configuration, and whose tier wins*** — the recurrence goes on [`C-mq7v3z8k`](../../../tracked/candidates/C-mq7v3z8k.md) rather than into a second item |
...
> **The successor now has a surface that outlives this phase, added 2026-08-28: it is *Managed configuration, and whose tier wins* in [`roadmap.md`](roadmap.md), with [`C-mq7v3z8k`](../../../tracked/candidates/C-mq7v3z8k.md) carrying its three sub-decisions.** The paragraph above was written on 2026-08-19, when the only place a deferral could live was the prose of the phase deferring it — the four tracked stores landed on 2026-08-26. **A successor named only in a document that will be marked COMPLETE is a successor that stops existing when the box is ticked**, and [`roadmap.md`](roadmap.md)'s own claim that *"when these phases close, decomposition is done"* is what made that silent. **The reasoning for the deferral is unchanged and is not reopened**; only its placement moved from prose to an entry with a named gate.
...
**This phase's stated intent is the first shape:** *the user keeps a tier they own and can extend.* A design that reaches for Claude Code's Managed tier because the word "managed" matches would silently adopt the second shape and **remove the very tier the checkbox promises the user.**
...
`~/.claude/` holds machine-local state — credentials, sessions, caches, per-project data — alongside the synced configuration. A digest over the whole tree changes on every run and answers nothing. A digest over the synced set answers the question, and **the synced set is already enumerated**: it is what the installer links, and there are seven of those targets. Requirement 2 exists so that whichever set is chosen is written down beside the tag, rather than being recoverable only by reading the function that computed it — which is the same property [Nothing a run relies on is invisible](phase4_nothing_invisible.md) is establishing for every other derived value in the fleet.

**READ the set from `install.sh`; do not copy it. Added 2026-08-28, and it is the [Architecture Standard](../../standards/architecture/architectural_standard.md)'s `derive ≠ declare` seam applied directly.** `SYMLINK_TARGETS` in [`install.sh`](../../../install.sh) is the array the installer actually links — re-verified 2026-08-28, still seven entries: `settings.json`, `CLAUDE.md`, `agents`, `commands`, `hooks`, `rules`, `skills`. **A digest built from a hand-copied list of seven is a hand-kept population**, which is precisely what [Every producer names its consumer](phase6_every_producer_names_its_consumer.md) requirement 3 exists to forbid: a list checked against itself cannot see the target that was added to the installer and never added to it. *(This sentence also cited [Nothing a run relies on is invisible](phase4_nothing_invisible.md) requirement 1 until 2026-08-29. **That citation was to a clause the operator's 2026-08-28 re-plan DELETED** — read-the-population-off-the-tree is gone from that phase, and its requirement 1 is now the frozen run context. The property still binds here; it is that phase's requirement 6 and this phase's own requirement 2 that carry it, not a deleted clause.)*

**This is not hypothetical, and there are two live proposals to change the set.** [`C-idwrru3n`](../../../tracked/candidates/C-idwrru3n.md) proposes per-file granularity in the installer's targets; [`C-7ymfdw28`](../../../tracked/candidates/C-7ymfdw28.md) proposes pointing the symlinks at a pinned worktree, and the two are constrained to be ruled together. **The day either lands, a copied seven makes the digest answer a question about a set nobody syncs — confidently, which is the failure mode this whole component is about.** Derive the set; if deriving it from a bash array is awkward, **that awkwardness is a finding about where the set is declared, not a licence to copy it.**
...
- [ ] **Derive that set by reading `SYMLINK_TARGETS` from [`install.sh`](../../../install.sh) at digest time**, and add the check that fails when the digest's population and the installer's disagree. A copied list is the hand-kept-population defect [Nothing a run relies on is invisible](phase4_nothing_invisible.md) forbids, and two open candidates propose changing the set.
...
- [ ] Name the reader as the digest's consumer wherever [Every producer names its consumer](phase6_every_producer_names_its_consumer.md)'s gate expects it — this phase pairs its producer with its consumer deliberately, and that pairing should be visible rather than incidental.
- [ ] **Record, plainly, that this reader is invoked ON DEMAND and has no cadence and no termination** — that is the fact [Every producer names its consumer](phase6_every_producer_names_its_consumer.md) requirement 2 has to rule on, and stating it here is what turns this pair from an awkward discovery into that phase's worked test case. **Do not invent a schedule for the reader to make it look conformant.**
...
- **This phase is the strongest argument for [Every producer names its consumer](phase6_every_producer_names_its_consumer.md), and it also dodges it** — it ships its consumer in the same phase as its producer. That is the correct shape and it is exactly why the gate is needed for the pairings that are not planned together. **Corrected 2026-08-28: this pairing is that phase's TEST CASE, not its exemplar.** Its candidate producer definition requires that the reader be invoked on a *named cadence* and that the surface *empty or terminate* — and this reader has neither: a person runs it when two runs are suspected to disagree, and bags accumulate and are never edited after sealing. **The sentence above was written against a weaker definition that asked only whether a reader was named, and was not re-checked when the definition gained the cadence clause.** Ruling that shape is [Every producer names its consumer](phase6_every_producer_names_its_consumer.md) requirement 2, and this phase deliberately rolls out first so the instance exists when the ruling is made. Source: [`C-k3nd8vwp`](../../../tracked/candidates/C-k3nd8vwp.md).
- **The tier half is deferred, and a deferral is PLACED rather than mentioned.** Its home is *Managed configuration, and whose tier wins* in [`roadmap.md`](roadmap.md), with [`C-mq7v3z8k`](../../../tracked/candidates/C-mq7v3z8k.md) carrying the detail — not this document, which will be marked complete. If the build dispatch finds new evidence bearing on the precedence direction, **report it as a recurrence against that item** rather than opening a second one or writing it into this phase doc.
