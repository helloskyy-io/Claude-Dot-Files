...
- **Inside it:** none. This phase does not need [Phase 3](phase3_nothing_invisible.md), [Phase 5](phase5_dual_mode_children.md) or [Phase 6](phase6_configuration_a_run_absorbed.md), and none of them needs it. **One-way note:** [Phase 5](phase5_dual_mode_children.md) writes nine new runners from one template in one sitting, which is the exact shape this phase's duplication ratchet exists to catch — so it must not hand that baseline nine new rows.
