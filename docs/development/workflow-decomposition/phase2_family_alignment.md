...
- **Inside it:** none. This phase does not need [Nothing a run relies on is invisible](phase4_nothing_invisible.md), [Dual-mode children](phase3_dual_mode_children.md) or [What configuration a run absorbed](phase5_configuration_a_run_absorbed.md), and none of them needs it. **One-way note:** [Dual-mode children](phase3_dual_mode_children.md) writes nine new runners from one template in one sitting, which is the exact shape this phase's duplication ratchet exists to catch — so it must not hand that baseline nine new rows.
