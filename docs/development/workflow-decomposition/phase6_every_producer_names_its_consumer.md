...
> **This phase was split out of [Nothing a run relies on is invisible](phase4_nothing_invisible.md) on 2026-08-28, and it is where that phase's *produced* half went.** It is not new work: requirements 1 and 3 below are carried verbatim from that phase's **pre-split requirements 4 and 5 (2026-08-18)**, and requirement 5 carries the second clause of its **pre-split requirement 6**. **Those three numbers were re-used by that phase's 2026-08-28 re-plan and now name different requirements**, which is why every pointer here carries the date of the list it read — see [`phase4_nothing_invisible.md`](phase4_nothing_invisible.md) § *The produced half — MOVED* for the mapping. **The number is 6 because it is the next free one** — [Nothing a run relies on is invisible](phase4_nothing_invisible.md) keeps its number and its filename, since a number is identity and never rollout order. See [`roadmap.md`](roadmap.md) § *Phases*.
>
...
1. **"Producer" is defined**, in a sentence a check can be built from, and the definition names what it deliberately excludes. *(Carried verbatim from [Nothing a run relies on is invisible](phase4_nothing_invisible.md)'s **pre-split requirement 4, as it stood on 2026-08-18** — that number now names a different requirement there.)* **This requirement stays unchecked and the definition does not exist as a check today** — what changed since it was written is that it no longer has to be invented from priors. See § *The stores supplied the definition this phase was missing*.
...
3. **The gate reaches producers outside `scripts/helpers/measure/`**, with the population read **off disk, never off a hand-kept list**, and **every exclusion named and asserted** by name in the check itself. *(Carried verbatim from [Nothing a run relies on is invisible](phase4_nothing_invisible.md)'s **pre-split requirement 5, as it stood on 2026-08-18** — that number now names a different requirement there.)* The first extension target is named rather than left as a direction — see § *The first extension target, counted off disk*.
