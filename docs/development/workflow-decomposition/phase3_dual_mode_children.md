...
- **[Nothing a run relies on is invisible](phase4_nothing_invisible.md) meets this phase at requirement 2's verbosity clause, it does not block this phase, and it now rolls out FIRST.** *(Corrected 2026-08-29. This bullet used to say the echo contract carried an open trade about what a parent may silence, and that either order worked. **Both halves are stale**: that trade was RULED on 2026-08-28 — there is no silencing flag, the context line is printed by the run that CONSTRUCTED the context and not by one handed it — and the roadmap re-sequenced that phase ahead of this one the same day, because nine runners written before the run context exists are nine new members of the class it is collapsing.)* **The practical consequence for this phase is that the ruling is an INPUT rather than a negotiation:** these nine adapters are handed a run context and inherit its echo, and none of them invents its own answer. See [`roadmap.md`](roadmap.md) § *The order, and what each part waits on*.
- **Not gated on [What configuration a run absorbed](phase5_configuration_a_run_absorbed.md) or [Family alignment](phase2_family_alignment.md).**
...
- **Verbosity inversion** — a person running one child wants to see what it is doing; a parent running nine wants one line each. This fleet already gets the mechanism right: `verbose` is an explicit parameter, never sniffed from the terminal. **Do not add TTY detection here.** The fleet's existing choice — `verbose` as an explicit parameter — is a first-party fact and carries this ruling on its own. *(The supporting negative — no first-party source found recommends detection without an explicit override — **rests on two sources rather than a survey**, which [`research/synthesis.md`](research/synthesis.md) § *Gaps* names itself as thin for the weight a reader might put on it. It is stated here so a later run does not re-use it for a heavier claim; the ruling does not need it.)*
...
- [ ] Rule the five divergences in § *The five places standalone and parent-driven diverge* once, written down, before the first adapter — nine adapters each answering these independently is exactly the drift [Family alignment](phase2_family_alignment.md) exists to stop.
...
- [ ] Confirm requirement 2's verbosity ruling against [Nothing a run relies on is invisible](phase4_nothing_invisible.md)'s echo contract, which is ruled and rolls out first: the context line is unconditional and is printed by the run that CONSTRUCTED the context. A standalone runner constructs one and prints it; a child handed one does not reprint it. **Do not add a per-runner suppression flag** — that is the shape that ruling rejected.
...
- **Nine adapters is a copying event waiting to happen.** They will be written in one sitting, from one template, by one run — which is the exact condition that produced the drifted pairs [Family alignment](phase2_family_alignment.md) ruled on. Reach for the shared mechanism first and accept a slightly heavier first adapter.
