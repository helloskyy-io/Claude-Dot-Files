...
**AND THE INHERITED APPARATUS DOES NOT REACH A PYTHON RUNNER — measured 2026-08-27, correcting a claim this doc makes twice.** Both halves of [Phase 2](phase2_family_alignment.md)'s machinery are scoped to prompt markdown. The duplication ratchet's population is `ASSISTANT.rglob("prompts/*.md")` (`tests/unit/test_prompt_blocks_are_shared_not_copied.py:137`), so a runner under `scripts/` cannot add a row to its baseline and cannot turn it red — which makes the implementation step below saying this phase *"must not hand it nine new rows"* a description of a guard that cannot receive them. And `FAMILY_RULINGS` (`tests/unit/fork_vs_parameterize.py:308`) is keyed on prompt-fragment stems, with a validator that rejects any ruling naming no category from the `_minor` tier contract — whose eight categories are all categories of *prompt guidance*, so a ruling covering nine runners cannot be expressed in that mechanism at all.

**The RISK the notes name is real and the guard behind it is not**, which is the whole of the correction. It has already happened at seven: [`C-8tv8ewto`](../../../tracked/candidates/C-8tv8ewto.md) records seven existing runners carrying a byte-identical `try/except RuntimeError` block, found by a review agent rather than by any guard, with the ratchet green throughout. **Requirement 6 therefore costs a design decision rather than a lookup, and this doc does not make it** — extending a guard's population to the runner corpus, promoting `preflight.py`'s `parse_or_exit` before the nine are written, and accepting the corpus as unguarded-and-said-so are all defensible. The three are laid out in [`C-7q2m4xzb`](../../../tracked/candidates/C-7q2m4xzb.md) for triage; a `plan-verify` pass corrects a determined defect and does not rule a design choice.
