...
- **Sprint Tracking**, **Sprint Close-Out**, **Cross-Roadmap Integration Pattern** — no sprints here; this repo plans in named phases (`docs/development/sprint.md`)
  - **BUT NOT its nested § *Completion checkboxes are human-in-the-loop everywhere*.** That clause is a `###` under Sprint Tracking and therefore looks excluded — **it is not**, per the rule three bullets down: *a carve-out justified by one clause must not swallow the clauses beside it.* The Sprint-Tracking exclusion is justified by *we have no sprints in their sense*, and that reasoning does not reach checkbox authority, which is about planning artifacts generally. **A producing run read it the other way on 2026-08-09 and was not being careless — the structure genuinely says so.**
  - **It binds here, AND we diverge from it deliberately.** Operator ruling, same date: a dispatch **may** flip a checkbox for work in its own diff, and `review-pr` **must verify every flip**. See `config/rules/standards-governance.md` § Completion checkboxes. **The divergence is going upstream as an amendment**; until it lands, ours governs and this note is the record of it. A divergence recorded is a decision; a divergence unrecorded is drift.
