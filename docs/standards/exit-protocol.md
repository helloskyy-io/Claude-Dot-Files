...
⟨PHASE 5+⟩ adds `artifact` to the envelope as an additive field per §5, and the conformance check that reads it. **Not added to `CHILD_SCHEMA` today** — Phase 3 shipped hours ago and a required-field change mid-flight routes every conforming run to a human, which E2(c) measured as producing *silence* rather than an error.

> **RE-ASSIGNED OFF PHASE 4 ON 2026-08-11, WITH THE REASON, RATHER THAN LEFT TO BE READ AS DONE.** Phase 4 did not add `artifact` and reported eight of nine complete without ever naming this obligation — the marker was assigning work to a phase whose own doc had no requirement covering it, which is how a standard's rollout item disappears between a phase that finished and a phase that never inherited it. It is re-assigned rather than deferred to nobody, and it inherits **the same trigger requirement 1 names: the first non-`review-pr` parent that routes on a typed record.** The reasoning is that phase's own: `artifact` is the enforcement point for §3's closed disposition vocabulary, and there is exactly one child emitting a record today whose findings a parent already compares field-by-field against the durable block — so adding the field now is a producer with no consumer, which is the admission failure [Phase 6](../development/memory-management-framework/phase6_read_what_it_writes.md) exists to stop. **The trigger is recorded in [`phase4_fleet_migration.md` § Close-out](../development/memory-management-framework/phase4_fleet_migration.md), as an unchecked box with this reason**, so it is carried by the same mechanism as the two other items riding that trigger rather than by a marker nobody re-reads.
...
> **R5b's scope, stated so the row is not read as wider than it is.** *(This blockquote carried a `⟨PHASE 4⟩` marker until 2026-08-11. The marker's declared meaning is "this answer has not been measured yet" and R5b shipped in Phase 4, so the marker inverted its own contract and would have been deleted at ratification along with two that mean the opposite.)* R5b fires only where a caller can state the reference it dispatched against; `route`'s `expected_ref` parameter has **no default**, so a caller with none says so explicitly rather than skipping the rule silently. The one production caller today is `review_pr_workflow`, which builds it from its own `--pr` input and `gh repo view --json nameWithOwner` in the repository it is operating in — two values the child cannot influence.
...
- ***§2.1's `run_id` Publish cell and the blockquote beneath it.*** The cell reads `publishable — CHANGED by Phase 4` and the blockquote narrates the `internal`→`publishable` move and what it supersedes from Phase 3. Same rule as §2's Status column: at ratification the cell collapses to `publishable` and the blockquote to a one-line backward-looking provenance note. **Listed here on 2026-08-11 because the pass that added it did not add it**, which is the omission the bullet below records the list being written to prevent, committed one section over and one day later.*
