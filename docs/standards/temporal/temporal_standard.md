...
> *Vendored from `helloskyy-io/MDC-Master-Planning` · `standards/development/temporal/temporal_standard.md` on 2026-08-20 (`8cb99e3`).*
...
- Lives **inside its workflow's own folder**, beside the workflow it compiles — see §10 for the binding tree layout:
  - `modules/{module}/[{purpose}/]{name}/{name}_helper.py`
  - Example: `modules/common/provision/agent_join/agent_join_helper.py` beside `agent_join_workflow.py`
- **`{name}`** is the workflow's short name — it names the folder AND prefixes every file in it: `{name}_workflow.py`, `{name}_helper.py`, `{name}_activities.py`.
- A helper shared by more than one workflow is **promoted** to the parent folder (§10) rather than duplicated.
...
**Every `error_code` vocabulary registers here — binding, and the registration is not optional on the grounds that no standard owns the component yet.** A vocabulary's value is that a consumer (the retry layer, alert rules, an operator triaging a failure) can **enumerate** what a system is capable of emitting; one discoverable only by reading a module is not enumerable, and codes that never reach this index are invisible to every consumer downstream.

This index covers **both** external-system integrations (Tailscale, GitHub, Vault, ArgoCD) **and** in-repo vocabularies minted by our own plan-classification, reconcile and subprocess layers (`CLUSTER_RECONCILE_*`, `SEED_*`, `WORKLOAD_RECONCILE_*`, `KUBECONFIG_REGISTER_*`). The distinction carries no rule — both kinds register identically.

**Where the detail lives:** an entry points at the **per-system standard** that documents the vocabulary where one exists. Where no standard owns the component, the entry points at the **owning module's docstring**, which then carries the full code list. *A missing standard is never a reason to skip registration* — that gap is exactly how a vocabulary goes unregistered, and it has happened three times.

**Engineer-editable carve-out (mirrors [Worker Deployment §3.1](./worker_deployment_standard.md)'s, and for the same reason).** The index below is a **factual registry of what the code registers** — not architectural substance. **An engineer dispatch MAY add or correct an index entry for a vocabulary its own change mints, in the same PR**, without a separate human ruling. The code is the source of truth; where the index and the live vocabulary diverge, the code wins and the index is the defect.

**What stays human-in-the-loop:** the *shape* of this section, the retry-semantics rules in §6.3, and any entry that asserts something beyond "these codes exist and here is where their list lives" — a carve-out claim, a cross-vocabulary exemption, a change to what a code *means*. Adding a row is bookkeeping; changing the rules that govern rows is not.

**Why the carve-out exists:** requiring a ruling to record a fact makes registration the most expensive step in shipping a vocabulary, and the predictable result is that it gets skipped — four vocabularies have already shipped unregistered and were each caught by hand. A rule that is costly to obey and unenforced is a rule that decays.
...
- **Synaptron (customer-workload reconcile + image roll)** — `SYNAPTRON_*` codes; no per-system standard owns this workload, so the list lives in the `modules/workload/synaptron/` activities module docstring
- **Git (local repository operations)** — `GIT_*` codes; no git standard exists, so the list lives in the `activities/git/` package docstring. Distinct from `GITHUB_*`, which covers the remote API — a local `git` failure and a GitHub API failure are different fire-paths and MUST NOT share a prefix
- **Proxmox (cluster API operations)** — `PROXMOX_*` codes; no Proxmox standard exists, so the list lives in the `activities/proxmox/` package docstring
- **Workload image adoption (generic adopt LEGO)** — `WORKLOAD_ADOPT_*` / `WORKLOAD_IMAGE_*` / `WORKLOAD_MANIFEST_*` codes; no standard owns the generic adopt path, so the list and its retry dispositions live in `modules/workload/adopt_image/adopt_image_activities.py`'s docstring. **Distinct from [Worker Deployment §3.2](./worker_deployment_standard.md)'s `WORKLOAD_*` executor vocabulary**, which covers the four generic customer-workload executors on `workload-worker` and is a different set that happens to share the prefix. §3.2's pass-through obligation does not reach this workflow — it is built from its own executors and namespaces per caller, which §3.2 permits provided every code is enumerated with its disposition and dispositions survive the remap.
- **Firewall (edge read surface)** — `FIREWALL_*` codes; the list lives in `modules/infra/firewall/`. Note `FIREWALL_CAPABILITY_UNAVAILABLE` is one of the two sanctioned terminal `*_UNAVAILABLE` exceptions named in the retry-semantics rules below
...
├── common/                       # Common module — cross-cutting workflows (often children of others)
│   ├── ingest_github_token/      # WORKFLOW FOLDER — named for the workflow it holds
│   │   ├── ingest_github_token_workflow.py    # Layer 1 — orchestration
│   │   ├── ingest_github_token_helper.py      # Layer 2 — compiler
│   │   └── ingest_github_token_activities.py  # Layer 3a — semantic wrappers
│   └── provision/                # PURPOSE folder — a slice of work, groups related workflows
│       ├── join_base.py          # promoted: shared by MORE THAN ONE workflow below
│       ├── agent_join/           # workflow folder
│       │   ├── agent_join_workflow.py
│       │   ├── agent_join_helper.py
│       │   ├── agent_join_activities.py
│       │   └── agent_join_inputs.py           # optional fourth file
│       └── genesis/              # workflow folder
│           ├── genesis_workflow.py
│           ├── genesis_helper.py
│           └── genesis_activities.py
├── infra/                        # Infra module — physical host and OS state
├── vm/                           # VM module — virtual machine orchestration
├── container/                    # Container module — container workload orchestration
├── service/                      # Service module — internal services
└── workload/                     # Workload module — customer workloads
...
Note: Folders are named by **what they do**, not by workflow type. Folders are only created when workflows are implemented.

### §10.1 Placement rules (binding)

**1. One workflow, one folder.** Every workflow lives in a folder named for it (`{name}/`), and every file in that folder carries the `{name}_` prefix. A workflow file at a purpose- or module-root is non-conformant. This holds at any depth: a workflow folder may sit directly under a module (`common/ingest_github_token/`) or inside a purpose folder (`common/provision/agent_join/`).

**2. One workflow definition per workflow file.** A `{name}_workflow.py` declares exactly one `@workflow.defn`. Lifecycle families over one entity (deploy / remove / reposition / update) are separate workflows and get separate folders — they are related, not the same workflow.

**3. The promotion rule — the only test for what sits at a parent level.** A helper, activities, inputs, or support module is promoted OUT of a workflow folder to the parent (purpose or module) level **if and only if more than one workflow uses it**. A single-consumer module stays in its workflow's folder. This is mechanical: the consumer count decides, never taste. It is also what makes a shared module visually distinct — anything at a parent level is shared **by definition**, so a reader never has to open a file to learn its scope.

**4. Folder name and file prefix MUST match.** A folder named `config_apply/` holding `home_assistant_config_apply_*.py` is non-conformant — the mismatch defeats the point of naming the folder for its workflow. Pick one and use it in both.

**5. Purpose folders are optional; module folders are not.** A purpose folder (`provision/`, `reconcile/`) groups related workflows within a module and is used when the grouping earns its level. A workflow with no natural purpose sibling sits directly under its module.

**6. A workflow folder may hold only one file, and that is conformant.** When a lifecycle family shares one promoted trio, each member's folder holds just its `{name}_workflow.py`. This is the correct outcome of rules 2 and 3, not a defect — and it gives the workflow a place to grow its own helper when it earns one.

**7. Activity-only modules are a distinct, allowed shape.** A folder under `modules/` may hold semantic activity wrappers and inputs with **no workflow at all** — a shared wrapper library scoped to a domain. It is not a workflow folder, is not required to contain a trio, and MUST NOT be "corrected" into one.

**8. Fourth file: `{name}_inputs.py` is optional.** Per §11.2, input models live in the helper by default and extract to `{name}_inputs.py` when the helper grows unwieldy. Presence or absence is not a conformance signal.

**Breaking it looks like:** a `*_workflow.py` sitting at a purpose- or module-root; two `@workflow.defn` in one file; a helper at a parent level with exactly one consumer; a single-consumer helper duplicated across sibling folders; a folder whose name does not match its files' prefix; or an activity-only module refactored to invent a workflow it never had.
...
1. **Define the input model** in `{name}_helper.py` by default (**one class per semantic activity**, name aligned with the activity). Extract to `{name}_inputs.py` only if the helper becomes unwieldy.
2. **Compile** instances in the helper and set **`"input": model_dump() / asdict(...)`** on each plan step.
3. **Add a thin function** to the workflow's own `{name}_activities.py` (§10.1) with `@activity.defn(name="meaningful-kebab-case")` and a **single `dict` payload** parameter; **validate/reconstruct** the model at the start.
...
| `modules/{module}/[{purpose}/]{name}/{name}_workflow.py` | Workflow orchestration (Layer 1) — exactly one `@workflow.defn` per file (§10.1) |
| `modules/{module}/[{purpose}/]{name}/{name}_helper.py` | Workflow helper (compiler) — validation, execution plan, and per-step input dataclasses |
| `modules/{module}/[{purpose}/]{name}/{name}_inputs.py` | **Optional** extraction: one input type per semantic activity when the helper grows too large |
| `modules/{module}/[{purpose}/]{name}/{name}_activities.py` | Workflow-scoped semantic wrappers — `@activity.defn(name=...)`; one parameter (payload); unpack and delegate to `activities/` |
| `modules/{module}/[{purpose}/]<shared>.py` | Promoted module — shared by MORE THAN ONE sibling workflow (§10.1 rule 3) |
