...
| C-056 | A routing policy for a verification that could not be performed — should an unperformed check DOWNGRADE the verdict, or only annotate it? | PR #71 | | `open` | **The half of `flaky-gh-discards-a-completed-review` that is genuinely a policy question, separated from the half that was a defect.** PR #71 fixed the defect: the two `gh` reads behind the render↔record invariant are read-only, so they are retried, and on exhaustion the run completes on the already-persisted route with the check recorded as UNPERFORMED. **What is left is unruled and is capability, not a bug** — today the verdict stands unchanged and the operator gets a loud note saying *"verify the posted block by hand."* The alternative is that a route which could not be verified is downgraded to `HOLD - needs-assistance` automatically. **Consequence of leaving it: an operator can act on a `MERGE` whose render↔record invariant never ran**, and nothing but a note distinguishes that from one that ran and agreed. **It has no phase home** — `grep -E 'retry\|flaky\|transient' phase4_fleet_migration.md` returns only an unrelated `api_retry` enum checkbox, re-verified at placement. It generalises past this one check: the same question applies to every parent-side verification the fleet grows, which is why it is a policy row rather than a checkbox on one function. Note that **Temporal's retry is the wrong granularity here and would make it worse** — an activity retry is a new attempt, i.e. a second 40-minute review |
