...
| `raw/hermes_assessment.md` | 2026-08-06 | high — 3 weeks | **PASS-WITH-FIXES** (the paper's own header token) — the *body* passed at round 1 with no citation defects found; rounds 3–4 corrected bookkeeping in the paper's own header and one enumeration in §6(h) |
| `raw/edge_identity_trust.md` | 2026-08-06 | high — 6 weeks (mixed-volatility; slow sections marked `[LOW]` in the body) | **PASS** (round 2, after two repairs; one non-blocking note closed at round 3; a round-4 repair to §5's dangling search-method pointer independently re-verified with no blocking findings) |
...
**Every candidate now rests on a verified paper.** Candidates 1–16 rest on this cycle's four papers, all of which passed the critic gate on 2026-08-06; candidates 17–32 are carried forward from prior verified cycles. **30–32 were restored by a correction pass**, which checked this table against the 2026-08-05 synthesis candidate by candidate and found three of its nineteen present in neither the table nor the retirement note below — since `synthesis.md` is rewritten and never appended (`README.md` line 33), an unaccounted candidate ceases to exist on merge. Their source papers carry verdicts like every other row (`temporal.md` PASS-WITH-FIXES, two independent passes; `bernstein_capability_mining.md` PASS-WITH-FIXES; `edge_identity_trust.md` PASS). That removes the draft's blanket caveat — but it does not make any candidate binding, and the per-candidate confidence marks in the source papers still govern.
...
| 30 | **Amend the Serverless Workers reading in `dedicated_edge_routing.md`: AWS Lambda caps an activity at 15 minutes and therefore cannot host the 10–60 minute `claude_cli` activity — only Cloud Run is even shape-compatible.** The sibling *does* name both providers — its §4.2 negative finding quotes Temporal's *"supports two compute providers"* and enumerates AWS Lambda and GCP Cloud Run — but records **no per-provider shape constraint**; `temporal.md` §6.2 supplies it and draws the consequence. Where it bites: the sibling's §8 item 8 declines wake-on-task on availability grounds — this adds that half the mechanism could not have served our activity shape anyway, which strengthens a decision already taken rather than reopening one. **Carry it with its own caveat:** whether Serverless Workers runs against a *self-hosted* service at all is **undocumented first-party** (`temporal.md` §6.2, a gap established by enumerating both doc directories) | change direction | `temporal.md` §6.2 |
| 31 | **Note that no first-party Claude ↔ Temporal RUNTIME integration exists** — established by enumerating `temporalio/contrib` (**eleven** entries, counted from the enumeration; no `anthropic/`, no `claude/`) while OpenAI, Google ADK, LangGraph and Strands all have one. Temporal's Anthropic surface is *developer tooling* (Claude Code plugin, two Skills, a Knowledge Base MCP Server). **The hand-rolled `claude_cli` integration is not a temporary state to wait out**, and `Phase: Temporal Integration` — in flight now under `scripts/workflows/temporal/` — should be scoped on that basis. Secondary: the Knowledge Base MCP Server and Developer Skill are usable by this repo today and nothing in the pool has noted them | adopt | `temporal.md` §6.3 |
| 32 | **Add the trust-domain claim to `problem-statement.md`, scoped to agent orchestration.** bernstein's cross-instance model is first-party single-operator and single-trust-domain (*"not multi-tenant in the security sense"*; cross-org federation listed out of scope), so our destination — many MDCs, distinct operators, distinct trust domains — is outside its shipped scope. **Narrowed, not superseded, by this cycle:** `edge_identity_trust.md` §7(b) affirms the bernstein fact as first-party and current, and rules that it establishes only that *the nearest agent orchestrator* has not done it — against workload identity / zero trust / distributed CI the property is ordinary. So the claim goes in **with the reference class stated**, not as a general novelty. Candidate 1 absorbs bernstein's *"not multi-tenant"* line as empirical support for differentiator #1 and candidate 18 replaces differentiator #2's wording with the credential version, but **neither carries this edit** — checked against `dedicated_edge_routing.md` §7's drafted replacement text, which contains no trust-domain sentence | adopt | `bernstein_capability_mining.md` §0.2; `edge_identity_trust.md` §7(b) |
...
**Carry-forward accounting — all nineteen of the 2026-08-05 candidates are now dispositioned.** Prior **#1/#3/#4/#7/#8/#10/#11/#14/#15/#18/#19** are candidates 27/28/29/17/18/20/21/23/24/26/19 here; **#2** and **#13** are retired above; **#12/#16/#17** are folded above; and **#5/#6/#9** are carried as **30/31/32**, having been absent from the draft of this synthesis entirely. **Why #5 is carried and not retired:** `topics.md` § *Deliberately NOT re-opened* rules out "Temporal Cloud pricing, billable Actions, serverless worker patterns" because `system-overview.md` § *Deployment target* settles self-hosting. That exclusion governs **what new research is commissioned**, and it is aimed at *Cloud* — worker hosting is a separate object from the server's deployment target, and `temporal.md` §6.2 records as a gap that first-party docs do not state whether Serverless Workers requires Cloud at all. Retiring an already-established, already-verified finding under a topic-selection rule would delete evidence rather than decline effort, so it is carried with that ambiguity attached and left for the reviewer to rule on. **Zero new research was spent on any of the three** — the text existed on the prior synthesis; this pass verified the anchors still resolve and wrote the disposition reasoning.

---

## Homeless findings

Named here rather than parked elsewhere, per §7 — a homeless finding means the surface is missing. **The first entry is a WITHDRAWAL**, kept in place rather than deleted so the correction is visible to anyone who read the prior four cycles: the surface it claimed was missing exists, and had already been used.

- **WITHDRAWN — "this repo has no surface that holds an upstream standards amendment we owe" is FALSE, and was already false on the day this synthesis was written.** One half stands: the Research Standard is **vendored MIRROR** from `MDC-Master-Planning`, so an amendment genuinely cannot be made in this repo. The other half does not. The standup tracker (issue #26) carries **T-10 · *"Upstream standards amendments owed to the skyycommand team"***, `state:` **resolved**, `resolved:` **2026-08-05** — its body recording that round 1 *"landed upstream at `6fe3829`"*, that Temporal §10.1 ruled folder-per-workflow binding and Research §3 *"absorbed all three sourcing amendments"*, and that round 2 was *"withdrawn as over-generated"*. **The landing is verifiable downstream inside this repo, in the file this synthesis cites throughout:** `docs/standards/research/research_standard.md` §3 now carries this pool's own three sourcing amendments — *"Verbatim means the exact characters were returned"* (line 106), *"A count is a claim and carries a claim's sourcing burden"* (107), *"A repaired span is a new claim"* (108) — under a header recording the re-vendor from `helloskyy-io/MDC-Master-Planning` on **2026-08-05**. The surface not only exists, it was exercised **on this pool's own findings, one day before this synthesis was written**, and issue #26's tracker rule 3 states where the next one goes: *"Anything with a clean done-state and a proposed action is an ISSUE, not a line here."*

  **What survives is a bookkeeping point, and it stands on its own.** Prior wording put the debt at "five owed amendments." That number was never checkable — nothing in `research/` enumerates the owed items, and §3 is explicit that a count of a population that cannot be enumerated is stated as a gap, never asserted. It is also stale in at least one direction: this cycle retires one of the five, since the sizing-rubric item needs the standard *applied* rather than amended (see the sizing bullet below), and the count was never adjusted. Stated against T-10 rather than asserted: **round 1 landed and round 2 was withdrawn as over-generated, so nothing is RECORDED as currently owed — and nothing in `research/` enumerates what a round 3 would contain**, which is why the tally could neither be confirmed nor corrected. *(This run's inference, not a paper's finding: the nearest round-3 candidate is the fetch-tooling bullet below — §3's verbatim rule being unsatisfiable by the available tooling. Whether that is a standard amendment or a §8 tooling-repo change is the reviewer's ruling, not this run's.)* The residual ask is hygiene, not a missing surface: file an amendment as an issue per tracker rule 3 **when the cycle generates it**, so a debt is never again carried as an unenumerable tally.
