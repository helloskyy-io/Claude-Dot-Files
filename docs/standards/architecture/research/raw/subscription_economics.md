...
**(a) Only one of the three elements is token-expensive.** By the problem statement's own definitions: element 1 (durable execution) is *"Mature technology, borrowed rather than invented"* whose cost is engineering time; element 3 (typed memory) is explicitly *"read in code, with no model in the loop"* — zero token cost by construction; element 4 (high-level loops) is *"if/then/else over the results of entire workflows"* — also code. Only element 2, layered self-improvement, spends tokens at scale. **Affordability therefore enables one of the three elements, not "the other three."** This is derivable from the problem statement's own text and is the strongest single objection in this section.
...
21. GitHub. *Requests in GitHub Copilot.* https://docs.github.com/en/copilot/concepts/billing/copilot-requests (fetched 2026-08-03)
...
33. `docs/standards/architecture/research/raw/anthropic_tos_and_enterprise.md` — permissibility of edge-held subscription auth. `Last validated: 2026-07-24`, `Critic: PASS`, `Revalidate: high — 4 weeks` → **past window; treated as unverified throughout.**
34. `docs/standards/architecture/research/raw/claude_code_integration_surface.md` — `claude -p` invocation surface, `--output-format json`, `total_cost_usd`. `Last validated: 2026-07-25`, `Critic: PASS`.
35. `docs/standards/architecture/research/raw/production_cases.md` — what other teams hit. `Last validated: 2026-07-23`, `Critic: PASS`.
36. `docs/standards/research/research_standard.md` §2 — records a 5-topic research cycle at *"~$58 / 44 min"*. Cited as an in-house order-of-magnitude datapoint for one bounded autonomous run; **the figure's provenance (metered spend vs. client-side estimate) is not stated in the source, so it is treated as unverified.**
...
**T2 — Establish the ceiling empirically, per plan.** T1 gives one point; T2 turns it into a planning number. Repeat across the plan tiers actually in use. Without this, no phase doc can state how many unattended loop-hours per week the architecture has to work with.
